...
if typing.TYPE_CHECKING:
    ...
    from harpiya.orm.environments import Transaction
...
class Cursor(BaseCursor):
    ...
    def execute_values(self, query, argslist, template=None, page_size=100, fetch=False):
        ...
        """
        # Harpiya Cursor only proxies all methods of psycopg2 Cursor. This is a patch for problems caused by passing
...
def connection_info_for(db_or_uri: str, readonly=False) -> tuple[str, dict]:
    ...
    :rtype: (str, dict)
    """
    if 'HARPIYA_PGAPPNAME' in os.environ:
        # Using manual string interpolation for security reason and trimming at default NAMEDATALEN=63
        app_name = os.environ['HARPIYA_PGAPPNAME'].replace('{pid}', str(os.getpid()))[0:63]
    else:
        app_name = "harpiya-%d" % os.getpid()
...
def db_connect(to: str, allow_uri=False, readonly=False) -> Connection:
    ...
    maxconn = (tools.config['db_maxconn_gevent'] if hasattr(harpiya, 'evented') and harpiya.evented else 0) or tools.config['db_maxconn']
...
def close_db(db_name: str) -> None:
    """ You might want to call harpiya.modules.registry.Registry.delete(db_name) along this function."""
    if _Pool:
