...
from .suite import HarpiyaSuite
from .result import HarpiyaTestResult
...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'harpiya.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"harpiya.upgrade.{module}",
        f"harpiya.addons.{module}.migrations",
        f"harpiya.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return HarpiyaSuite(sorted(tests, key=lambda t: getattr(t, 'test_sequence', 0)))
...
def run_suite(suite, global_report=None):
    ...
    results = HarpiyaTestResult(global_report=global_report)
    suite(results)
