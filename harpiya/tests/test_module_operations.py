...
from harpiya.tools import config, topological_sort, unique
from harpiya.modules.registry import Registry
from harpiya.netsvc import init_logger
from harpiya.tests import standalone_tests
import harpiya.tests.loader

_logger = logging.getLogger('harpiya.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of Harpiya modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=addons_path,
        help="Comma-separated list of paths to directories containing extra Harpiya modules")
...
def test_standalone(args):
    ...
    for module_name in registry._init_modules:
        ...
        harpiya.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    if args.addons_path:
        ...
        harpiya.modules.module.initialize_sys_path()
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'harpiya.modules.loading': {'level': 'CRITICAL'},
            'harpiya.sql_db': {'level': 'CRITICAL'},
            'harpiya.models.unlink': {'level': 'WARNING'},
            'harpiya.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
