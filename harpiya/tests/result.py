...
HARPIYA_TEST_MAX_FAILED_TESTS = max(1, int(os.environ.get('HARPIYA_TEST_MAX_FAILED_TESTS', sys.maxsize)))

stats_logger = logging.getLogger('harpiya.tests.stats')
...
class HarpiyaTestResult(object):
    """
    ...
    def _checkShouldStop(self):
        if self.total_errors_count() >= HARPIYA_TEST_MAX_FAILED_TESTS:
            ...
            if not global_report.shouldStop:
                _logger.error(
                    ...
                    "Remaining tests will be skipped.", HARPIYA_TEST_MAX_FAILED_TESTS)
