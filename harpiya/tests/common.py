# -*- coding: utf-8 -*-
"""
The module :mod:`harpiya.tests.common` provides unittest test cases and a few
...
import harpiya.orm.registry
from harpiya import api
from harpiya.exceptions import AccessError
from harpiya.fields import Command
from harpiya.modules.registry import Registry
from harpiya.service import security
from harpiya.sql_db import BaseCursor, Cursor, Savepoint
from harpiya.tools import config, float_compare, mute_logger, profiler, SQL, DotDict
from harpiya.tools.mail import single_email_re
from harpiya.tools.misc import find_in_path, lower_logging
from harpiya.tools.xml_utils import _validate_xml
...
def __getattr__(name):
    ...
    warnings.warn(
        "Since 18.0: harpiya.tests.common.Form is deprecated, use harpiya.tests.Form",
...
# The harpiya library is supposed already configured.
HOST = '127.0.0.1'
...
def get_db_name():
    dbnames = harpiya.tools.config['db_name']
...
def standalone(*tags):
    ...
    ``tags`` and the corresponding Harpiya module name.
    """
    def register(func):
        # register func by harpiya module name
        if func.__module__.startswith('harpiya.addons.'):
...
def new_test_user(env, login='', groups='base.group_user', context=None, **kwargs):
     ...
     * email: it is either the login (if it is a valid email) or a generated
       ...
       to email being required for most harpiya operations;
    """
    if not login:
...
class BaseCase(case.TestCase):
    """ Subclass of TestCase for Harpiya-specific code. This class is abstract and
    ...
    def __init_subclass__(cls):
        ...
        if cls.__module__.startswith('harpiya.addons.'):
            if getattr(cls, 'test_tags', None) is None:
                cls.test_tags = {'standard', 'at_install'}
            cls.test_module = cls.__module__.split('.')[2]

    longMessage = True      # more verbose error message by default: https://www.harpiya.com/r/Vmh
    warm = True             # False during warm-up phase (see :func:`warmup`)
    _python_version = sys.version_info

    _tests_run_count = int(os.environ.get('HARPIYA_TEST_FAILURE_RETRIES', 0)) + 1
    ...
    def browse_ref(self, xid):
        ...
        :param xid: fully-qualified :term:`external identifier`, in the form
                    :samp:`{module}.{identifier}`
        :raise: ValueError if not found
        :returns: :class:`~harpiya.models.BaseModel`
        """
    ...
    def debug_mode(self):
        ...
        request = Mock(
            ...
            env=self.env,
            session=DotDict(harpiya.http.get_default_session(), debug='1'),
        )
        try:
            self.env.flush_all()
            self.env.invalidate_all()
            harpiya.http._request_stack.push(request)
            yield
            self.env.flush_all()
            self.env.invalidate_all()
        finally:
            popped_request = harpiya.http._request_stack.pop()
    ...
    def _patchExecute(self, actual_queries, flush=True):
        ...
        with (
            patch('harpiya.sql_db.Cursor.execute', execute),
        ...
        if self.warm:
            ...
            with patch('random.random', lambda: 1):
                ...
                if count != expected:
                    ...
                    if "/harpiya/addons/" in filename:
                        filename = filename.rsplit("/harpiya/addons/", 1)[1]
                    if count > expected:
    ...
    def assertRecordValues(
            self,
            records: harpiya.models.BaseModel,
...
class Approx:  # noqa: PLW1641
    ...
    """
    def __init__(self, value: float, rounding: int | float | harpiya.addons.base.models.res_currency.Currency, /, decorate: bool) -> None:  # noqa: PYI041
        self.value = value
...
class TransactionCase(BaseCase):
    ...
    cr: Cursor = None
    muted_registry_logger = mute_logger(harpiya.orm.registry._logger.name)
    freeze_time = None
    ...
    def setUpClass(cls):
        ...
        cls._crypt_context_patcher = patch('harpiya.addons.base.models.res_users.ResUsersPatchedInTest._crypt_context', _crypt_context)
...
def save_test_file(test_name, content, prefix, extension='png', logger=_logger, document_type='Screenshot', date_format="%Y%m%d_%H%M%S_%f"):
    ...
    screenshots_dir = pathlib.Path(harpiya.tools.config['screenshots']) / get_db_name() / 'screenshots'
...
class ChromeBrowser:
    ...
    def __init__(self, test_case: HttpCase, success_signal: str = DEFAULT_SUCCESS_SIGNAL, headless: bool = True, debug: bool = False):
        ...
        self.user_data_dir = tempfile.mkdtemp(suffix='_chrome_harpiya')

        otc = harpiya.tools.config
...
which leads to stray network requests and inconsistencies."""
    ...
    def _from_remoteobject(self, arg):
        ...
        # of the way Harpiya widgets are created they all appear as Class(...)
...
class HttpCase(TransactionCase):
    ...
    @classmethod
    def http_port(cls):
        if harpiya.service.server.server is None:
            return None
        return harpiya.service.server.server.httpd.server_port
    ...
    def _wait_remaining_requests(self, timeout=10):
        ...
        def get_http_request_threads():
            return [t for t in threading.enumerate() if t.name.startswith('harpiya.service.http.request.')]
        ...
        if request_threads:
            ...
            harpiya.tools.misc.dumpstacks()

    def logout(self, keep_db=True):
        self.session.logout(keep_db=keep_db)
        harpiya.http.root.session_store.save(self.session)

    def authenticate(self, user, password, browser: ChromeBrowser = None):
        if getattr(self, 'session', None):
            harpiya.http.root.session_store.delete(self.session)

        self.session = session = harpiya.http.root.session_store.new()
        session.update(harpiya.http.get_default_session(), db=get_db_name())
        session.context['lang'] = harpiya.http.DEFAULT_LANG
        ...
        if user: # if authenticated
            ...
            with patch('harpiya.addons.base.models.res_users.ResUsersPatchedInTest._check_credentials', new=patched_check_credentials):
        ...
        harpiya.http.root.session_store.save(session)
    ...
    def browser_js(self, url_path, code, ready='', login=None, timeout=60, cookies=None, error_checker=None, watch=False, success_signal=DEFAULT_SUCCESS_SIGNAL, debug=False, cpu_throttling=None, **kw):
        ...
        try:
            if "bus.bus" in self.env.registry:
                from harpiya.addons.bus.websocket import CloseCode, _kick_all, WebsocketConnectionHandler
                from harpiya.addons.bus.models.bus import BusBus
            ...
            cpu_throttling_os = os.environ.get('HARPIYA_BROWSER_CPU_THROTTLING') # used by dedicated runbot builds
    ...
    def start_tour(self, url_path, tour_name, step_delay=None, **kwargs):
        ...
        options = {
            ...
            'startUrl': url_path,
            'delayToCheckUndeterminisms': kwargs.pop('delay_to_check_undeterminisms', int(os.getenv("HARPIYA_TOUR_DELAY_TO_CHECK_UNDETERMINISMS", "0")) or 0),
        }
        code = kwargs.pop('code', f"harpiya.startTour({tour_name!r}, {json.dumps(options)})")
        ready = kwargs.pop('ready', f"harpiya.isTourReady({tour_name!r})")
    ...
    def profile(self, **kwargs):
        ...
        return profiler.Nested(_profiler, patch('harpiya.http.Request._get_profiler_context_manager', route_profiler))
...
def tagged(*tags):
    ...
    By default, all Test classes from harpiya.tests.common have a test_tags
...
class freeze_time:
    """ Object to replace the freezegun in Harpiya test suites
