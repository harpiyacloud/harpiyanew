...
import harpiya.api
import harpiya.modules.neutralize
import harpiya.release
import harpiya.sql_db
import harpiya.tools
from harpiya.exceptions import AccessDenied
from harpiya.release import version_info
from harpiya.sql_db import db_connect
from harpiya.tools import osutil, SQL
from harpiya.tools.misc import exec_pg_environ, find_pg_tool
...
def check_super(passwd):
    if passwd and harpiya.tools.config.verify_admin_password(passwd):
        return True
    raise harpiya.exceptions.AccessDenied()

# This should be moved to harpiya.modules.db, along side initialize().
def _initialize_db(db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        harpiya.tools.config['load_language'] = lang

        registry = harpiya.modules.registry.Registry.new(db_name, update_module=True, new_db_demo=demo)

        with closing(registry.cursor()) as cr:
            env = harpiya.api.Environment(cr, harpiya.api.SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = harpiya.tools.email_split(login)
                if emails:
...
def _create_empty_database(name):
    db = harpiya.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = harpiya.tools.config['db_template']
    ...
    try:
        db = harpiya.sql_db.db_connect(name)
        with db.cursor() as cr:
            cr.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
            if harpiya.tools.config['unaccent']:
                ...
                # But in the case of Harpiya, we consider that those data don't
...
def exp_duplicate_database(db_original_name, db_name, neutralize_database=False):
    ...
    harpiya.sql_db.close_db(db_original_name)
    db = harpiya.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = harpiya.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = harpiya.api.Environment(cr, harpiya.api.SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)
        if neutralize_database:
            harpiya.modules.neutralize.neutralize_database(cr)

    from_fs = harpiya.tools.config.filestore(db_original_name)
    to_fs = harpiya.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    harpiya.modules.registry.Registry.delete(db_name)
    harpiya.sql_db.close_db(db_name)
    ...
    fs = harpiya.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'harpiya_dump': '1',
        'db_name': cr.dbname,
        'version': harpiya.release.version,
        'version_info': harpiya.release.version_info,
        'major_version': harpiya.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip'):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            filestore = harpiya.tools.config.filestore(db_name)
            if os.path.exists(filestore):
                shutil.copytree(filestore, os.path.join(dump_dir, 'filestore'))
            with open(os.path.join(dump_dir, 'manifest.json'), 'w') as fh:
                db = harpiya.sql_db.db_connect(db_name)
                with db.cursor() as cr:
...
def restore_db(db, dump_file, copy=False, neutralize_database=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        registry = harpiya.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = harpiya.api.Environment(cr, harpiya.api.SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    harpiya.modules.registry.Registry.delete(old_name)
    harpiya.sql_db.close_db(old_name)
    ...
    old_fs = harpiya.tools.config.filestore(old_name)
    new_fs = harpiya.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    harpiya.tools.config.set_admin_password(new_password)
    harpiya.tools.config.save(['admin_passwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        harpiya.modules.registry.Registry.new(db, update_module=True, upgrade_modules={'base'})
    return True
...
@harpiya.tools.mute_logger('harpiya.sql_db')
...
def list_dbs(force=False):
    if not harpiya.tools.config['list_db'] and not force:
        raise harpiya.exceptions.AccessDenied()

    if not harpiya.tools.config['dbfilter'] and harpiya.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, Harpiya will not
        ...
        return sorted(harpiya.tools.config['db_name'])
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of Harpiya
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if harpiya.tools.sql.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        harpiya.sql_db.close_db(database_name)
...
def exp_list_lang():
    return harpiya.tools.misc.scan_languages()

def exp_list_countries():
    list_countries = []
    root = ET.parse(os.path.join(harpiya.tools.config.root_path, 'addons/base/data/res_country_data.xml')).getroot()
...
def exp_server_version():
    ...
    """
    return harpiya.release.version
