...
from harpiya import api, sql_db
from harpiya.modules.registry import Registry
from harpiya.release import nt_service_name
from harpiya.tools import config, osutil, OrderedSet
from harpiya.tools.cache import log_ormcache_stats
from harpiya.tools.misc import stripped_sys_argv, dumpstacks
...
def set_limit_memory_hard():
    ...
    import harpiya  # for eventd
    if harpiya.evented and config['limit_memory_hard_gevent']:
...
class RequestHandler(CommonRequestHandler):
    def setup(self):
        ...
        me.name = 'harpiya.service.http.request.%s' % (me.ident,)
...
class ThreadedWSGIServerReloadable(LoggingBaseWSGIServerMixIn, werkzeug.serving.ThreadedWSGIServer):
    ...
    def __init__(self, host, port, app):
        # The HARPIYA_MAX_HTTP_THREADS environment variable allows to limit the amount of concurrent
        ...
        self.max_http_threads = os.environ.get("HARPIYA_MAX_HTTP_THREADS")
...
class FSWatcherWatchdog(FSWatcherBase):
    def __init__(self):
        ...
        import harpiya.addons  # noqa: PLC0415
        for path in harpiya.addons.__path__:
...
class FSWatcherInotify(FSWatcherBase):
    ...
    def start(self):
        self.started = True
        self.thread = threading.Thread(target=self.run, name="harpiya.service.autoreload.watcher")
...
class ThreadedServer(CommonServer):
    ...
    def cron_thread(self, number):
        ...
        from harpiya.addons.base.models.ir_cron import IrCron  # noqa: PLC0415
    ...
    def cron_spawn(self):
        ...
        for i in range(config['max_cron_threads']):
            t = threading.Thread(target=self.cron_thread, args=(i,), name=f"harpiya.service.cron.cron{i}")
            t.daemon = True
            t.type = 'cron'
            t.start()
    ...
    def run(self, preload=None, stop=False):
        ...
        if stop:
            if config['test_enable']:
                from harpiya.tests.result import _logger as logger  # noqa: PLC0415
...
class Worker(object):
    ...
    def setproctitle(self, title=""):
        setproctitle('harpiya: %s %s %s' % (self.__class__.__name__, self.pid, title))
...
class WorkerHTTP(Worker):
    ...
    def __init__(self, multi):
        ...
        # The HARPIYA_HTTP_SOCKET_TIMEOUT environment variable allows to control socket timeout for
        ...
        sock_timeout = os.environ.get("HARPIYA_HTTP_SOCKET_TIMEOUT")
...
def load_server_wide_modules():
    from harpiya.modules.module import load_openerp_module  # noqa: PLC0415
...
def preload_registries(dbnames):
    ...
    for dbname in dbnames:
        try:
            ...
            if config['test_enable']:
                from harpiya.tests import loader  # noqa: PLC0415
                t0 = time.time()
...
def start(preload=None, stop=False):
    """ Start the harpiya http server and cron processor.
    """
    global server

    load_server_wide_modules()
    import harpiya.http  # noqa: PLC0415

    if harpiya.evented:
        server = GeventServer(harpiya.http.root)
    elif config['workers']:
        if config['test_enable']:
            _logger.warning("Unit testing in workers mode could fail; use --workers 0.")

        server = PreforkServer(harpiya.http.root)
    else:
        if platform.system() == "Linux" and sys.maxsize > 2**32 and "MALLOC_ARENA_MAX" not in os.environ:
            ...
            # which Harpiya is based upon in order to limit the memory usage for threaded workers.
        ...
        server = ThreadedServer(harpiya.http.root)

    watcher = None
    if 'reload' in config['dev_mode'] and not harpiya.evented:
        if inotify:
