...
It will initialize the `harpiya.evented` variable.
"""
import harpiya
import sys

harpiya.evented = False


def patch_evented():
    if harpiya.evented or not (len(sys.argv) > 1 and sys.argv[1] == 'gevent'):
        return
    ...
    def gevent_wait_callback(conn, timeout=None):
        ...
        # under the BSD license (see in harpiya/debian/copyright)
        while 1:
            state = conn.poll()
