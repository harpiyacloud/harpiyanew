...
from harpiya.tools.arabic_reshaper import reshape
from harpiya.tools.parse_version import parse_version
...
def add_banner(pdf_stream, text=None, logo=False, thickness=2 * cm):
    """ Add a banner on a PDF in the upper right corner, with Harpiya's logo (optionally).

    :param pdf_stream (BytesIO):    The PDF stream where the banner will be applied.
    :param text (str):              The text to be displayed.
    :param logo (bool):             Whether to display Harpiya's logo in the banner.
    ...
    harpiya_logo = Image.open(file_open('base/static/img/main_partner-image.png', mode='rb'))
    harpiya_color = colors.Color(113 / 255, 75 / 255, 103 / 255, 0.8)
    ...
    for p in range(old_pdf.getNumPages()):
        ...
        can.setFillColor(harpiya_color)
        ...
        logo and can.drawImage(
            ImageReader(harpiya_logo), 0.25 * thickness, -2.05 * thickness, 40, 40, mask='auto', preserveAspectRatio=True)
...
class HarpiyaPdfFileReader(PdfFileReader):
...
class HarpiyaPdfFileWriter(PdfFileWriter):
    ...
    def embed_harpiya_attachment(self, attachment, subtype=None):
        assert attachment, "embed_harpiya_attachment cannot be called without attachment."
