...
def _get_idref(self, env, model_str, idref):
    from harpiya import fields, release  # noqa: PLC0415
    idref2 = dict(idref,
    ...
    if node.tag in ('field','value'):
        ...
        if a_eval := node.get('eval'):
            ...
            try:
                ...
                logging.getLogger('harpiya.tools.convert.init').error(
        ...
        from harpiya.models import BaseModel  # noqa: PLC0415
...
form: module.record_id""" % (xml_id,)
    ...
    def _tag_menuitem(self, rec, parent=None):
        ...
        from harpiya.fields import Command  # noqa: PLC0415
        groups = []
    ...
    def _tag_root(self, el):
        for rec in el:
            ...
            try:
                ...
                raise ParseError(msg) from None  # Restart with "--log-handler harpiya.tools.convert:DEBUG" for complete traceback
    ...
    def parse(self, de):
        assert de.tag in self.DATA_ROOTS, "Root xml tag must be <openerp>, <harpiya> or <data>."
        self._tag_root(de)
    DATA_ROOTS = ['harpiya', 'data', 'openerp']
