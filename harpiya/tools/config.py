...
from harpiya import release
from harpiya.tools.func import classproperty
from . import appdirs
...
class _HarpiyaOption(optparse.Option):
...
class _FileOnlyOption(_HarpiyaOption):
...
class _PosixOnlyOption(_HarpiyaOption):
    def __init__(self, *opts, **attrs):
        if os.name != 'posix':
...
class configmanager:
    def __init__(self):
        ...
        # dictionary mapping option destination (keys in self.options) to HarpiyaOptions.
        self.options_index = {}
    ...
    @property
    def rcfile(self):
        self._warn("Since 19.0, use harpiya.tools.config['config'] instead", DeprecationWarning, stacklevel=2)
        return self['config']

    @rcfile.setter
    def rcfile(self, rcfile):
        self._warn(f"Since 19.0, use harpiya.tools.config['config'] = {rcfile!r} instead", DeprecationWarning, stacklevel=2)
        self._runtime_options['config'] = rcfile

    def _build_cli(self):
        HarpiyaOption = type('HarpiyaOption', (_HarpiyaOption,), {'config': self})
        FileOnlyOption = type('FileOnlyOption', (_FileOnlyOption, HarpiyaOption), {})
        PosixOnlyOption = type('PosixOnlyOption', (_PosixOnlyOption, HarpiyaOption), {})

        version = "%s %s" % (release.description, release.version)
        parser = optparse.OptionParser(version=version, option_class=HarpiyaOption)
        ...
        parser.add_option(FileOnlyOption(dest='publisher_warranty_url', my_default='http://services.harpiya.com/publisher-warranty/', file_exportable=False))
        ...
        group.add_option("-s", "--save", action="store_true", dest="save", my_default=False, file_loadable=False,
                         help="save configuration to ~/.harpiyarc (or to ~/.openerp_serverrc if it exists)")
        ...
        group.add_option("-D", "--data-dir", dest="data_dir", type='path',  # sensitive default set in _load_default_options
                         help="Directory where to store Harpiya data")
        ...
        temp_tests_dir = os.path.join(tempfile.gettempdir(), 'harpiya_tests')
        ...
        group.add_option('--log-handler', action="append", type='comma', my_default=[':INFO'], metavar="MODULE:LEVEL",
                         help='setup a handler at LEVEL for a given MODULE. An empty MODULE indicates the root logger. '
                              'This option can be repeated. Example: "harpiya.orm:DEBUG" or "werkzeug:CRITICAL" (default: ":INFO")')
        group.add_option('--log-web', action="append_const", dest="log_handler", const=("harpiya.http:DEBUG",),
                         help='shortcut for --log-handler=harpiya.http:DEBUG')
        group.add_option('--log-sql', action="append_const", dest="log_handler", const=("harpiya.sql_db:DEBUG",),
                         help='shortcut for --log-handler=harpiya.sql_db:DEBUG')
        ...
        group = optparse.OptionGroup(parser, "Internationalisation options",
            "Use these options to translate Harpiya to another language. "
    ...
    def _load_default_options(self):
        ...
        if rcfilepath := os.getenv('HARPIYA_RC'):
            pass
        elif rcfilepath := os.getenv('OPENERP_SERVER'):
            self._warn("Since ages ago, the OPENERP_SERVER environment variable has been replaced by HARPIYA_RC", DeprecationWarning)
        elif os.name == 'nt':
            rcfilepath = os.path.join(os.path.abspath(os.path.dirname(sys.argv[0])), 'harpiya.conf')
        elif os.path.isfile(rcfilepath := os.path.expanduser('~/.harpiyarc')):
            pass
        elif os.path.isfile(rcfilepath := os.path.expanduser('~/.openerp_serverrc')):
            self._warn("Since ages ago, the ~/.openerp_serverrc file has been replaced by ~/.harpiyarc", DeprecationWarning)
        else:
            rcfilepath = '~/.harpiyarc'
    ...
    def parse_config(self, args: list[str] | None = None, *, setup_logging: bool | None = None) -> None:
        ...
        This method initializes harpiya.tools.config and openerp.conf (the
        ...
        Typical usage of this method:
            ...
            harpiya.tools.config.parse_config(sys.argv[1:])
        """
        from harpiya import modules, netsvc  # noqa: PLC0415
        ...
        if setup_logging is not False:
            ...
            if setup_logging is None:
                warnings.warn(
                    "As of Harpiya 18, it's recommended to specify whether"
                    " you want Harpiya to setup its own logging (or want to"
                    " handle it yourself)",
    ...
    def _load_cli_options(self, opt):
        # harpiya.cli.command.main parses the config twice, the second time
