"""
This code is what let us use ES6-style modules in harpiya.
Classic Harpiya modules are composed of a top-level :samp:`harpiya.define({name},{dependencies},{body_function})` call.
This processor will take files starting with an `@harpiya-module` annotation (in a comment) and convert them to classic modules.
If any file has the ``/** harpiya-module */`` on top of it, it will get processed by this class.
It performs several operations to get from ES6 syntax to the usual harpiya one with minimal changes.
...
from harpiya.tools.misc import OrderedSet

_logger = logging.getLogger(__name__)

def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom harpiya modules.
    ...
    legacy_harpiya_define = get_aliased_harpiya_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_harpiya_define, module_path, dependencies),
    ]
    for s in steps:
        content = s(content)
    if legacy_harpiya_define:
        content += legacy_harpiya_define
    return content
...
def url_to_module_path(url):
    """
    Harpiya modules each have a name. (harpiya.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_harpiya_define(module_path, dependencies, content):
    """
    Wraps the current content (source code) with the harpiya.define call.
    ...
    """
    return f"""harpiya.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
HARPIYA_MODULE_RE = re.compile(r"""
    \s*                             # starting white space
    \/(\*|\/)                       # /* or //
    .*                              # any comment in between (optional)
    @harpiya-module                    # '@harpiya-module' statement
...
def is_harpiya_module(url, content):
    """
    Detect if the file is a native harpiya module.
    We look for a comment containing @harpiya-module.

    :param url:
    :param content: source code
    :return: is this a harpiya module that need transpilation ?
    """
    result = HARPIYA_MODULE_RE.match(content)
...
def get_aliased_harpiya_define_content(module_path, content):
    """
    ...
    Example:
    If we have a require call somewhere in the harpiya source base being:
    ...
    > harpiya.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @harpiya-module */
        // after
        /** @harpiya-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @harpiya-module */
        // after
        /** @harpiya-module alias=web.AbstractAction default=false */

    :return: the alias content to append to the source code.
    """
    matchobj = HARPIYA_MODULE_RE.match(content)
    if matchobj:
        alias = matchobj['alias']
        if alias:
            if matchobj['default']:
                return """\nharpiya.define(`%s`, ['%s'], function (require) {
                        return require('%s');
