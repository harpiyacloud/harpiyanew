...
from harpiya.tools import pdf
from harpiya.tools.misc import file_open
from harpiya.tools.pdf import reshape_text
import io
...
class TestPdf(TransactionCase):
    ...
    def setUp(self):
        ...
        self.minimal_pdf_reader = pdf.HarpiyaPdfFileReader(self.minimal_reader_buffer)

    def test_harpiya_pdf_file_reader(self):
        ...
        r = pdf.HarpiyaPdfFileReader(io.BytesIO(out.getvalue()))
        self.assertEqual(len(list(r.getAttachments())), 1)

    def test_harpiya_pdf_file_writer(self):
        attachments = list(self.minimal_pdf_reader.getAttachments())
        self.assertEqual(len(attachments), 0)
        ...
        for count, (name, data) in enumerate([
            ...
            pdf_writer = pdf.HarpiyaPdfFileWriter()
    ...
    def test_harpiya_pdf_file_reader_with_owner_encryption(self):
        pdf_writer = pdf.HarpiyaPdfFileWriter()
        pdf_writer.cloneReaderDocumentRoot(self.minimal_pdf_reader)
        ...
        with io.BytesIO(encrypted_content) as reader_buffer:
            pdf_reader = pdf.HarpiyaPdfFileReader(reader_buffer)
    ...
    def test_merge_pdf(self):
        ...
        merged_pdf_reader = pdf.HarpiyaPdfFileReader(merged_reader_buffer)
    ...
    def test_branded_file_writer(self):
        ...
        self.assertEqual(pdf_info['/Producer'], 'Harpiya')
        self.assertEqual(pdf_info['/Creator'], 'Harpiya')
        reader_buffer.close()
