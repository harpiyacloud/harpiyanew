...
import harpiya
from harpiya.tests import TransactionCase
from harpiya.tools import file_path, file_open, file_open_temporary_directory
from harpiya.tools.config import configmanager


EMPTY_CONFIG_PATH = file_path('base/tests/config/empty.conf')
PROJECT_PATH = harpiya.tools.config.root_path.removesuffix('/harpiya')
DEFAULT_DATADIR = harpiya.tools.config._default_options['data_dir']
...
class TestConfigManager(TransactionCase):
    ...
    def setUp(self):
        super().setUp()
        patcher = patch.dict('os.environ', {'HARPIYA_RC': EMPTY_CONFIG_PATH})
        patcher.start()
    ...
    def test_01_default_config(self):
        self.assertConfigEqual({
            ...
            'proxy_access_token': '',
            'publisher_warranty_url': 'http://services.harpiya.com/publisher-warranty/',
            'reportgz': False,
    ...
    def test_02_config_file(self):
        ...
        with self.assertLogs('harpiya.tools.config', 'WARNING') as capture:
        ...
        self.assertEqual(capture.output, [
            "WARNING:harpiya.tools.config:option addons_path, no such directory '/tmp/harpiya', skipped",
            "WARNING:harpiya.tools.config:option upgrade_path, no such directory '/tmp/upgrade', skipped",
            "WARNING:harpiya.tools.config:test file '/tmp/file-file' cannot be found",
        ])
    ...
    def test_04_harpiya16_config_file(self):
        # test that loading the Harpiya 16.0 generated default config works
        ...
        output = [
            (f"WARNING:harpiya.tools.config:unknown option '{option}' in "
             f"the config file at {config_path}, option stored as-is, "
             "without parsing")
            for option in ('demo', 'geoip_database', 'osv_memory_age_limit')
        ] + [
            (f"WARNING:harpiya.tools.config:option {option} reads 'False' "
    ...
    def test_06_cli(self):
        ...
        self.assertEqual(capture.output, [
            "WARNING:harpiya.tools.config:option --without-demo: since 19.0, invalid boolean value: 'rigolo', assume True",
            "WARNING:harpiya.tools.config:test file '/tmp/file-file' cannot be found",
        ])
        ...
        self.assertConfigEqual({
            ...
            'log_handler': [
                ':WARNING',
                'harpiya.tools.config:DEBUG',
                'harpiya.http:DEBUG',
                'harpiya.sql_db:DEBUG',
            ],
            'log_db': 'logdb',
            'log_db_level': 'debug',
            'log_level': 'debug',
