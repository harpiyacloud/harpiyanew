...
from harpiya.tools.mimetypes import fix_filename_extension, get_extension, guess_mimetype
...
<Document xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns="urn:iso:std:iso:20022:tech:xsd:pain.001.001.03">
  <CstmrCdtTrfInitn>
    <GrpHdr>
      <MsgId>123456Harpiya Software Technologies, LLC893873733</MsgId>
      <CreDtTm>2018-11-21T09:47:32</CreDtTm>
      <NbOfTxs>0</NbOfTxs>
      <CtrlSum>0.0</CtrlSum>
      <InitgPty>
        <Nm>Harpiya Software Technologies, LLC</Nm>
        <Id>
          <OrgId>
            <Othr>
              <Id>BE0477472701</Id>
...
class test_guess_mimetype(BaseCase):
    ...
    def test_default_mimetype_empty(self):
        ...
        # harpiya implementation returns application/octet-stream by default
    ...
    def test_mimetype_svg(self):
        ...
        self.assertTrue(mimetype.startswith('image/svg'))
        # Tests that whitespace padded SVG are not detected as SVG in harpiya implementation
        if not magic:
    ...
    def test_mimetype_fix_extension(self):
        ...
        with self.assertLogs('harpiya.tools.mimetypes', 'WARNING') as capture:
            self.assertEqual(fix('image.txt', 'image/jpeg'), 'image.txt.jpg')
            self.assertEqual(fix('words.jpg', 'text/plain'), 'words.jpg.txt')
        self.assertEqual(capture.output, [
            "WARNING:harpiya.tools.mimetypes:File 'image.txt' has an invalid "
                "extension for mimetype 'image/jpeg', adding '.jpg'",
            "WARNING:harpiya.tools.mimetypes:File 'words.jpg' has an invalid "
