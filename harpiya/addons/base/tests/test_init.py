...
from harpiya.tests import BaseCase
...
class TestInit(BaseCase):
    ...
    def harpiya_modules_to_test(self):
        import harpiya.cli  # noqa: PLC0415
        for path in (*harpiya.__path__, *harpiya.cli.__path__):
            parent = Path(path)
            for module in parent.iterdir():
                if (module.is_dir() or module.suffix == '.py') and '__' not in module.name:
                    if parent.name == 'harpiya':
                        yield f"harpiya.{module.stem}"
                    else:
                        yield f"harpiya.{parent.name}.{module.stem}"
    ...
    def test_import(self):
        ...
        for module in sorted(self.harpiya_modules_to_test()):
