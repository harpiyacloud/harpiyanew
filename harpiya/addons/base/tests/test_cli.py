...
from harpiya.cli.command import commands, load_addons_commands, load_internal_commands
from harpiya.tools import config, file_path
from harpiya.tests import BaseCase, Like
...
class TestCommand(BaseCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.harpiya_bin = Path(__file__).parents[4].resolve() / 'harpiya-bin'
        addons_path = config.format('addons_path', config['addons_path'])
        cls.run_args = (sys.executable, cls.harpiya_bin, f'--addons-path={addons_path}')
    ...
    def test_docstring(self):
        ...
        for name, cmd in commands.items():
            self.assertTrue(cmd.__doc__,
                msg=f"Command {name} needs a docstring to be displayed with 'harpiya-bin help'")
            self.assertFalse('\n' in cmd.__doc__ or len(cmd.__doc__) > 120,
                msg=f"Command {name}'s docstring format is invalid for 'harpiya-bin help'")
        ...
        for name in ('bonbon', 'café'):
            with self.subTest(name):
                ...
                self.assertEqual(
                    ...
                    "Use 'harpiya-bin --help' to see the list of available commands."
                )
    ...
    def test_upgrade_code_standalone(self):
        from harpiya.cli import upgrade_code  # noqa: PLC0415
        proc = sp.run(
    ...
    def test_shell(self):
        ...
        self.assertEqual(shell.stdout.read().splitlines(), [
            ...
            Like("harpiya: <module 'harpiya' ...>"),
            Like("openerp: <module 'harpiya' ...>"),
            ">>> Hello from Python!",
            '>>> '
        ])
