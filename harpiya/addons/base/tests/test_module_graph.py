...
class TestGraph(BaseCase):
    ...
    def _test_graph_order(
        ...
        with (
            patch('harpiya.modules.module_graph.ModuleGraph._update_from_database'),
            patch('harpiya.modules.module_graph.get_manifest', lambda name: manifests.get(name, {})),
            patch('harpiya.modules.module_graph.ModuleGraph._imported_modules', {'studio_customization'}),
        ):
            dummy_cr = None
