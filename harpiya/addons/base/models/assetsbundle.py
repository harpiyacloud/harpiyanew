...
from harpiya.tools import (func, misc, transpile_javascript,
    is_harpiya_module, SourceMapGenerator, profiler, OrderedSet)
from harpiya.tools.json import scriptsafe as json
from harpiya.tools.constants import SCRIPT_EXTENSIONS, STYLE_EXTENSIONS
from harpiya.tools.misc import file_open, file_path
            ...
            if self.templates:
                ...
                template_bundle = textwrap.dedent(f"""
                    ...
                    *******************************************/
                    ...
                    harpiya.define("{self.name}.bundle.xml", ["@web/core/templates"], function(require) {{
                        "use strict";
        ...
        * 't-inherit-mode':  'primary' or 'extension'.
        ...
        for asset in self.templates:
            ...
            try:
                ...
                template = content if content.startswith('<harpiya>') else f'<templates>{asset.content}</templates>'
    ...
    def compile_css(self, compiler, source):
        ...
        def sanitize(matchobj):
            ...
            if '.' not in ref and line not in imports and not ref.startswith(('.', '/', '~')):
                imports.append(line)
                return line
            msg = "Local import '%s' is forbidden for security reasons. Please remove all @import {your_file} imports in your custom files. In Harpiya you have to import all files in the assets, and not through the @import statement." % ref
            _logger.warning(msg)
...
class JavascriptAsset(WebAsset):
    ...
    def is_transpiled(self):
        if self._is_transpiled is None:
            self._is_transpiled = bool(is_harpiya_module(self.url, super().content))
