...
from harpiya.exceptions import LockError, UserError
from harpiya.modules.registry import Registry
from harpiya.tools import SQL
from harpiya.tools.constants import GC_UNLINK_LIMIT
...
BASE_VERSION = harpiya.modules.get_manifest('base')['version']
...
HARPIYA_NOTIFY_FUNCTION = os.getenv('HARPIYA_NOTIFY_FUNCTION', 'pg_notify')
...
class IrCron(models.Model):
    ...
    def create(self, vals_list):
        for vals in vals_list:
            vals['usage'] = 'ir_cron'
        if os.getenv('HARPIYA_NOTIFY_CRON_CHANGES'):
    ...
    def _check_modules_state(cr, jobs):
        ...
        # reset_module_states.
        from harpiya.modules.loading import reset_modules_state  # noqa: PLC0415
    ...
    def write(self, vals):
        ...
        if ('nextcall' in vals or vals.get('active')) and os.getenv('HARPIYA_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
    ...
    def _trigger_list(self, at_list: list[datetime]):
        ...
        if min(at_list) <= now or os.getenv('HARPIYA_NOTIFY_CRON_CHANGES'):
            self._cr.postcommit.add(self._notifydb)
        return triggers
    ...
    def _notifydb(self):
        ...
        The HARPIYA_NOTIFY_CRON_CHANGES environment variable allows to force the notifydb on both
        IrCron modification and on trigger creation (regardless of call_at)
        """
        with harpiya.sql_db.db_connect('postgres').cursor() as cr:
            cr.execute(SQL("SELECT %s('cron_trigger', %s)", SQL.identifier(HARPIYA_NOTIFY_FUNCTION), self.env.cr.dbname))
