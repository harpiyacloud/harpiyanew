...
from harpiya import api, fields, models, tools, _, Command
from harpiya.api import SUPERUSER_ID
from harpiya.exceptions import AccessDenied, AccessError, UserError, ValidationError
from harpiya.http import request, DEFAULT_LANG
from harpiya.osv import expression
from harpiya.tools import is_html_empty, frozendict, lazy_property, SQL
...
class ResUsers(models.Model):
    ...
    def _unlink_except_master_data(self):
        ...
        if SUPERUSER_ID in self.ids:
            raise UserError(_('You can not remove the admin user as it is used internally for resources created by Harpiya (updates, module installation, ...)'))
    ...
    def change_password(self, old_passwd, new_passwd):
        ...
        :return: True
        :raise: harpiya.exceptions.AccessDenied when old password is wrong
        :raise: harpiya.exceptions.UserError when new password is not set or empty
        """
        if not old_passwd:
            raise AccessDenied()
    ...
    def _deactivate_portal_user(self, **post):
        ...
        it removed because they don't use this Harpiya portal anymore.
        ...
        if self._on_login_cooldown(failures, previous):
            ...
            if ipaddress.ip_address(source).is_private:
                _logger.warning(
                    ...
                    "and *might* be a proxy. If your Harpiya is behind a proxy, "
                    "it may be mis-configured. Check that you are running "
                    "Harpiya in Proxy Mode and that the proxy is properly configured, see "
                    "https://www.harpiya.com/documentation/master/administration/install/deploy.html#https for details.",
                    source
                )
