...
from harpiya.sql_db import close_db, db_connect
from harpiya.tests import HOST, BaseCase, Like, get_db_name, tagged
from harpiya.tools import lazy_property, mute_logger, SQL
...
class TestHttpRegistry(BaseCase):
    ...
    def setUpClass(cls):
        ...
        cls.addClassCleanup(lazy_property.reset_all, harpiya.http.root)
        cls.classPatch(harpiya.conf, 'server_wide_modules', ['base', 'web', 'rpc', 'test_http'])

        # make sure there are always many databases, to break monodb
        cls._db_list = cls.startClassPatcher(patch('harpiya.http.db_list'))
        cls._db_list.return_value = ['postgres', get_db_name()]
        cls.startClassPatcher(patch('harpiya.http.db_filter',
    ...
    def authenticate(self, *, db=None):
        session = harpiya.http.root.session_store.new()
        session.update(harpiya.http.get_default_session(), db=db or get_db_name())
    ...
    def url_open(self, path, *, allow_redirects=False):
        ...
        url = urljoin(f"http://{HOST}:{harpiya.tools.config['http_port']}", path)
    ...
    def test_signaling(self):
        ...
        with self.assertLogs('harpiya.registry', logging.INFO) as capture:
            res = self.url_open('/test_http/ensure_db')
            self.assertEqual(res.status_code, 200)
        self.assertEqual(capture.output, [
            "INFO:harpiya.registry:Reloading the model registry after database signaling.",
            Like("INFO:harpiya.registry:Registry loaded in ...s"),
        ])
    ...
    def test_missing_db(self):
        ...
        with self.assertLogs('harpiya.http', logging.WARNING) as capture:
            res = self.url_open('/test_http/ensure_db')
        ...
        self.assertEqual(capture.output, [
            Like("WARNING:harpiya.http:Database or registry unusable, trying without\n"
                 f'Traceback...database "{db_duplicate}" does not exist...')
        ] * 2)

    @mute_logger('harpiya.sql_db')
    def test_corrupt_ir_module_module_table(self):
        ...
        with self.assertLogs('harpiya.registry', logging.ERROR) as capture1, \
             self.assertLogs('harpiya.http', logging.WARNING) as capture2:
            res = self.url_open('/test_http/greeting-public')
            self.assertEqual(res.status_code, 404)
        self.assertEqual(capture1.output, [
            "ERROR:harpiya.registry:Failed to load registry",
        ])
