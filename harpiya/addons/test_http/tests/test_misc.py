...
import harpiya
from harpiya.http import root, content_disposition
from harpiya.tests import tagged
from harpiya.tests.common import HOST, new_test_user, get_db_name, BaseCase
from harpiya.tools import config, file_path, parse_version
from harpiya.addons.test_http.controllers import CT_JSON

from harpiya.addons.test_http.utils import TEST_IP
...
class TestHttpMisc(TestHttpBase):
    ...
    def test_misc1_reverse_proxy(self):
        # client <-> reverse-proxy <-> harpiya
        client_ip = '127.0.0.16'
        reverseproxy_ip = gethostbyname(HOST)
        host = 'mycompany.harpiya.com'
    ...
    def test_misc2_local_redirect(self):
        def local_redirect(path):
            fake_req = harpiya.tools.misc.DotDict(db=False)
            return harpiya.http.Request.redirect(fake_req, path, local=True).headers['Location']
    ...
    def test_misc3_is_static_file(self):
        ...
        self.assertEqual(root.get_static_file(f'harpiya.com/{uri}', host='harpiya.com'), path, "Valid file with valid host")
        self.assertEqual(root.get_static_file(f'http://harpiya.com/{uri}', host='harpiya.com'), path, "Valid file with valid host")
        ...
        self.assertIsNone(root.get_static_file(f'harpiya.com/{uri}'), "No host allowed")
        self.assertIsNone(root.get_static_file(f'http://harpiya.com/{uri}'), "No host allowed")
    ...
    def test_misc5_geoip(self):
        ...
        headers = {
            ...
            'X-Forwarded-Host': 'harpiya.com',
            'X-Forwarded-Proto': 'https'
        }
        with patch.dict(harpiya.tools.config.options, {'proxy_mode': True}):
    ...
    def test_misc6_upload_file_retry(self):
        from harpiya.addons.test_http import controllers  # pylint: disable=C0415
...
class TestHttpEnsureDb(TestHttpBase):
    ...
    def test_ensure_db1_grant_db(self):
        ...
        self.assertEqual(harpiya.http.root.session_store.get(res.cookies['session_id']).db, 'db0')
    ...
    def test_ensure_db3_change_db(self):
        ...
        new_session = harpiya.http.root.session_store.get(res.cookies['session_id'])
    ...
    def test_ensure_db4_unicode(self):
        ...
        self.assertEqual(
            harpiya.http.root.session_store.get(res.cookies['session_id']).db,
            'basededonnée1')
