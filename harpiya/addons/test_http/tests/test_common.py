...
import harpiya
from harpiya.http import Session
from harpiya.addons.base.tests.common import HttpCaseWithUserDemo
from harpiya.tools import config, lazy_property
from harpiya.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        cls.classPatch(harpiya.http.Application, 'session_store', session_store)
        cls.classPatch(harpiya.http.Application, 'geoip_city_db', geoip_resolver)
        cls.classPatch(harpiya.http.Application, 'geoip_country_db', geoip_resolver)

    def setUp(self):
        super().setUp()
        harpiya.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('harpiya.http.db_list') as db_list, \
             patch('harpiya.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('harpiya.http.db_list') as db_list, \
             patch('harpiya.http.db_filter') as db_filter, \
             patch('harpiya.http.Registry') as Registry:
