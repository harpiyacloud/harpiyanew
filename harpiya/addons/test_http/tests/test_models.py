...
import harpiya.http
from harpiya.tests import tagged, get_db_name
from harpiya.tests.common import new_test_user, Like
from harpiya.tools import mute_logger
from harpiya.tools.misc import submap
from harpiya.addons.test_http.utils import HtmlTokenizer
...
class TestHttpModels(TestHttpBase):
    ...
    def test_models3_stargate_ko(self):
        ...
        self.assertEqual(logs.output, ["WARNING:harpiya.http:The goauld destroyed the gate"])
    ...
    def test_models4_stargate_setname(self):
        ...
        res = self.url_open(f'/test_http/{milky_way.id}/setname?readonly=0', {
            'name': "Wilky May",
            'csrf_token': harpiya.http.Request.csrf_token(self),
        })
    ...
    def test_models5_stargate_setname_readonly(self):
        ...
        with self.assertLogs('harpiya.http', 'WARNING') as capture_http,\
             self.assertLogs('harpiya.sql_db', 'WARNING') as capture_sql_db:
