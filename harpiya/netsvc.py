...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "harpiya.netsvc.log is deprecated starting Harpiya 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        if tools.config['log_db'] != '%d':
            with contextlib.suppress(Exception), tools.mute_logger('harpiya.sql_db'), sql_db.db_connect(tools.config['log_db'], allow_uri=True).cursor() as cr:
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('harpiya.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='harpiya.tools.image')
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("HARPIYA_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'harpiya.http.rpc.request:INFO',
    'harpiya.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['harpiya:DEBUG', 'harpiya.sql_db:INFO', 'harpiya.http.rpc:DEBUG'],
    'debug_rpc': ['harpiya:DEBUG', 'harpiya.sql_db:INFO', 'harpiya.http.rpc.request:DEBUG'],
    'debug': ['harpiya:DEBUG', 'harpiya.sql_db:INFO'],
    'debug_sql': ['harpiya.sql_db:DEBUG'],
    'info': [],
    'runbot': ['harpiya:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['harpiya:WARNING', 'werkzeug:WARNING'],
    'error': ['harpiya:ERROR', 'werkzeug:ERROR'],
    'critical': ['harpiya:CRITICAL', 'werkzeug:CRITICAL'],
}
