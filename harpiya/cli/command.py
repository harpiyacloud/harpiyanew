...
import harpiya.cli
from harpiya.modules import initialize_sys_path, load_script
...
def load_internal_commands():
    """ Load ``commands`` from ``harpiya.cli`` """
    for path in harpiya.cli.__path__:
        for module in Path(path).iterdir():
            if module.suffix != '.py':
                continue
            __import__(f'harpiya.cli.{module.stem}')
    ...
    for path in harpiya.addons.__path__:
        for fullpath in Path(path).glob(f'*/cli/{command}.py'):
            if (found_command := fullpath.stem) and Command.is_valid_name(found_command):
                # loading as harpiya.cli and not harpiya.addons.{module}.cli
                # so it doesn't load harpiya.addons.{module}.__init__
                mapping[f'harpiya.cli.{found_command}'] = fullpath 
...
def find_command(name: str) -> Command | None:
    ...
    # import from harpiya.cli
    with contextlib.suppress(ImportError):
        __import__(f'harpiya.cli.{name}')
        return commands[name]

    # import from harpiya.addons.*.cli
