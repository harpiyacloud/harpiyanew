...
class Neutralize(Command):
    ...
    def run(self, args):
        parser = harpiya.tools.config.parser
        ...
        opt = harpiya.tools.config.parse_config(args, setup_logging=True)
        ...
        try:
            with harpiya.sql_db.db_connect(dbname).cursor() as cursor:
                if opt.to_stdout:
                    installed_modules = harpiya.modules.neutralize.get_installed_modules(cursor)
                    queries = harpiya.modules.neutralize.get_neutralization_queries(installed_modules)
                    ...
                    harpiya.modules.neutralize.neutralize_database(cursor)
