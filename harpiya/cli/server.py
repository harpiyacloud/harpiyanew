...
from harpiya.release import author as __author__  # noqa: F401
from harpiya.release import version as __version__  # noqa: F401
from harpiya.service import server
from harpiya.tools import config

from . import Command

# Also use the `harpiya` logger for the main script.
_logger = logging.getLogger('harpiya')

re._MAXCACHE = 4096  # default is 512, a little too small for harpiya
...
def report_configuration():
    ...
    import harpiya.release  # noqa: PLC0415
    _logger.info("Harpiya version %s", harpiya.release.version)
    if os.path.isfile(config['config']):
        _logger.info("Using configuration file at %s", config['config'])
    _logger.info('addons paths: %s', harpiya.addons.__path__)
    ...
    if sys.version_info[:2] > harpiya.release.MAX_PY_VERSION:
        _logger.warning("Python %s is not officially supported, please use Python %s instead",
            '.'.join(map(str, sys.version_info[:2])),
            '.'.join(map(str, harpiya.release.MAX_PY_VERSION))
        )
...
def setup_pid_file():
    ...
    This function assumes the configuration has been initialized.
    """
    import harpiya  # for evented  # noqa: PLC0415
    if not harpiya.evented and config['pidfile']:
        pid = os.getpid()
...
def export_translation():
    ...
    from harpiya.tools.translate import trans_export  # noqa: PLC0415
...
def import_translation():
    from harpiya.modules.registry import Registry  # noqa: PLC0415
    from harpiya.tools.translate import TranslationImporter  # noqa: PLC0415
...
def main(args):
    ...
    for db_name in config['db_name']:
        from harpiya.service import db  # noqa: PLC0415
        try:
...
class Server(Command):
    """Start the harpiya server (default command)"""
