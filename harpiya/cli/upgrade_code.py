...
* addon: the harpiya addon in which the file is;
...
try:
    ...
    from harpiya.modules import initialize_sys_path
    from harpiya.tools import config, parse_version
except ImportError:
    # Assume the script is directy executed (by opposition to be
    # executed via harpiya-bin), happily release/parse_version are
    # standalone so we can hack our way there without importing harpiya
    ...
    class Command:
        ...
        def parser(self):
            return argparse.ArgumentParser(
                ...
                description=__doc__.replace('/harpiya/upgrade_code', str(UPGRADE)),
...
class UpgradeCode(Command):
    """ Rewrite the entire source code using the scripts found at /harpiya/upgrade_code """
    name = 'upgrade_code'
    ...
    def run(self, cmdargs):
        ...
        if initialize_sys_path:
            ...
            initialize_sys_path()
            options.addons_path = harpiya.addons.__path__
        else:
