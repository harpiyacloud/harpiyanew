...
r"""\
Harpiya HTTP layer / WSGI application
...
:class:`~harpiya.http.Controller`: class and its companion the
:func:`~harpiya.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an Harpiya http request. The Harpiya http request is
...
import harpiya.init  # import first for core setup
...
Harpiya URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.harpiya.com/documentation/master/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through Harpiya via py-QWeb form, embed a CSRF
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~harpiya.service.db.list_dbs`:
    ...
    """
    try:
        dbs = harpiya.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, Harpiya will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': harpiya.service.common.dispatch,
        'db': harpiya.service.db.dispatch,
        'object': harpiya.service.model.dispatch,
    }
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~harpiya.tools.file_path`
        :param filter_ext: See :func:`~harpiya.tools.file_path`
    ...
    def get_response(
        ...
        **send_file_kwargs
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`harpiya.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~harpiya.http.Controller` can use the :func:`~harpiya.http.route`:
    ...
    :func:`~harpiya.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(harpiya.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['harpiya', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~harpiya.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['harpiya', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are Harpiya addons.
        """
        # Controllers defined outside of harpiya addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of harpiya addons can be extended in
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~harpiya.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> harpiya_ip = socket.gethostbyname('harpiya.com')
        >>> GeoIP(harpiya_ip).country.iso_code
        'FR'
    """
...
class Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~harpiya.http.Response`.
        :rtype: Response
...
class Request:
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~harpiya.addons.base.models.res_users.ResUsers>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~harpiya.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        try:
            ...
            self.env = harpiya.api.Environment(cr_readonly, self.session.uid, self.session.context)
            try:
            ...
            with contextlib.closing(self.registry.cursor(readonly=readonly_cr)) as cr:
                ...
                try:
                    ...
                    if isinstance(exc, HTTPException) and exc.code is None:
                        raise  # bubble up to harpiya.http.Application.__call__
                    if (
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~harpiya.http.Response.load` method for the compatible
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "Harpiya Session Expired"
...
class Application:
    ...
    def initialize(self):
        ...
        from harpiya.service.server import load_server_wide_modules  # noqa: PLC0415
    ...
    def statics(self):
        ...
        """
        mod2path = {}
        for addons_path in harpiya.addons.__path__:
    ...
    def session_store(self):
        path = harpiya.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if harpiya.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
            ...
            try:
                ...
                if self.get_static_file(httprequest.path):
                    ...
                    try:
                        ...
                        if (httprequest.path.startswith('/harpiya/')
                            or httprequest.path in (
                                '/harpiya', '/web', '/web/login', '/test_http/ensure_db',
                            )):
