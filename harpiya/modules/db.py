...
""" Initialize the database for module management and Harpiya installation. """
...
if typing.TYPE_CHECKING:
    from harpiya.sql_db import BaseCursor, Cursor
...
def is_initialized(cr: Cursor) -> bool:
    ...
    """
    return harpiya.tools.sql.table_exists(cr, 'ir_module_module')
...
def initialize(cr: Cursor) -> None:
    ...
    """
    try:
        f = harpiya.tools.misc.file_path('base/data/base_data.sql')
    except FileNotFoundError:
        m = "File not found: 'base.sql' (provided by module 'base')."
        _logger.critical(m)
        raise OSError(m)

    with harpiya.tools.misc.file_open(f) as base_sql_file:
        cr.execute(base_sql_file.read())  # pylint: disable=sql-injection

    for i in harpiya.modules.get_modules():
        mod_path = harpiya.modules.get_module_path(i)
        if not mod_path:
            continue

        # This will raise an exception if no/unreadable descriptor file.
        info = harpiya.modules.get_manifest(i)
...
def has_trigram(cr: BaseCursor) -> bool:
    ...
    pg_trgm module but any similar function will be picked by Harpiya.
