...
import harpiya.addons
import harpiya.release as release
import harpiya.tools as tools
import harpiya.upgrade
...
class UpgradeHook:
    """Makes the legacy `migrations` package being `harpiya.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^harpiya\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("harpiya.addons.base.maintenance.migrations", "harpiya.upgrade")
...
def initialize_sys_path() -> None:
    """
    Setup the addons path ``harpiya.addons.__path__`` with various defaults
    ...
    for path in (
        ...
        *tools.config['addons_path'],
        ...
        if os.access(path, os.R_OK) and path not in harpiya.addons.__path__:
            harpiya.addons.__path__.append(path)
    ...
    for up in tools.config['upgrade_path'] or [legacy_upgrade_path]:
        if up not in harpiya.upgrade.__path__:
            harpiya.upgrade.__path__.append(up)

    # create decrecated module alias from harpiya.addons.base.maintenance.migrations to harpiya.upgrade
    spec = importlib.machinery.ModuleSpec("harpiya.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = harpiya.upgrade  # type: ignore
    sys.modules["harpiya.addons.base.maintenance"] = maintenance_pkg
    sys.modules["harpiya.addons.base.maintenance.migrations"] = harpiya.upgrade

    # hook for upgrades and namespace freeze
    if not getattr(initialize_sys_path, 'called', False):  # only initialize once
        harpiya.addons.__path__._path_finder = lambda *a: None  # prevent path invalidation
        harpiya.upgrade.__path__._path_finder = lambda *a: None  # prevent path invalidation
...
def get_module_path(module: str, downloaded: bool = False, display_warning: bool = True) -> str | None:
    ...
    if re.search(r"[\/\\]", module):
        return None
    for adp in harpiya.addons.__path__:
...
def get_resource_from_path(path: str) -> tuple[str, str, str] | None:
    ...
    """
    resource = None
    sorted_paths = sorted(harpiya.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name: str) -> None:
    ...
    qualname = f'harpiya.addons.{module_name}'
...
def get_modules() -> list[str]:
    ...
    plist: list[str] = []
    for ad in harpiya.addons.__path__:
