...
import harpiya.tools.sql
import harpiya.tools.translate
from harpiya import api, tools
...
def load_module_graph(
    ...
    report: HarpiyaTestResult | None = None,
    ...
    assert isinstance(env.cr, harpiya.sql_db.Cursor), "Need for a real Cursor to load modules"
    ...
    t0 = time.time()
    loading_extra_query_count = harpiya.sql_db.sql_counter
    ...
    for index, package in enumerate(graph, 1):
        ...
        module_extra_query_count = harpiya.sql_db.sql_counter
        ...
        if new_install:
            py_module = sys.modules['harpiya.addons.%s' % (module_name,)]
        ...
        if tools.config['test_enable'] and (needs_update or not update_from_config):
            ...
            if suite.countTestCases():
                ...
                # Python tests
                tests_t0, tests_q0 = time.time(), harpiya.sql_db.sql_counter
                ...
                test_queries = harpiya.sql_db.sql_counter - tests_q0

                # tests may have reset the environment
                module = env['ir.module.module'].browse(module_id)


        extra_queries = harpiya.sql_db.sql_counter - module_extra_query_count - test_queries
        extras = []
        if test_queries:
    ...
    _logger.runbot("%s modules loaded in %.2fs, %s queries (+%s extra)",
                   ...
                   harpiya.sql_db.sql_counter - loading_extra_query_count)  # extra queries: testes, notify, any other closed cursor
        ...
        if update_module:
            ...
            if modules_to_remove:
                ...
                for pkg in pkgs:
                    ...
                    if uninstall_hook:
                        py_module = sys.modules['harpiya.addons.%s' % (pkg.name,)]
