...
from harpiya.tools import float_compare, float_is_zero, float_repr, float_round
from harpiya.tools.misc import SENTINEL, Sentinel
...
class Float(Field[float]):
    ...
    :param digits: a pair (total, decimal) or a string referencing a
        :class:`~harpiya.addons.base.models.decimal_precision.DecimalPrecision` record name.
    ...
    :func:`~harpiya.fields.Float.round()` to round a float with the given precision.
    :func:`~harpiya.fields.Float.is_zero()` to check if a float equals zero at the given precision.
    :func:`~harpiya.fields.Float.compare()` to compare two floats at the given precision.
...
class Monetary(Field[float]):
    ...
    :class:`res_currency<harpiya.addons.base.models.res_currency.Currency>`.
    ...
    :param str currency_field: name of the :class:`Many2one` field
        holding the :class:`res_currency <harpiya.addons.base.models.res_currency.Currency>`
    ...
    def convert_to_cache(self, value, record, validate=True):
        ...
        if value and validate:
            # FIXME @rco-harpiya: currency may not be already initialized if it is
