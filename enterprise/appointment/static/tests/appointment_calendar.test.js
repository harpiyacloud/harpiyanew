...
test("create/search anytime appointment type", async () => {
    ...
    patchWithCleanup(navigator, {
        clipboard: {
            writeText: (value) => {
                expect(value).toBe(
                    `http://amazing.harpiya.com/appointment/3?filter_staff_user_ids=%5B${100}%5D`
                );
            },
        },
    });
    ...
    await mountView({
        ...
        session: {
            "web.base.url": "http://amazing.harpiya.com",
        },
    });
    await contains(".dropdownAppointmentLink").click();
...
test("click & copy appointment type url", async () => {
    ...
    patchWithCleanup(navigator, {
        clipboard: {
            writeText: (value) => {
                expect(value).toBe(
                    `http://amazing.harpiya.com/appointment/2?filter_staff_user_ids=%5B${100}%5D`
                );
            },
        },
    });
