...
from harpiya.addons.sale_amazon import utils as amazon_utils
...
class TestAmazon(common.TestAmazonCommon):
    ...
    def test_update_marketplaces_no_change(self):
        ...
        with patch(
            'harpiya.addons.sale_amazon.models.amazon_account.AmazonAccount'
    ...
    def test_sync_orders_cancel_abort(self):
        """ Test the pickings that were confirmed at harpiya and then order is canceled at Amazon. """
    ...
    def test_inventory_sync_is_skipped_when_disabled(self):
        ...
        with patch(
            'harpiya.addons.sale_amazon.utils.submit_feed', return_value='An_amazing_id'
        ) as mock:
    ...
    @mute_logger('harpiya.addons.sale_amazon.models.amazon_account')
    @mute_logger('harpiya.addons.sale_amazon.models.amazon_offer')
    ...
    def test_sync_pickings(self):
        ...
        with patch(
            ...
            'harpiya.addons.sale_amazon.utils.submit_feed', new=Mock(return_value='An_amazing_id'),
        ) as mock:
            self.account._sync_orders(auto_commit=False)
            order = self.env['sale.order'].search([('amazon_order_ref', '=', '123456789')])
            picking = self.env['stock.picking'].search([('sale_id', '=', order.id)])
