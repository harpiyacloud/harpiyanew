...
from harpiya import api, fields, models, Command, _
from harpiya.addons.l10n_mx_edi.models.l10n_mx_edi_document import (
...
from harpiya.addons.base.models.ir_qweb import keep_query
...
class AccountMove(models.Model):
        ...
        for invoice_values in pay_results['invoice_results']:
            ...
            for key in (
                ...
                for tax_values in inv_cfdi_values[key]:
                    ...
                    # Handle the case where the rounding method was changed between Harpiya versions.
    ...
    def _l10n_mx_edi_cfdi_invoice_post_cancel(self):
        ...
        - The user can request a cancellation from Harpiya.
        - The user can cancel the invoice from the SAT, then update the SAT state in Harpiya.
        """
    ...
    def _l10n_mx_edi_cfdi_payment_post_cancel(self):
        ...
        - The user can request a cancellation from Harpiya.
        - The user can cancel the payment from the SAT, then update the SAT state in Harpiya.
        """
        self._l10n_mx_edi_cfdi_move_post_cancel()
    ...
    def _l10n_mx_edi_import_cfdi_fill_invoice_line(self, tree, line):
        # Product
        code = tree.attrib.get('NoIdentificacion')  # default_code if export from Harpiya
