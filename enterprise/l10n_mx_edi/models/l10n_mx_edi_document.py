...
from harpiya.tools.zeep import Client, Transport
from pytz import timezone

from harpiya import _, api, models, modules, fields, tools
from harpiya.osv import expression
from harpiya.tools import frozendict
from harpiya.tools.float_utils import float_is_zero, float_round
...
class L10n_Mx_EdiDocument(models.Model):
    ...
    _description = "Mexican documents that needs to transit outside of Harpiya"
...
""" % {'boundary': boundary, 'cfdi_b64': cfdi_b64}
    ...
    def _fetch_and_update_sat_status(self, batch_size=100, extra_domain=None):
        ...
        In the second case, the cancellation could be done Harpiya-side and then we need to check if the SAT is up-to-date,
        or could be done manually government-side forcing Harpiya to update the invoice's state.
