...
from harpiya import models, api, fields, _
from harpiya.addons.account_avatax.lib.avatax_client import AvataxClient
...
class AccountExternalTaxMixin(models.AbstractModel):
    ...
    def _uncommit_external_taxes(self):
        for record in self.filtered('is_avatax'):
            ...
            error = self._handle_response(query_result, _(
                'Harpiya could not change the state of the transaction related to %(document)s in'
                ' AvaTax\nPlease check the status of `%(technical)s` in the AvaTax portal.',
    ...
    def _void_external_taxes(self):
        for record in self.filtered('is_avatax'):
            ...
            error = self._handle_response(query_result, _(
                'Harpiya could not void the transaction related to %(document)s in AvaTax\nPlease '
