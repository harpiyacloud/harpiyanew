...
from harpiya.exceptions import UserError, ValidationError, RedirectWarning
from harpiya.tests.common import tagged
from harpiya.modules.neutralize import get_neutralization_queries
...
class TestAccountAvalaraInternal(TestAccountAvalaraInternalCommon):
    def test_01_harpiya_invoice(self):
        ...
        with patch('harpiya.addons.account_avatax.models.account_external_tax_mixin.AccountExternalTaxMixin._uncommit_external_taxes') as mocked_uncommit:
            invoice.button_draft()
            mocked_uncommit.assert_called()

    def test_02_harpiya_invoice(self):
    ...
    def test_03_harpiya_invoice(self):
    ...
    def test_01_harpiya_refund(self):
        invoice, response = self._create_invoice_01_and_expected_response()
    ...
    def test_02_harpiya_refund(self):
    ...
    def test_invoice_multi_company(self):
        ...
        with self._capture_request(return_value=response):
            ...
            # harpiya.exceptions.ValidationError
    ...
    def test_invoice_multi_taxline(self):
        ...
        with self._capture_request(return_value=response):
            # ensure this doesn't raise:
            # harpiya.exceptions.ValidationError: Expected singleton:
            invoice.button_external_tax_calculation()
...
class TestAccountAvalaraInternalIntegration(TestAccountAvalaraInternalCommon):
    def test_integration_01_harpiya_invoice(self):
        with self._skip_no_credentials():
            invoice, _ = self._create_invoice_01_and_expected_response()
            self.assertInvoice(invoice, test_exact_response=False)
            invoice.button_draft()

    def test_integration_02_harpiya_invoice(self):
        with self._skip_no_credentials():
...
class TestAccountAvalaraSalesTaxAdministration(TestAccountAvataxCommon):
    ...
    def test_disable_avatax(self):
        ...
        with patch('harpiya.addons.account_avatax.lib.avatax_client.AvataxClient.request') as mocked_request:
