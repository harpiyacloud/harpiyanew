...
import { click, queryAll, queryFirst } from "@harpiya/hoot-dom";
import { animationFrame, mockMatchMedia } from "@harpiya/hoot-mock";
...
test("initial action crashes", async () => {
    expect.errors(1);
    redirect("/harpiya/action-__test__client__action__?menu_id=1");
...
test("Navigate to an application from the HomeMenu should generate only one pushState", async () => {
    ...
    await animationFrame();
    expect(".o_home_menu").toHaveCount(1);
    expect.verifySteps(["/harpiya", "/harpiya/action-1002", "/harpiya", "/harpiya/action-1001", "/harpiya"]);
});
