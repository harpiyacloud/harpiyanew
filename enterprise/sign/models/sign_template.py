...
from harpiya.tools import config, misc, pdf, format_date
from harpiya.tools.pdf import PdfFileReader, PdfFileWriter, PdfReadError, reshape_text
        ...
        for it in self.sign_item_ids:
            ...
            if it.type_id == name_item_type:
                ...
                if it.type_id != sign_item_type_date:
                    value = _("Sample generated by Harpiya for %s.", role_name)
                else:
                    value = format_date(self.env, fields.Date.today())
            elif it.type_id.item_type == "textarea":
                value = _("""Harpiya is a suite of open source business apps
