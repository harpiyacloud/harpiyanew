...
import { Component, onWillStart, useState } from "@harpiya/owl";
...
export class ThankYouDialog extends Component {
    ...
    generateButtons() {
        ...
        if (this.suggestSignUp) {
            this.state.signUpButton = {
                ...
                click: () => {
                    window.open(
                        "https://www.harpiya.com/trial?selected_app=sign&utm_source=db&utm_medium=sign",
                        "_blank"
                    ); 
                },
            };
    ...
    onClickClose() {
        if (this.suggestSignUp) {
            window.open(`https://harpiya.com/app/sign`, "_self");
            return;
        } 
        if (session.is_frontend) {
