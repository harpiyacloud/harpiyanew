import { Component, markup, xml } from "@harpiya/owl";
...
const articleCommandSteps = [
    { // open the command bar
        trigger: `[name="body"] .harpiya-editor-editable > p:last-child`,
        run: function () {
...
const fileCommandSteps = [{ // open the command bar
    trigger: '.harpiya-editor-editable > p',
    run: function () {
        openPowerbox(this.anchor);
    },
...
const tocCommandSteps = [{ // open the command bar
    ...
    trigger: '.o_field_html .harpiya-editor-editable',
}];
...
const embedKanbanEditArticleSteps = [{ // Create a new article using quick create in OnGoing Column
    ...
    trigger: '.harpiya-editor-editable:contains("EditorCommandsArticle Content")',
}];
...
const articleCommandComposerSteps = [{ // open the chatter
    ...
    trigger: `${composeBody} .harpiya-editor-editable > div.o-paragraph`,
    ...
    trigger: `${composeBody} .harpiya-editor-editable a:nth-child(2).o_knowledge_article_link:contains("LinkedArticle")[contenteditable="false"]`,
}, { // verify that the first block is still there and contenteditable=false
    trigger: `${composeBody} .harpiya-editor-editable a:nth-child(1).o_knowledge_article_link:contains("EditorCommandsArticle")[contenteditable="false"]`,
}, { // send the message
    trigger: '.o_mail_send',
    run: "click",
}, {
...
const clipboardUsageSteps = [{ // open the chatter
    ...
    trigger: '.o_form_sheet .o_field_html .harpiya-editor-editable p:contains("Hello world")',
}];
