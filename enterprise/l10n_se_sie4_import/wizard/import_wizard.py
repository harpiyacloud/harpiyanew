...
from harpiya import _, api, Command, fields, models, modules, tools
from harpiya.tools import DEFAULT_SERVER_DATE_FORMAT
...
class SIE4ImportWizard(models.TransientModel):
    ...
    import_opening_balance = fields.Boolean(
        ...
        help="Compare opening balance information from Harpiya with the file and create a move with the difference found.",
    )
    ...
    def _import_key_ub(self, data_map, year_idx, account_code, balance):
        ...
        In Harpiya, we only save the opening balance move, so we don't really need this item.
    ...
    def _import_special_key_ver(self, data_map, verification_str, verification_date, verification_text, transactions):
        ...
        In Harpiya, they are specified as ``account.move``, where each transaction is an ``account.move.line``
    ...
    def _prepare_sie4_opening_balance_move(self, data_map):
        ...
        It then compares the difference of the opening balance found in Harpiya with the file.
