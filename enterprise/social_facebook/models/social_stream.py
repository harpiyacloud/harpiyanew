...
class SocialStream(models.Model):
    ...
    def _format_facebook_message(self, message, tags):
        ...
        example:
            "Hello Harpiya :)" -> "Hello @[542132] Harpiya :)"
            "Hello Harpiya Social :)" -> "Hello @[542132] Harpiya-Social :)"
            "Hello Harpiya - Social :)" -> "Hello @[542132] Harpiya-Social :)"

        details:
            "Hello @[42] faketag Harpiya - Social :)" -> "Hello @ [42] faketag @[542132] Harpiya-Social :)""
            -> With associated tag: {name: "Harpiya - Social", offset: 20, length: 13}

            We take the message until the tag offset: "Hello @[42] faketag "
            and remove the fake tag in the process "Hello @ [42] faketag "
            Then we insert the forged tag "@[542132] Harpiya-Social"
