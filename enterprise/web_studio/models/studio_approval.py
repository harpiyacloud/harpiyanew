...
class StudioApprovalRule(models.Model):
    ...
    def set_approval(self, res_id, approved):
        ...
        :rtype: boolean
        :raise: harpiya.exceptions.AccessError when the user does not have write
                access to the underlying record
        :raise: harpiya.exceptions.UserError when any of the other checks failed
        """
        self.ensure_one()
    ...
    def delete_approval(self, res_id):
        ...
        :raise: harpiya.exceptions.AccessError when the user does not have write
                access to the underlying record
        :raise: harpiya.exceptions.UserError when any there is no existing entry
    ...
    def _set_approval(self, res_id, approved):
        ...
        :rtype: :class:`~harpiya.addons.web_studio.models.StudioApprovalEntry`
        :raise: harpiya.exceptions.AccessError when the user does not have write
                access to the underlying record
        :raise: harpiya.exceptions.UserError when any of the other checks failed
        """
        ...
        # Harpiya itself (not PG); the NOWAIT ensures that no deadlock will happen
