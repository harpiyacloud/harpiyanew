...
from harpiya.addons.spreadsheet_edition.tests.spreadsheet_test_case import SpreadsheetTestCase
...
class SpreadsheetMixinControllerTest(SpreadsheetTestCase, HttpCase):
    ...
    def test_default_company_custom_colors(self):
        ...
        response = self.url_open(f'/spreadsheet/data/spreadsheet.test/{spreadsheet.id}')
        data = response.json()
        self.assertEqual(data['company_colors'], ['#FFFFFF', '#003563'])
    ...
    def test_two_companies_custom_colors(self):
        ...
        response = self.url_open(f'/spreadsheet/data/spreadsheet.test/{spreadsheet.id}')
        data = response.json()
        self.assertEqual(data['company_colors'], ['#aa0000', '#aa1111', '#FFFFFF', '#003563', '#bb0000', '#bb1111'])
