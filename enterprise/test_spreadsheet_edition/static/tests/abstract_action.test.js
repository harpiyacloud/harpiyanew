...
import { animationFrame, rightClick, waitFor, runAllTimers } from "@harpiya/hoot-dom";
    ...
    onRpc(
        ...
        () => {
            return {
                data: {},
                name: "test",
                company_colors: ["#003563", "not a valid color"],
            };
        },
        { pure: true }
    );
    const { model } = await createSpreadsheetTestAction("spreadsheet_test_action");
    expect(model.getters.getCustomColors()).toEqual(["#003563"]);
});
