...
class Website_GeneratorRequest(models.Model):
    ...
    def _create_model_records(self, tar, harpiya_blocks):
        super()._create_model_records(tar, harpiya_blocks)
        ecommerce_redirects = {}
        category_pages_redirects, created_categories_mapping = self._create_public_category_pages(tar, harpiya_blocks)
        ecommerce_redirects.update(category_pages_redirects)
        product_redirects = self._create_products(tar, harpiya_blocks, created_categories_mapping)
        ecommerce_redirects.update(product_redirects)
        harpiya_blocks['direct_html_replacements_mapping'].update(ecommerce_redirects)

    def _create_products(self, tar, harpiya_blocks, created_categories_mapping):
        # First extract all the product.template vals
        products = harpiya_blocks.get('products', {})
        all_product_vals = []
        ...
        all_product_images = harpiya_blocks['website'].get('all_product_images', {})
        all_image_vals = []
        product_redirects = {}
    ...
    def _create_public_category_pages(self, tar, harpiya_blocks):
        categories = harpiya_blocks.get('categories', {})
        all_product_images = harpiya_blocks['website'].get('all_product_images', {})
