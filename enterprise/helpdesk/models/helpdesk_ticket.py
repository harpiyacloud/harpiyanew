...
from harpiya import api, Command, fields, models, tools, _
...
class HelpdeskTicket(models.Model):
    ...
    def _compute_open_hours(self):
        for ticket in self:
            if ticket.create_date:  # fix from https://github.com/harpiya/enterprise/commit/928fbd1a16e9837190e9c172fa50828fae2a44f7
    ...
    def write(self, vals):
        ...
        if stage_fold := ('stage_id' in vals and self.env['helpdesk.stage'].browse(vals['stage_id']).fold):
            harpiyabot_partner_id = self.env['ir.model.data']._xmlid_to_res_id('base.partner_root')
            for ticket in self:
                ...
                if exceeded_hours:
                    ...
                    ticket.message_post(body=message, subtype_xmlid="mail.mt_note", author_id=harpiyabot_partner_id)
