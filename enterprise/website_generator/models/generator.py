...
from harpiya.addons.iap.tools.iap_tools import iap_jsonrpc
from harpiya.exceptions import AccessError
from harpiya.tools import LazyTranslate
from urllib.parse import urljoin, urlparse

_lt = LazyTranslate(__name__)

DEFAULT_WSS_ENDPOINT = 'https://iap-scraper.harpiya.com/'
...
class Website_GeneratorRequest(models.Model):
    ...
    def create(self, vals_list):
        ...
        for req in wg_requests:
            ...
            # created via the harpiya.com start trial.
            if not req.uuid:
    ...
    def _generate_site(self, tar):
        harpiya_blocks = self._load_input(tar)
        harpiya_blocks['direct_html_replacements_mapping'] = {}
        harpiya_blocks['regex_html_replacements_mapping'] = {}
        website = self._get_website(harpiya_blocks, tar)
        # Generate the images attachments (Modifies harpiya_blocks in place)
        self._save_images_as_attachments(harpiya_blocks, tar)
        self._create_model_records(tar, harpiya_blocks)
        # Create redirects, modifies harpiya_blocks in place
        self._apply_all_html_replacements(harpiya_blocks)
        self._generate_pages(website, harpiya_blocks)
        return website, harpiya_blocks

    def _apply_all_html_replacements(self, harpiya_blocks):
        direct_html_replacements_mapping = harpiya_blocks.get('direct_html_replacements_mapping', {})
        regex_html_replacements_mapping = harpiya_blocks.get('regex_html_replacements_mapping', {})
        sorted_original_html = sorted(direct_html_replacements_mapping.keys(), key=len, reverse=True)

        homepage = harpiya_blocks['homepage']
        ...
        for page_name, page_dict in harpiya_blocks.get('pages', {}).items():
            harpiya_blocks['pages'][page_name]['body_html'] = self._apply_html_replacements(page_dict.get('body_html', []), sorted_original_html, direct_html_replacements_mapping, regex_html_replacements_mapping)

    def _create_model_records(self, tar, harpiya_blocks):
    ...
    def _generate_pages(self, website, harpiya_blocks):
        # Create pages
        for page_url, page_data in harpiya_blocks.get('pages', {}).items():
        ...
        homepage._construct_homepage(harpiya_blocks['homepage'])

    def _get_website(self, harpiya_blocks, tar):
        website = self.env['website'].get_current_website()
        self.write({'website_id': website.id})
        website_info = harpiya_blocks.get('website')
        if not website_info:
            raise ValueError("Website info not found in the input")
        homepage_url = harpiya_blocks.get('homepage', {}).get('url')
        if not homepage_url:
    ...
    def _save_images_as_attachments(self, harpiya_blocks, tar):
        ...
        all_images = harpiya_blocks['website'].get('all_images', {})
        ...
        harpiya_blocks['direct_html_replacements_mapping'].update({html.escape(k): v.image_src for k, v in attachments_url_src.items()})

        # Create attachments for all images (cropped)
        customized_attachments_url_src = {}
        customized_image_mappings = {}
        for page_dict in [harpiya_blocks['homepage']] + list(harpiya_blocks.get('pages', {}).values()):
        ...
        harpiya_blocks['regex_html_replacements_mapping'].update(customized_image_mappings)
    ...
    def convert_scraping_request_ICP(self):
        ...
        return {
            'type': 'ir.actions.act_url',
            'url': "/harpiya/action-website_generator.website_generator_screen?reload=true",
            'target': 'self',
        }
