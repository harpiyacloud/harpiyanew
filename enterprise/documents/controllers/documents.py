...
from harpiya import fields, http, _
from harpiya.exceptions import MissingError
from harpiya.http import request, content_disposition
from harpiya.osv import expression
from harpiya.tools import config, replace_exceptions, str2bool, consteq

from harpiya.addons.mail.controllers.attachment import AttachmentController
...
class ShareRoute(http.Controller):
    ...
    def _make_zip(self, name, documents):
        ...
        :param harpiya.models.Model documents: documents to load in the ZIP
    ...
    def documents_home(self, access_token):
        ...
        if request.env.user._is_public():
            ...
            # Internal users use the /harpiya/documents/<access_token> route
            return request.redirect(
                f'/harpiya/documents/{quote(access_token, safe="")}',
    ...
    def _documents_render_public_view(self, document_sudo):
        ...
        if (
            ...
            return request.redirect(f'/harpiya/documents/{quote(target_sudo.access_token, safe="")}')
    ...
    def documents_redirect(self, access_token):
        return request.redirect(f'/harpiya/documents/{quote(access_token, safe="")}', HTTPStatus.MOVED_PERMANENTLY)
    ...
    def documents_download_all_legacy(self, access_token=None, share_id=None):
        logger.warning("Deprecated since Harpiya 18. Please access /documents/content/<access_token> instead.")
    ...
    def share_portal(self, share_id=None, token=None):
        logger.warning("Deprecated since Harpiya 18. Please access /harpiya/documents/<access_token> instead.")
        return request.redirect(f'/harpiya/documents/{quote(token or "", safe="")}', code=HTTPStatus.MOVED_PERMANENTLY)
