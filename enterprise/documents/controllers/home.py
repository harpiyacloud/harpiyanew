...
from harpiya.addons.web.controllers import home as web_home
from harpiya.addons.web.controllers.utils import ensure_db
...
class Home(web_home.Home):
    def _web_client_readonly(self):
        ...
        if (
            path.startswith('/harpiya/documents')
            and (request.httprequest.args.get('access_token') or path.removeprefix('/harpiya/documents/'))
    ...
    def web_client(self, s_action=None, **kw):
        """ Handle direct access to a document with a backend URL (/harpiya/documents/<access_token>).
        ...
        if not document_sudo:
            ...
            if document_sudo := Redirect._get_redirection(access_token):
                return request.redirect(
                    f'/harpiya/documents/{quote(document_sudo.access_token, safe="")}',
        ...
        # /harpiya/documents/<id> to /harpiya/documents/<access-token> in the
        # URL.
