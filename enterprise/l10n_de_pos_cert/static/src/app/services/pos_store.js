...
patch(PosStore.prototype, {
            ...
            const message = {
                ...
                unknown: _t(
                    "An unknown error has occurred! Try to validate this order or cancel it again. " +
                        "Please contact Harpiya for more information."
                ),
            };
     ...
     * send it to Harpiya. Two cases can happen:
     * - Failure to send to Fiskaly => we assume that if one order fails, EVERY order will fail
     * - Failure to send to Harpiya => the order is already sent to Fiskaly, we store them locally with the TSS info
     */
    async syncAllOrders(options = {}) {
        ...
        if (result && fiskalyFailure.length === 0) {
            ...
            throw harpiyaError || fiskalyError;
        }
    },
