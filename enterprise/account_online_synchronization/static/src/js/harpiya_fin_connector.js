...
function HarpiyaFinConnector(parent, action) {
    ...
    const runbotRegexp = /^https:\/\/[a-z0-9-_]+\.[a-z0-9-_]+\.harpiya\.com$/;
    if (!modeRegexp.test(action.params.proxyMode) && !runbotRegexp.test(action.params.proxyMode)) {
        return;
    }
    let url = 'https://' + action.params.proxyMode + '.harpiyafin.com/proxy/v1/harpiyafin_link';
    if (runbotRegexp.test(action.params.proxyMode)) {
        url = action.params.proxyMode + '/proxy/v1/harpiyafin_link';
    }
    ...
    loadJS(url)
        .then(function () {
            ...
            HarpiyaFin.create(params);
            HarpiyaFin.open();
        });
    return;
}

actionRegistry.add('harpiya_fin_connector', HarpiyaFinConnector);

export default HarpiyaFinConnector;
