...
from harpiya import api, fields, models, modules, tools
from harpiya.exceptions import UserError, CacheMiss, MissingError, ValidationError, RedirectWarning
from harpiya.http import request
from harpiya.addons.account_online_synchronization.models.harpiyafin_auth import HarpiyaFinAuth
from harpiya.tools.misc import format_amount, format_date, get_lang
from harpiya.tools import _, LazyTranslate

_lt = LazyTranslate(__name__)
_logger = logging.getLogger(__name__)
pattern = re.compile("^[a-z0-9-_]+$")
runbot_pattern = re.compile(r"^https:\/\/[a-z0-9-_]+\.[a-z0-9-_]+\.harpiya\.com$")

class HarpiyaFinRedirectException(UserError):
...
class AccountOnlineAccount(models.Model):
    ...
    fetching_status = fields.Selection(
        selection=[
            ...
            ('processing', 'Processing'),  # When currently importing in harpiya
            ('done', 'Done'),  # When every transaction have been imported in harpiya
        ]
    )
    ...
    def _assign_journal(self, swift_code=False, journal_type='bank'):
        ...
        Also, Create and assign bank & swift/bic code if harpiyafin returns one
    ...
    def _refresh(self):
        """
            ...
            - Provider is in the process of importing the transactions so we should wait until he has
                finished before fetching them in Harpiya
        ...
        while True:
            ...
            resp_json = self.account_online_link_id._fetch_harpiya_fin('/proxy/v1/refresh', data=data)
    ...
    def _retrieve_transactions(self, date=None, transactions_type='posted'):
        ...
        while True:
            ...
            resp_json = self.account_online_link_id._fetch_harpiya_fin(f'/proxy/v2/transactions/{transactions_type}', data=data)
    ...
    def _get_filtered_transactions(self, new_transactions):
        ...
        # Remove transactions already imported in Harpiya
        for transaction in new_transactions:
...
class AccountOnlineLink(models.Model):
    ...
    has_unlinked_accounts = fields.Boolean(default=True, help="True if that connection still has accounts that are not linked to an Harpiya journal")

    # Information received from HarpiyaFin, should not be tampered with
    ...
    def _handle_harpiyafin_redirect_exception(self, mode='link'):
        if mode == 'link':
    ...
    @api.model
    def _get_harpiyafin_url(self, url):
        ...
        endpoint_url = 'https://%s.harpiyafin.com%s' % (proxy_mode, url)
        if runbot_pattern.match(proxy_mode):
            endpoint_url = '%s%s' % (proxy_mode, url)
        return endpoint_url

    def _fetch_harpiya_fin(self, url, data=None, ignore_status=False):
        """
        Method used to fetch data from the Harpiya Fin proxy.
        ...
        endpoint_url = self._get_harpiyafin_url(url)
        cron = self.env.context.get('cron', False)
        data['utils'] = {
            'request_timeout': timeout,
            'lang': get_lang(self.env).code,
            'server_version': harpiya.release.serie,
            'db_uuid': self.env['ir.config_parameter'].sudo().get_param('database.uuid'),
            'cron': cron,
        }
        if request:
            # many banking institutions require the end-user IP/user_agent for traceability
            # of client-initiated actions. It won't be stored on harpiyafin side.
            data['utils']['psu_info'] = {
        ...
        try:
            ...
            resp = requests.post(url=endpoint_url, json=data, timeout=timeout, auth=HarpiyaFinAuth(record=self.sudo()))
            resp_json = resp.json()
    ...
    def _handle_response(self, resp_json, url, data, ignore_status=False):
        ...
        if not resp_json.get('error'):
            ...
            state = result.get('harpiyafin_state') or False
            ...
            if error.get('code') == 101:  # access token expired, not an error
                ...
                return self._fetch_harpiya_fin(url, data, ignore_status)
                ...
                raise HarpiyaFinRedirectException(mode=error.get('data', {}).get('mode', 'link'))
            ...
            state = error_details.get('harpiyafin_state') or 'error'
        ...
        try:
            ...
            if reset_tx:
                ...
                if subject and message:
                    ...
                    harpiya_help_description = f'''ClientID: {self.client_id}\nInstitution: {self.name}\nError Reference: {error_reference}\nError Message: {message_post}\n'''
                    harpiya_help_summary = f'Bank sync error ref: {error_reference} - Provider: {provider} - Client ID: {self.client_id}'
                    if context.get('redirect_warning_url'):
                        if context['redirect_warning_url'] == 'harpiya_support':
                            url_params = urllib.parse.urlencode({'stage': 'bank_sync', 'summary': harpiya_help_summary, 'description': harpiya_help_description[:1500]})
                            url = f'https://www.harpiya.com/help?{url_params}'
                            message += _("\n\nIf you've already opened a ticket for this issue, don't report it again: a support agent will contact you shortly.")
                            message_post = Markup('%s<br>%s <a href="%s" >%s</a>') % (message, _("You can contact Harpiya support"), url, _("Here"))
                            button_label = _('Report issue')
                        else:
                            url = "https://www.harpiya.com/documentation/18.0/applications/finance/accounting/bank/bank_synchronization.html#faq"
                ...
                self.env.cr.commit()
                # and then raise either a redirectWarning error so that customer can easily open an issue with Harpiya,
    ...
    def _get_access_token(self):
        for link in self:
            resp_json = link._fetch_harpiya_fin('/proxy/v1/get_access_token', ignore_status=True)
    ...
    def _get_refresh_token(self):
        ...
        for link in self.sudo():
            resp_json = link._fetch_harpiya_fin('/proxy/v1/renew_token', ignore_status=True)
    ...
    def unlink(self):
        ...
        for link in self:
            try:
                resp_json = link.with_context(delete_sync=True)._fetch_harpiya_fin('/proxy/v1/delete_user', data={'provider_data': link.provider_data}, ignore_status=True)  # delete proxy user
                if resp_json.get('delete', True) is True:
                    to_unlink += link
            except (HarpiyaFinRedirectException, UserError, RedirectWarning):
    ...
    def _fetch_accounts(self, online_identifier=False):
        ...
        while True:
            ...
            resp_json = self._fetch_harpiya_fin('/proxy/v1/accounts', data)
        ...
        try:
            ...
            for online_account in accounts_to_synchronize:
                ...
                try:
                    ...
                    raise
                except HarpiyaFinRedirectException as redirect_exception:
                    self._notify_connection_update(
                        journal=journal,
                        connection_state_details={
                            'status': 'error',
                            'error_type': 'harpiyafin_redirect',
                            'action': self._handle_harpiyafin_redirect_exception(mode=redirect_exception.mode),
                        },
                    )
                    raise
            ...
            return
        except HarpiyaFinRedirectException as e:
            return self._handle_harpiyafin_redirect_exception(mode=e.mode)
    ...
    def _update_connection_status(self):
        self.ensure_one()
        resp_json = self._fetch_harpiya_fin('/proxy/v2/connection_status', ignore_status=True)

        self._update_expiring_date(resp_json)

        # Returning what we receive from Harpiya Fin to allow function extension
        return resp_json
    ...
    def _authorize_access(self, data_access_token):
        ...
        try:
            resp_json = self._fetch_harpiya_fin('/proxy/v1/authorize_access', data)
    ...
    def success(self, mode, data):
        ...
        try:
            ...
            message = _("This version of Harpiya appears to be outdated and does not support the '%s' sync mode. "
    ...
    def exchange_token(self, exchange_token):
        ...
        resp_json = self._fetch_harpiya_fin('/proxy/v1/exchange_token', data=data, ignore_status=True)
    ...
    def _open_iframe(self, mode='link', include_param=None, preferred_institution=False, journal_id=False, journal_type='bank'):
        ...
        if self.client_id and self.sudo().refresh_token:
            try:
                self._get_access_token()
            except HarpiyaFinRedirectException:
        ...
        action = {
            ...
            'params': {
                ...
                'includeParam': {
                    ...
                    'serverVersion': harpiya.release.serie,
