...
class HarpiyaFinAuth(requests.auth.AuthBase):
    """ This is used to sign the request going towards HarpiyaFin
        e.g.:
            requests.get(HARPIYAFIN + '/example', auth=HarpiyaFinAuth())
            By using `auth=HarpiyaFinAuth(self)` when doing a http request, the request will be signed and
    ...
    def __call__(self, request):
        ...
        request.headers.update({
            'harpiyafin-client-id': self.client_id,
            'harpiyafin-access-token': self.access_token,
            'harpiyafin-signature': base64.b64encode(h.digest()),
            'harpiyafin-timestamp': msg_timestamp,
        })
        return request
