...
from harpiya import fields, tools
from harpiya.addons.account_online_synchronization.tests.common import AccountOnlineSynchronizationCommon
...
class TestAccountOnlineAccount(AccountOnlineSynchronizationCommon):
    ...
    @patch('harpiya.addons.account_online_synchronization.models.account_online.AccountOnlineLink._fetch_harpiya_fin')
    def test_retrieve_pending_transactions(self, patched_fetch_harpiyafin):
        self.account_online_link.state = 'connected'
        patched_fetch_harpiyafin.side_effect = [{
            'transactions': [
    ...
    @patch('harpiya.addons.account_online_synchronization.models.account_online.AccountOnlineAccount._retrieve_transactions', return_value={})
    @patch('harpiya.addons.account_online_synchronization.models.account_online.AccountOnlineAccount._refresh', return_value={'success': True, 'data': {}})
    ...
    def test_currently_processing_fetching_transactions(self, patched_refresh, patched_transactions):
        self.account_online_account.fetching_status = 'processing'  # simulate the fact that we are currently creating entries in harpiya
    ...
    @patch('harpiya.addons.account_online_synchronization.models.account_online.requests')
    def test_delete_with_redirect_error(self, patched_request):
        ...
        # such error are returned as a HarpiyaFinRedirectException with mode link to reopen the iframe and link with a new
        ...
        first_call = self._mock_harpiyafin_error_response(code=102)
        second_call = self._mock_harpiyafin_error_response(code=300, data={'mode': 'link'})
    ...
    def test_redirect_mode_link(self, patched_request):
        # Use case being tested: Call to open the iframe which result in a HarpiyafinRedirectException in link mode
        ...
        first_call = self._mock_harpiyafin_error_response(code=300, data={'mode': 'link'})
        second_call = self._mock_harpiyafin_response(data={'delete': True})
        patched_request.post.side_effect = [first_call, second_call]
