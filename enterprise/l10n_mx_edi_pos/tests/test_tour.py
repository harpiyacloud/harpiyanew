...
class TestUi(TestMxEdiPosCommon, TestPointOfSaleHttpCommon):
    ...
    def test_mx_pos_invoice_order_and_refund(self):
        self.start_tour("/harpiya", "l10n_mx_edi_pos.test_mx_pos_invoice_order_and_refund", login=self.env.user.login)

    def test_mx_pos_invoice_order_default_usage(self):
        self.start_tour("/harpiya", "l10n_mx_edi_pos.tour_invoice_order_default_usage", login=self.env.user.login)

    def test_mx_pos_invoice_previous_order(self):
        self.start_tour("/harpiya", "l10n_mx_edi_pos.tour_invoice_previous_order", login=self.env.user.login)
    ...
    def test_mx_pos_invoice_previous_order_default_usage(self):
        self.start_tour("/harpiya", "l10n_mx_edi_pos.tour_invoice_previous_order_default_usage", login=self.env.user.login)
        invoice = self.env['account.move'].search([('move_type', '=', 'out_invoice')], order='id desc', limit=1)
        self.assertRecordValues(invoice, [{
            'partner_id': self.partner_mx.id,
            'l10n_mx_edi_usage': "I01",
