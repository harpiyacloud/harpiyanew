...
from harpiya import Command, _, api, fields, models
from harpiya.exceptions import UserError
from harpiya.service.model import PG_CONCURRENCY_EXCEPTIONS_TO_RETRY

from harpiya.addons.sale_shopee import const, utils
...
class ShopeeShop(models.Model):
    ...
    status = fields.Selection(
        help="The shop status on Shopee."
             ...
             "- Active: The shop is active and can be synchronized with Harpiya."
             "- Error: The shop is banned and cannot be synchronized with Harpiya.",
        selection=[
            ('inactive', "Inactive"),
            ('active', "Active"),
            ('error', "Error"),
    ...
    last_orders_sync_date = fields.Datetime(
        ...
        help="The last time the orders were synchronized with Shopee. Orders whose status has "
             "not changed since this date will not be created nor updated in Harpiya.",
        required=True,
        default=fields.Datetime.now,
    )
    ...
    def _process_order_data(self, order_data):
        ...
        - If the order doesn't exist in Harpiya and order status in const.ORDER_STATUSES_TO_SYNC, an
        order will be created with the data from Shopee.
        - If the order exists in Harpiya, depending on the status:
            -> Status = "CANCELLED" -> we cancel the order in Harpiya if it's not already cancelled.
            -> Status = "SHIPPED"/"COMPLETED" -> we update the status of the order in Harpiya and
        ...
        if not order:
            ...
            if shopee_status == 'CANCELLED' and order.state != 'cancel':
                ...
                # package as picked up by the carrier. We then mark the picking as done in Harpiya.
