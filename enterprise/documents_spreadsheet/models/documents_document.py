...
from harpiya import _, Command, fields, models, api
from harpiya.exceptions import UserError, AccessError, ValidationError
from harpiya.osv import expression
from harpiya.tools import image_process, consteq
from harpiya.tools.misc import DEFAULT_SERVER_DATE_FORMAT
...
class DocumentsDocument(models.Model):
    ...
    def _contains_live_data(self):
        """Return true if the spreadsheet contains live data, like Harpiya pivots, chart, etc."""
        self.ensure_one()
        if self.handler != 'spreadsheet':
            return False

        snapshot = json.loads(self._get_spreadsheet_serialized_snapshot())
        if snapshot.get("lists") or snapshot.get("pivots") or snapshot.get("chartHarpiyaMenusReferences"):
            return True

        for sheet in snapshot.get("sheets", []):
            for figure in sheet.get("figures", []):
                if figure.get("data", {}).get("type", "").startswith("harpiya_"):
                    return True

        return any(
            command.get("type") in ("INSERT_HARPIYA_LIST", "ADD_PIVOT", "LINK_HARPIYA_MENU_TO_CHART") or (
                command.get("type") == "CREATE_CHART" and
                command.get("definition", {}).get("type", "").startswith("harpiya_")
            )
