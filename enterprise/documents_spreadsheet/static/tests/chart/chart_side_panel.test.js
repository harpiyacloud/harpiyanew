...
test("From an Harpiya chart, can only change to an Harpiya chart", async () => {
    const { model, env } = await createSpreadsheetFromGraphView();
    await openChartSidePanel(model, env);
    const target = getFixture();
    await contains(".o-type-selector").click();
    const harpiyaChartTypes = chartSubtypeRegistry
        .getKeys()
        .filter((key) => key.startsWith("harpiya_"))
        .sort();
    ...
    const optionValues = Array.from(options)
        ...
        .sort();
    expect(optionValues).toEqual(harpiyaChartTypes);
});
...
test("From a spreadsheet chart, can only change to a spreadsheet chart", async () => {
    ...
    const optionValues = Array.from(options)
        .map((option) => option.dataset.id)
        .sort();
    const nonHarpiyaChartTypes = chartSubtypeRegistry
        .getKeys()
        .filter((key) => !key.startsWith("harpiya_"))
        .sort();

    expect(optionValues).toEqual(nonHarpiyaChartTypes);
});

test("Possible chart types are correct when switching from a spreadsheet to an harpiya chart", async () => {
    const { model, env } = await createSpreadsheetFromGraphView();
    createBasicChart(model, "nonHarpiyaChartId");
    await openChartSidePanel(model, env);
    const target = getFixture();
    await contains(".o-type-selector").click();
    ...
    expect(optionValues.every((value) => value.startsWith("harpiya_"))).toBe(true);

    model.dispatch("SELECT_FIGURE", { id: "nonHarpiyaChartId" });
...
test("Change harpiya chart type", async () => {
    const { model, env } = await createSpreadsheetFromGraphView();
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    expect(model.getters.getChart(chartId).type).toBe("harpiya_bar");
    await openChartSidePanel(model, env);
    /** @type {HTMLSelectElement} */
    await changeChartType("harpiya_pie");
    expect(model.getters.getChart(chartId).type).toBe("harpiya_pie");

    await changeChartType("harpiya_line");
    ...
    await changeChartType("harpiya_bar");
    expect(model.getters.getChart(chartId).type).toBe("harpiya_bar");
    expect(model.getters.getChart(chartId).stacked).toBe(false);

    await changeChartType("harpiya_stacked_bar");
    expect(model.getters.getChart(chartId).type).toBe("harpiya_bar");
    expect(model.getters.getChart(chartId).stacked).toBe(true);

    await changeChartType("harpiya_stacked_line");
    expect(model.getters.getChart(chartId).type).toBe("harpiya_line");
    expect(model.getters.getChart(chartId).stacked).toBe(true);
});
...
test("Harpiya area chart", async () => {
    const { model, env } = await createSpreadsheetFromGraphView();
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    await openChartSidePanel(model, env);
    await changeChartType("harpiya_area");

    let chartDefinition = model.getters.getChartDefinition(chartId);
    expect(chartDefinition.type).toBe("harpiya_line");
    expect(chartDefinition.fillArea).toBe(true);
    expect(chartDefinition.stacked).toBe(false);

    await changeChartType("harpiya_stacked_area");
...
test("Open chart harpiya's data properties", async function () {
    const target = getFixture();
    const { model, env } = await createSpreadsheetFromGraphView();
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("radar chart", async () => {
    ...
    const chartId = model.getters.getChartIds(sheetId)[0];
    await openChartSidePanel(model, env);
    await changeChartType("harpiya_radar");

    expect(model.getters.getChartDefinition(chartId).type).toBe("harpiya_radar");
...
test("filled radar chart", async () => {
    ...
    const chartId = model.getters.getChartIds(sheetId)[0];
    await openChartSidePanel(model, env);
    await changeChartType("harpiya_filled_radar");
...
test("waterfall chart", async () => {
    ...
    const chartId = model.getters.getChartIds(sheetId)[0];
    await openChartSidePanel(model, env);
    await changeChartType("harpiya_waterfall");

    expect(model.getters.getChartDefinition(chartId).type).toBe("harpiya_waterfall");
    const runtime = model.getters.getChartRuntime(chartId);
...
test("population pyramid chart", async () => {
    ...
    const chartId = model.getters.getChartIds(sheetId)[0];
    await openChartSidePanel(model, env);
    await changeChartType("harpiya_pyramid");

    expect(model.getters.getChartDefinition(chartId).type).toBe("harpiya_pyramid");
    const runtime = model.getters.getChartRuntime(chartId);
    expect(runtime.chartJsConfig.type).toBe("bar");
...
test("scatter chart", async () => {
    ...
    const chartId = model.getters.getChartIds(sheetId)[0];
    await openChartSidePanel(model, env);
    await changeChartType("harpiya_scatter");

    expect(model.getters.getChartDefinition(chartId).type).toBe("harpiya_scatter");
    const runtime = model.getters.getChartRuntime(chartId);
...
test("combo chart", async () => {
    ...
    const chartId = model.getters.getChartIds(sheetId)[0];
    await openChartSidePanel(model, env);
    await changeChartType("harpiya_combo");

    expect(model.getters.getChartDefinition(chartId).type).toBe("harpiya_combo");
    const runtime = model.getters.getChartRuntime(chartId);
    expect(runtime.chartJsConfig.type).toBe("bar");
...
test("horizontal & stacked horizontal bar charts", async () => {
    ...
    const chartId = model.getters.getChartIds(sheetId)[0];
    await openChartSidePanel(model, env);
    await changeChartType("harpiya_horizontal_bar");

    expect(model.getters.getChartDefinition(chartId).type).toBe("harpiya_bar");
    ...
    await changeChartType("harpiya_horizontal_stacked_bar");
    expect(model.getters.getChartDefinition(chartId).type).toBe("harpiya_bar");
...
test("doughnut charts", async () => {
    ...
    const chartId = model.getters.getChartIds(sheetId)[0];
    await openChartSidePanel(model, env);
    await changeChartType("harpiya_doughnut");

    expect(model.getters.getChartDefinition(chartId).type).toBe("harpiya_pie");
