...
import { manuallyDispatchProgrammaticEvent } from "@harpiya/hoot-dom";
...
test("can link an harpiya menu to a basic chart chart in the side panel", async function () {
    const { model } = await createSpreadsheet({ serverData });
    createBasicChart(model, chartId);
    await animationFrame();
    await openChartSidePanel();
    let harpiyaMenu = model.getters.getChartHarpiyaMenu(chartId);
    expect(harpiyaMenu).toBe(undefined, { message: "No menu linked with chart at start" });

    expect(".o-ir-menu-selector input").toHaveCount(1, {
        message: "A menu to link charts to harpiya menus was added to the side panel",
    });
    await contains(".o-ir-menu-selector input").click();
    await contains(".ui-menu-item").click();
    harpiyaMenu = model.getters.getChartHarpiyaMenu(chartId);
    expect(harpiyaMenu.xmlid).toBe("documents_spreadsheet.test.menu", {
        message: "Harpiya menu is linked to chart",
    });
});

test("can link an harpiya menu to a scorecard chart chart in the side panel", async function () {
    const { model } = await createSpreadsheet({ serverData });
...
test("can link an harpiya menu to a gauge chart chart in the side panel", async function () {
    const { model } = await createSpreadsheet({ serverData });
...
test("can remove link between an harpiya menu and a chart in the side panel", async function () {
    const { model } = await createSpreadsheet({ serverData });
    createBasicChart(model, chartId);
    await animationFrame();
    model.dispatch("LINK_HARPIYA_MENU_TO_CHART", {
        chartId,
        harpiyaMenuId: "documents_spreadsheet.test.menu",
    });
    await openChartSidePanel();
    await animationFrame();
    const irMenuField = target.querySelector(".o-ir-menu-selector input");
    ...
    expect(harpiyaMenu).toBe(undefined, { message: "no menu is linked to chart" });
});
