...
import { components, registries } from "@harpiya/o-spreadsheet";
...
async function openMenuSelector(params = {}) {
    ...
    await contains(".o-menu-item[data-name='harpiyaMenu']").click();
...
test("insert a new ir menu link", async function () {
    ...
    expect(cell.content).toBe("[menu with xmlid](harpiya://ir_menu_xml_id/test_menu)", {
...
test("insert a new ir menu link when the menu does not have an xml id", async function () {
    ...
    await contains("button.o-save").click();
    const cell = getCell(model, "A1");
    expect(cell.content).toBe("[menu without xmlid](harpiya://ir_menu_id/12)", {
        message: "The content should be the complete markdown link",
    });
...
test("ir.menu link keep breadcrumb", async function () {
    ...
    setCellContent(model, "A1", "[menu with xmlid](harpiya://ir_menu_xml_id/test_menu)");
