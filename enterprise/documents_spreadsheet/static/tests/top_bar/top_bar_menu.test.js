    ...
    const spreadsheetData = {
        ...
        sheets: [
            ...
            },
            {
                id: "sh2",
                cells: {
                    A2: { content: `=HARPIYA.PIVOT(1,"probability","bar","false","foo",2)` },
                },
            },
        ],
        pivots: {
...
test("Can Insert harpiya formulas from Insert > Functions > Harpiya", async function () {
    const { model } = await createSpreadsheet();

    setCellContent(model, "A1", `Hi :)`);

    // Skip the visibility checks because on small screens (height <~700 px) the menus become too
    // tall for the screen and need to be scrolled to have the "Harpiya" menu item visible.
    ...
    await contains(".o-menu-item[title='Harpiya']", skipVisibilityChecks).click();
    await contains(".o-menu-item[title='HARPIYA.CURRENCY.RATE']", skipVisibilityChecks).click();

    await press("Enter");
    await animationFrame();

    expect(getCellContent(model, "A1")).toBe("=HARPIYA.CURRENCY.RATE()");
});
