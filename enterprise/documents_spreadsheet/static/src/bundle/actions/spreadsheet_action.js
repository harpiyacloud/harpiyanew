...
import { useState, useSubEnv } from "@harpiya/owl";
...
export class SpreadsheetAction extends AbstractSpreadsheetAction {
    ...
    async saveAsTemplate() {
        const model = new Model(this.model.exportData(), {
            custom: {
                env: this.env,
                harpiyaDataProvider: this.model.config.custom.harpiyaDataProvider,
            },
        });
    ...
    async freezeAndShareSpreadsheet() {
        ...
        const { freezeHarpiyaData } = harpiya.loader.modules.get("@spreadsheet/helpers/model");
        const data = await freezeHarpiyaData(this.model);
