...
export class IoTRestartHarpiya extends Component {
    ...
    async onClick() {
        this.dialog.add(ConfirmationDialog, {
            body: _t("Are you sure you want to restart Harpiya on the IoT box?"),
            confirm: () => this.restartHarpiya(),
            cancel: () => {},
        });
    }
    ...
    async callRestartMethodOnServer() {
        ...
        try {
            ...
            const response = await this.http.get(`${this.ip_url}/hw_posbox_homepage/restart_harpiya_service`);
    ...
    doWarnFail() {
        this.notification.add(_t("Failed to restart Harpiya on %s", this.name), { type: "danger" });
    }
}

export const ioTRestartHarpiya = {
    component: IoTRestartHarpiya,
    extractProps: ({ attrs }) => {
        return {
...
registry.category("view_widgets").add("iot_restart_harpiya", ioTRestartHarpiya);
