...
class TestSubscriptionInvoice(TestSubscriptionCommon):
    ...
    def test_sale_subscription_post_invoice(self):
        ...
        with patch('harpiya.addons.sale_subscription.models.sale_order_line.SaleOrderLine._reset_subscription_quantity_post_invoice', patched_reset), freeze_time("2021-01-01"):
            sub = self.subscription
            sub.action_confirm()
            self.env['sale.order']._cron_recurring_create_invoice()
