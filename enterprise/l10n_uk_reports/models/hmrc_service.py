...
from harpiya.release import version
import logging
...
class HmrcService(models.AbstractModel):
    ...
    def _get_fraud_prevention_info(self, client_data=None):
        ...
        try:
            ...
            gov_dict['Gov-Vendor-Version'] = '&'.join([urls.url_quote("Harpiya") + "=" + urls.url_quote(gov_vendor_version)]*2) # We can not percent encode the separator and we need to do it for the key and the value. Client and Server sides are the same
            gov_dict['Gov-Client-User-IDs'] = urls.url_quote("Harpiya") + "=" + urls.url_quote(self.env.user.name)  # We can not percent encode the separator, same as Gov-Vendor-Version
            ...
            gov_dict['Gov-Vendor-Product-Name'] = urls.url_quote(f"Harpiya {version}")
            ...
            if hashed_license:
                gov_dict['Gov-Vendor-License-IDs'] = urls.url_quote("Harpiya") + "=" + urls.url_quote(hashed_license)
        except Exception:
    ...
    def _get_proxy_server(self):
        mode = self.env['ir.config_parameter'].sudo().get_param("l10n_uk_reports.hmrc_mode", 'production')
        proxy_server = 'https://l10n-uk-hmrc.api.harpiya.com' if mode == 'production' else 'https://l10n-uk-hmrc.test.harpiya.com'
        return proxy_server
