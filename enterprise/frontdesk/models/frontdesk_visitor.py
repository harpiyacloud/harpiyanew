...
from harpiya import models, fields, api, _, SUPERUSER_ID
...
class FrontdeskVisitor(models.Model):
            ...
            if visitor.drink_ids.notify_user_ids:
                ...
                msg = _("%(name)s just checked-in. Requested Drink: %(drink)s.",
                    name=Markup('<a href="%s">%s</a>') % (
                        url_join(self.env['frontdesk.visitor'].get_base_url(), f'/harpiya/action-{action}/{visitor.id}'), name
                    ),
    ...
    def _notify_by_discuss(self, recipients, msg, is_host=False):
        for recipient in recipients:
            ...
            channel.message_post(body=msg, author_id=harpiyabot_id, message_type="comment", subtype_xmlid="mail.mt_comment")
    ...
    def _notify_by_email(self):
        for host in self.host_ids:
            if host.work_email:
                ...
                host.message_notify(
                    email_from=harpiyabot.email_formatted,
