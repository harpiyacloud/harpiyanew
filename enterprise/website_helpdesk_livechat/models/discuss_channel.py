...
from harpiya.tools import is_html_empty, plaintext2html
from harpiya.osv.expression import OR
from harpiya.tools.mimetypes import get_extension
        ...
        if key[0].lower() == ticket_command:
            if len(key) == 1:
                ...
                for message in self.message_ids.sorted(key=lambda r: r.id):
                    if (not message.attachment_ids and is_html_empty(message.body)) or message.author_id == harpiyabot:
                        continue
