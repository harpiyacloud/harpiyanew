...
from harpiya.addons.pos_restaurant.tests import test_frontend
...
class TestUi(test_frontend.TestFrontendCommon):
    ...
    def test_cancel_order_notifies_display(self):
        ...
        # open a session, the /pos/ui controller will redirect to it
        with patch('harpiya.addons.pos_preparation_display.models.preparation_display.Pos_Preparation_DisplayDisplay._send_load_orders_message', new=_send_load_orders_message):
            self.pos_config.printer_ids.unlink()
            self.pos_config.with_user(self.pos_user).open_ui()
