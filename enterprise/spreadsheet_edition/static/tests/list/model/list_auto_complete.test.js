...
test("HARPIYA.LIST id", async function () {
    ...
    for (const formula of ["=HARPIYA.LIST(", "=HARPIYA.LIST( ", "=HARPIYA.LIST.HEADER("]) {
...
test("HARPIYA.LIST id exact match", async function () {
    const { store: composer, model } = await makeStore(CellComposerStore);
    insertListInSpreadsheet(model, {
        model: "partner",
        columns: ["foo", "bar", "date", "product_id"],
    ...
    await animationFrame();
    composer.startEdition("=HARPIYA.LIST(1");
    const autoComplete = composer.autocompleteProvider;
    expect(autoComplete).toBe(undefined);
});

test("HARPIYA.LIST field name", async function () {
    ...
    await animationFrame();
    composer.startEdition("=HARPIYA.LIST(1,1,");
    const autoComplete = composer.autocompleteProvider;
    ...
    expect(composer.currentContent).toBe('=HARPIYA.LIST(1,1,"id"');
    expect(composer.autocompleteProvider).toBe(undefined, { message: "autocomplete closed" });
});

test("HARPIYA.LIST.HEADER field name", async function () {
    const { model } = await createModelWithDataSource();
    const { store: composer } = await makeStoreWithModel(model, CellComposerStore);
    insertListInSpreadsheet(model, {
        model: "partner",
    ...
    await animationFrame();
    composer.startEdition("=HARPIYA.LIST.HEADER(1,");
    const autoComplete = composer.autocompleteProvider;
    const allFields = Object.keys(Partner._fields);
    expect(autoComplete.proposals.map((p) => p.text)).toEqual(
        allFields.map((field) => `"${field}"`),
...
test("HARPIYA.LIST field name with invalid list id", async function () {
    const { store: composer, model } = await makeStore(CellComposerStore);
    insertListInSpreadsheet(model, {
        model: "partner",
        columns: ["foo", "bar", "date", "product_id"],
    ...
    for (const listId of ["", "0", "42"]) {
        composer.startEdition(`=HARPIYA.LIST(${listId},1,`);
        const autoComplete = composer.autocompleteProvider;
        expect(autoComplete).toBe(undefined);
        composer.cancelEdition();
    }
