import { beforeEach, describe, test } from "@harpiya/hoot";
...
function insertList(model, id, anchor = [0, 0]) {
    ...
    return model.dispatch("INSERT_HARPIYA_LIST", {
...
test("Add two lists concurrently", async () => {
    ...
    spExpect([alice, bob, charlie]).toHaveSynchronizedValue(
        ...
        `=HARPIYA.LIST.HEADER(1,"foo")`
    );
    spExpect([alice, bob, charlie]).toHaveSynchronizedValue(
        (user) => getCellFormula(user, "A26"),
        `=HARPIYA.LIST.HEADER(2,"foo")`
    );
    await animationFrame();
...
test("Can undo a command before a INSERT_HARPIYA_LIST", async () => {
...
test("Rename and remove a list concurrently", async () => {
    insertList(alice, "1");
    await network.concurrent(() => {
        alice.dispatch("RENAME_HARPIYA_LIST", {
            listId: "1",
            name: "test",
        });
        bob.dispatch("REMOVE_HARPIYA_LIST", {
            listId: "1",
        });
    });
    spExpect([alice, bob, charlie]).toHaveSynchronizedValue(
...
test("Re-insert and remove a list concurrently", async () => {
    ...
    await network.concurrent(() => {
        ...
        alice.dispatch("RE_INSERT_HARPIYA_LIST", {
            id: "1",
            col: 0,
            row: 0,
...
test("remove and update a domain of a list concurrently", async () => {
    insertList(alice, "1");
    await network.concurrent(() => {
        alice.dispatch("REMOVE_HARPIYA_LIST", {
            listId: "1",
        });
        bob.dispatch("UPDATE_HARPIYA_LIST_DOMAIN", {
            listId: "1",
            domain: [["foo", "in", [55]]],
        });
    });
...
test("remove and update a sorting of a list concurrently", async () => {
    ...
    await network.concurrent(() => {
        ...
        bob.dispatch("UPDATE_HARPIYA_LIST", {
            listId: "1",
            list: {
                ...listDefinition,
                searchParams: {
...
test("Duplicate and remove list at the same time concurrently", async () => {
    ...
    await network.concurrent(() => {
        ...
        alice.dispatch("DUPLICATE_HARPIYA_LIST", {
            listId: "1",
            newListId: "2",
        });
    });
...
test("Duplicate list concurrently", async () => {
    insertList(alice, "1");
    await network.concurrent(() => {
        bob.dispatch("DUPLICATE_HARPIYA_LIST", {
            listId: "1",
            newListId: "2",
        });
        alice.dispatch("DUPLICATE_HARPIYA_LIST", {
