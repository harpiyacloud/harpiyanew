...
import { Model, helpers } from "@harpiya/o-spreadsheet";
import {
...
test("link is kept when copying chart", async () => {
    ...
    expect(model.getters.getChartHarpiyaMenu(chartId).id).toBe(1);
    ...
    for (const _chartId of chartIds) {
        expect(model.getters.getChartHarpiyaMenu(_chartId).id).toBe(1);
    }
});

test("copy/paste Harpiya chart field matching", async () => {
    ...
    model.dispatch("PASTE", { target: [toZone("A1")] });
    const chartIds = model.getters.getChartIds(sheetId);
    expect(model.getters.getHarpiyaChartFieldMatching(chartId1, filterId).chain).toBe(
        "partner_id.company_id"
    );
    expect(model.getters.getHarpiyaChartFieldMatching(chartId2, filterId).chain).toBe(
        "user_id.company_id"
    );
    expect(model.getters.getHarpiyaChartFieldMatching(chartIds[2], filterId).chain).toBe(
        "user_id.company_id"
    );
});

test("cut/paste Harpiya chart field matching", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "harpiya_pie" });
    insertChartInSpreadsheet(model, "harpiya_bar");
    const sheetId = model.getters.getActiveSheetId();
    const [chartId1, chartId2] = model.getters.getChartIds(sheetId);
    ...
    expect(model.getters.getHarpiyaChartFieldMatching(chartIds[1], filterId).chain).toBe(
        "user_id.company_id"
    );
});
