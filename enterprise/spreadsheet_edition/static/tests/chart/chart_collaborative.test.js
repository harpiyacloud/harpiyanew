...
test("Chart link to harpiya menu collaborative", async () => {
    const chartId = "1";
    const sheetId = alice.getters.getActiveSheetId();
    createBasicChart(alice, chartId);
    await network.concurrent(() => {
        alice.dispatch("DELETE_FIGURE", { id: chartId, sheetId });
        bob.dispatch("LINK_HARPIYA_MENU_TO_CHART", {
            chartId,
            harpiyaMenuId: "harpiyaTestMenu",
        });
    });
    ...
    spExpect([alice, bob, charlie]).toHaveSynchronizedValue(
        (user) => user.getters.getChartHarpiyaMenu(chartId),
        undefined
    );
});
