...
test("HARPIYA.FILTER.VALUE", async function () {
    ...
    for (const formula of [
        "=HARPIYA.FILTER.VALUE(",
        '=HARPIYA.FILTER.VALUE("',
        '=HARPIYA.FILTER.VALUE("fil',
        "=HARPIYA.FILTER.VALUE(fil",
    ]) {
        ...
        expect(composer.currentContent).toBe('=HARPIYA.FILTER.VALUE("filter 1"');
...
test("escape double quotes in filter name", async function () {
    ...
    composer.startEdition("=HARPIYA.FILTER.VALUE(");
    const autoComplete = composer.autocompleteProvider;
