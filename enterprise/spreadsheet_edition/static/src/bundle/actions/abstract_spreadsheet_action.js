...
import { onMounted, onWillStart, useState, Component, useSubEnv, onWillUnmount } from "@harpiya/owl";
...
export class AbstractSpreadsheetAction extends Component {
    ...
    getModelConfig() {
        ...
        const harpiyaDataProvider = new HarpiyaDataProvider(this.env);
        harpiyaDataProvider.addEventListener("data-source-updated", () => {
            this.model.dispatch("EVALUATE_CELLS");
        });
        return {
            custom: { env: this.env, orm: this.orm, harpiyaDataProvider },
            external: {
