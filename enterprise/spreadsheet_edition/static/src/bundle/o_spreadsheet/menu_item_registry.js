...
import { registries, stores } from "@harpiya/o-spreadsheet";
...
topbarMenuRegistry.addChild("data_sources_data", ["data"], (env) => {
    ...
    const lists_items = env.model.getters.getListIds().map((listId, index) => {
        ...
        return {
            ...
            icon: "o-spreadsheet-Icon.HARPIYA_LIST",
    ...
    const charts_items = env.model.getters.getHarpiyaChartIds().map((chartId, index) => {
        return {
            id: `item_chart_${chartId}`,
            name: env.model.getters.getHarpiyaChartDisplayName(chartId),
            sequence: sequence++,
            execute: (env) => {
            ...
            separator: index === env.model.getters.getHarpiyaChartIds().length - 1,
        };
    });
