...
import { astToFormula, UIPlugin, tokenize } from "@harpiya/o-spreadsheet";
...
export class ListAutofillPlugin extends UIPlugin {
    ...
    getNextListValue(formula, isColumn, increment) {
        ...
        if (functionName === "HARPIYA.LIST.HEADER") {
    ...
    getTooltipListFormula(formula, isColumn) {
        ...
        if (isColumn || functionName === "HARPIYA.LIST.HEADER") {
            const fieldName = functionName === "HARPIYA.LIST" ? evaluatedArgs[2] : evaluatedArgs[1];
    ...
    _getListFunction(listId, position, field) {
        return `=HARPIYA.LIST(${listId},${position},"${field}")`;
    }

    _getListHeaderFunction(listId, field) {
        return `=HARPIYA.LIST.HEADER(${listId},"${field}")`;
    }
}
