...
export class CommentsStore extends SpreadsheetStore {
    ...
    handle(cmd) {
        switch (cmd.type) {
            case "SET_VIEWPORT_OFFSET":
                if (this.cellPopoverStore.persistentCellPopover?.type === "HarpiyaCellComment") {
        ...
        if (this.getters.isGridSelectionActive() && !this.getters.isReadonly()) {
            ...
            if (threads && threads.length) {
                ...
                if (!thread.isResolved) {
                    this.cellPopoverStore.open(position, "HarpiyaCellComment");
    ...
    toggleComments() {
        ...
        if (
            ...
            this.cellPopoverStore.persistentCellPopover?.type === "HarpiyaCellComment"
        ) {
            this.cellPopoverStore.close();
            this.selectedThreadId = undefined;
        }
