...
WSFE_ERRORS = {
    ...
    '10016-2': _lt('There may have been a mismatch in the numbering of this type of document between Harpiya and AFIP.'),
...
WSFEX_ERRORS = {
    ...
    '1580': _lt('If the customer belongs to a Free Zone you should give the customer CUIT number in order to continue\n * If is a Foreign Customer is possible that Harpiya does not have Legal VAT/Natrual VAT configured for your customer country.\n\n\t 1. Please go to AFIP page and search for the code in the section "Tablas del Sistema > Paises" that is in the bottom right of the page and get the corresponding code for the country.\n\t 2. Go back to  Harpiya, in the menú Sales > Settings > Address Book > Localization > Countries, activate the developer mode and add the AFIP code in the corresponding field'),
    '1668': _lt('The registration for the online exportation invoices (invoice generate from AFIP page) is different than the registration for electronic exportation invoices. Therefore you must configure this one also because you will not be able to create invoices of this type in Harpiya.\n\nWhen configuring, you must check the option "Factura Electrónica Exportación de Servicios" and then "Webservice/Facturador Plus".\n\nView image https://www.adhoc.com.ar/web/content/162597?unique=202798c78764e1c1d31b35d46556a7947ac511e8&download=true'),
...
WBSFE_ERRORS = {
    '501': _lt('This is an internal error in AFIP. Contact to your Harpiya provider in order to let it know that is happening and to investigate what is going on with the AFIP service'),
