...
from harpiya.exceptions import UserError, RedirectWarning
from harpiya.tools.float_utils import float_repr, float_round
from harpiya.tools import html2plaintext, plaintext2html
from harpiya.tools.sql import column_exists, create_column
...
class AccountMove(models.Model):
    ...
    def _post(self, soft=True):
        ...
        for inv in ar_edi_invoices:
            ...
            # in Harpiya for consistency, this way if an error ocurrs later in another invoice we will have the ones
            # correctly validated in AFIP in Harpiya (CAE, Result, xml response/request).
                ...
                if response.Errors:
                    ...
                    if '10016' in return_codes:
                        try:
                            ...
                            harpiya_current_invoice_dict = request_data['FeDetReq'][0]['FECAEDetRequest']
                            harpiya_current_invoice_number = harpiya_current_invoice_dict['CbteDesde']
                            # verify if the invoice that is being validated in Harpiya has lower date than the last one registered in afip
                            last_afip_inv_date = response2.ResultGet.CbteFch
                            harpiya_current_invoice_date = harpiya_current_invoice_dict['CbteFch']
                        except (Timeout, ConnectionError, RequestException, HTTPError, KeyError):
                            return_codes.extend(['10016-1', '10016-2'])
                        else:
                            if last_afip_inv_date > harpiya_current_invoice_date:
                                return_codes.remove('10016')
                                return_codes.append('10016-1')
                            # verify if the invoice that is being validated in Harpiya follows the sequence of the last invoice registered in afip
                            # if the sequence is reset (number 00000001) then is not necessary to know the last afip number because number 00000000 does
                            # not exists
                            elif harpiya_current_invoice_number <= last_number_afip and harpiya_current_invoice_number != 1:
                                return_codes.remove('10016')
    ...
    def _l10n_ar_check_afip_auth_verify_required(self):
        ...
        if still_missing:
            if len(still_missing) > 1:
                raise UserError(_(
                    'We can not post these vendor bills in Harpiya because the '
            ...
            raise UserError(_(
                'We can not post this vendor bill in Harpiya because the AFIP '
