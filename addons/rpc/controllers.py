...
from harpiya.http import Controller, route, dispatch_rpc, request, Response
from harpiya.fields import Date, Datetime, Command
from harpiya.tools import lazy
from harpiya.tools.misc import frozendict
...
# User code must use the exceptions defined in ``harpiya.exceptions`` (not
...
def xmlrpc_handle_exception_int(e):
    if isinstance(e, harpiya.exceptions.RedirectWarning):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    elif isinstance(e, harpiya.exceptions.AccessError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_ERROR, str(e))
    elif isinstance(e, harpiya.exceptions.AccessDenied):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_ACCESS_DENIED, str(e))
    elif isinstance(e, harpiya.exceptions.UserError):
        fault = xmlrpc.client.Fault(RPC_FAULT_CODE_WARNING, str(e))
    else:
        info = sys.exc_info()
...
def xmlrpc_handle_exception_string(e):
    ...
    elif isinstance(e, harpiya.exceptions.MissingError):
    ...
    # InternalError
        ...
        info = sys.exc_info()
        formatted_info = "".join(traceback.format_exception(*info))
        fault = xmlrpc.client.Fault(harpiya.tools.exception_to_unicode(e), formatted_info)

    return dumps(fault)


class HarpiyaMarshaller(xmlrpc.client.Marshaller):
    ...
    # Historically, harpiya is sending binary as base64 string.
...
def dumps(params: list | tuple | xmlrpc.client.Fault) -> str:
    response = HarpiyaMarshaller(allow_none=False).dumps(params)
    return f"""\
<?xml version="1.0"?>
<methodResponse>
{response}
