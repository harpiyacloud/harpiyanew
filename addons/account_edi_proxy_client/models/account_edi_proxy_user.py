...
from .account_edi_proxy_auth import HarpiyaEdiProxyAuth
    ...
    def _make_request(self, url, params=False):
        ...
        try:
            response = requests.post(
                ...
                auth=HarpiyaEdiProxyAuth(user=self)).json()
        ...
        if proxy_error:
            ...
            if error_code == 'invalid_signature':
                raise AccountEdiProxyError(
                    error_code,
                    _("Invalid signature for request. This might be due to another connection to harpiya Access Point "
        ...
        if edi_mode == 'demo':
            ...
            if 'error' in response:
                if response['error'] == 'A user already exists with this identification.':
                    ...
                    # Harpiya side. We are for now forced to check the error message.
