...
from harpiya.addons.base.tests.common import HttpCase, HttpCaseWithUserDemo
from harpiya.addons.mail.tests.common import MailCommon, mail_new_test_user
from harpiya.http import STATIC_CACHE_LONG
from harpiya import Command, fields


@harpiya.tests.tagged("-at_install", "post_install", "mail_controller")
...
class TestMessageLinks(MailCommon, HttpCase):
    ...
    def test_message_link_by_employee(self):
        ...
        with self.subTest(channel_message=channel_message):
            expected_url = self.base_url() + f'/harpiya/action-mail.action_discuss?active_id={channel_message.res_id}&highlight_message_id={channel_message.id}'
