...
class TestTranslationController(HttpCaseWithUserDemo):
    ...
    def test_access_right(self):
        with self.assertRaises(JsonRpcException, msg="harpiya.http.SessionExpiredException"):
        ...
        with self.assertRaises(JsonRpcException, msg="harpiya.exceptions.AccessError"), mute_logger("harpiya.http"):
            self._mock_translation_request({"message_id": self.message.id})
