...
# harpiya-mailgate
#
# This program will read an email from stdin and forward it to harpiya. Configure
# a pipe alias in your mail server to use it, postfix uses a syntax that looks
# like:
#
# email@address: "|/home/harpiya/src/harpiya-mail.py"
#
# while exim uses a syntax that looks like:
#
# *: |/home/harpiya/src/harpiya-mail.py
...
def main():
    ...
    op.add_option("-d", "--database", dest="database", help="Harpiya database name (default: %default)", default='harpiya')
    op.add_option("-u", "--userid", dest="userid", help="Harpiya user id to connect with (default: %default)", default=1, type=int)
    op.add_option("-p", "--password", dest="password", help="Harpiya user password (default: %default)", default='admin')
    op.add_option("--host", dest="host", help="Harpiya host (default: %default)", default='localhost')
    op.add_option("--port", dest="port", help="Harpiya port (default: %default)", default=8069, type=int)
    ...
    try:
        ...
        if e.faultString == 'Access denied' and e.faultCode == 3:
            ...
            postfix_exit(EX_NOUSER, "alias does not exist in harpiya\n", o.debug)
        else:
...
try:
    ...
    # may include the password for the harpiya user.
