...
export class OutOfFocusService {
    ...
    async notify(message, thread) {
        ...
        let icon = "/mail/static/src/img/harpiyabot_transparent.png";
        if (!author) {
    ...
    sendNotification({ message, sound = true, title, type, icon }) {
        ...
        try {
            ...
            if (error.message.includes("ServiceWorkerRegistration")) {
                this.sendHarpiyaNotification(message, { sound, title, type });
            } else {
                throw error;
            }
        }
     ...
     */
    async sendHarpiyaNotification(message, options) {
        const { sound } = options;
