...
export class Persona extends Record {
    ...
    /** @type {ReturnType<import("@harpiya/owl").markup>|string|undefined} */
    ...
    storeAsTrackedImStatus = Record.one("Store", {
        ...
        onAdd() {
            ...
            this.store.env.services.bus_service.addChannel(`harpiya-presence-${model}_${this.id}`);
        },
        onDelete() {
            if (!this.store.env.services.bus_service.isActive) {
                return;
            ...
            const model = this.type === "partner" ? "res.partner" : "mail.guest";
            this.store.env.services.bus_service.deleteChannel(`harpiya-presence-${model}_${this.id}`);
        },
        eager: true,
