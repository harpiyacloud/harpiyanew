 ...
 * @param {Object} param1
 * @param {string} [param1.action] harpiya client action
...
async function openDiscussChannel(channelId, { action, joinCall = false, source } = {}) {
    const discussURLRegexes = [new RegExp("/harpiya/discuss")];
    if (action) {
        discussURLRegexes.push(
            new RegExp(`/harpiya/\\d+/action-${action}`),
            new RegExp(`/harpiya/action-${action}`)
        );
    }
    let targetClient;
    ...
    if (action) {
        const url = new URL(`/harpiya/action-${action}`, location.origin);
...
self.addEventListener("notificationclick", (event) => {
    ...
    if (event.notification.data) {
        ...
        if (model === "discuss.channel") {
            ...
            event.waitUntil(clients.openWindow(`/harpiya/${modelPath}/${res_id}`));
        }
    }
});
