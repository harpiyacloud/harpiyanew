...
import { animationFrame, Deferred, press, tick, waitFor } from "@harpiya/hoot-dom";
import { mockDate } from "@harpiya/hoot-mock";
import {
    asyncStep,
    Command,
    getService,
...
import { EventBus } from "@harpiya/owl";
...
test("Posting message should transform links.", async () => {
    ...
    await start();
    await openDiscuss(channelId);
    await insertText(".o-mail-Composer-input", "test https://www.harpiya.com/");
    await press("Enter");
    await contains("a[href='https://www.harpiya.com/']");
});
...
test("no out-of-focus notification on receiving self messages in chat", async () => {
    ...
    // simulate receiving a new message of self with harpiya out-of-focused
...
test("out-of-focus notif on needaction message in channel", async () => {
    ...
    // simulate receiving a new needaction message with harpiya out-of-focused
...
test("receive new chat message: out of harpiya focus (notification, chat)", async () => {
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({ name: "Dumbledore" });
    const userId = pyEnv["res.users"].create({ partner_id: partnerId });
    const channelId = pyEnv["discuss.channel"].create({
    ...
    await contains(".o-mail-ChatWindow", { count: 0 });
    await waitForSteps(["init_messaging"]);
    // simulate receiving a new message with harpiya out-of-focused
    withUser(userId, () =>
        rpc("/mail/message/post", {
            post_data: {
                body: "New message",
...
test("receive new chat messages: out of harpiya focus (tab title)", async () => {
    let stepCount = 0;
    const pyEnv = await startServer();
    ...
    // simulate receiving a new message in chat 1 with harpiya out-of-focused
    ...
    // simulate receiving a new message in chat 2 with harpiya out-of-focused
    await withUser(bobUserId, () =>
        rpc("/mail/message/post", {
            post_data: { body: "Hello world!", message_type: "comment" },
    ...
    await waitForSteps(["set_counters:discuss"]);
    // simulate receiving another new message in chat 2 with harpiya focused
    await withUser(bobUserId, () =>
        rpc("/mail/message/post", {
            post_data: { body: "Hello world!", message_type: "comment" },
            thread_id: channelId_2,
...
test("Partner IM status is displayed as thread icon in top bar of channels of type 'chat'", async () => {
    ...
    pyEnv["discuss.channel"].create([
        ...
        },
        {
            channel_member_ids: [
                Command.create({ partner_id: serverState.partnerId }),
                Command.create({ partner_id: serverState.harpiyabotId }),
            ],
            channel_type: "chat",
        },
    ]);
    ...
    await click(".o-mail-DiscussSidebarChannel", { text: "HarpiyaBot" });
...
test("Chatter notification in messaging menu should open the form view even when discuss app is open", async () => {
    ...
    const messageId = pyEnv["mail.message"].create({
        ...
        author_id: serverState.harpiyabotId,
        needaction: true,
        res_id: partnerId,
    });
    pyEnv["mail.notification"].create({
