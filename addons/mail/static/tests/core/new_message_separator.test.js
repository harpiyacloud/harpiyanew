...
import { click as hootClick, press, queryFirst } from "@harpiya/hoot-dom";
...
test("show new message separator on receiving new message when out of harpiya focus", async () => {
    const pyEnv = await startServer();
    const partnerId = pyEnv["res.partner"].create({ name: "Foreigner partner" });
