...
from harpiya import _, api, exceptions, fields, models, tools
from harpiya.addons.mail.tools.discuss import Store
from harpiya.addons.mail.tools.web_push import push_to_end_point, DeviceUnreachableError
from harpiya.exceptions import MissingError, AccessError
...
from harpiya.tools.mail import (
...
class MailThread(models.AbstractModel):
    ...
    def _detect_write_to_catchall(self, msg_dict):
        ...
        # (see harpiya/harpiya#161782), to clean when reaching master
    ...
    def _message_route_process(self, message, message_dict, routes):
        ...
        for model, thread_id, custom_values, user_id, alias in routes or ():
            ...
            # switch to harpiyabot for all incoming message creation
            ...
            for x in ('from', 'recipients', 'references', 'in_reply_to', 'x_harpiya_message_id',
            ...
            if thread_root._name == 'mail.thread':  # message with parent_id not linked to record
                ...
                # checks anyway for real author and filters inactive (like harpiyabot)
    ...
    def message_process(self, model, message, custom_values=None,
        ...
        if msg_dict.get('x_harpiya_message_id'):
            message_ids.append(msg_dict['x_harpiya_message_id'])
        ...
        if existing_msg_ids:
            if msg_dict.get('x_harpiya_message_id'):
                _logger.info('Ignored mail from %s to %s with Message-Id %s / Context Message-Id %s: found duplicated Message-Id during processing',
                             msg_dict.get('email_from'), msg_dict.get('to'), msg_dict.get('message_id'), msg_dict.get('x_harpiya_message_id'))
            else:
    ...
    def message_parse(self, message, save_original=False):
        ...
        msg_dict['x_harpiya_message_id'] = (message.get('X-Harpiya-Message-Id') or '').strip()
    ...
    def _get_bounced_message_data(self, message, message_dict):
        ...
        """
        reference_ids = []
        headers = ('Message-Id', 'X-Harpiya-Message-Id', 'X-Microsoft-Original-Message-ID')
    ...
    def _partner_find_from_emails(self, records_emails, avoid_alias=True, ban_emails=None,
        ...
        # inspired notably from harpiya/harpiya@80a0b45df806ffecfb068b5ef05ae1931d655810; final
    ...
    def _message_compute_real_author(self, author_id):
        ...
        # message author. In any case avoid harpiyabot.
        if self.env.user.active:  # note that harpiyabot is always inactive, there is a python check
    ...
    def _notify_by_email_get_base_mail_values(self, message, recipients_data, additional_values=None):
        ...
        # SES SMTP may replace Message-Id and In-Reply-To refers an internal ID not stored in Harpiya)
    ...
    def _notify_by_web_push_prepare_payload(self, message, msg_vals=False):
        ...
        if author_id:
            ...
            icon = '/web/static/img/harpiya-icon-192x192.png'
    ...
    def _notify_get_recipients_classify(self, message, recipients_data,
        ...
        :return list: list of groups (see '_notify_get_recipients_groups')
          with 'recipients' key filled with matching partners, like
            [{
                'active': True,
                'button_access': {'url': 'https://harpiya.com/url', 'title': 'Title'},
                'has_button_access': False,
    ...
    def _notify_get_recipients_for_extra_notifications(self, message, recipients_data, msg_vals=False):
        """ Never send to author and to people outside Harpiya (email) except comments """
        notif_pids = []
        notif_pids_notinbox = []
