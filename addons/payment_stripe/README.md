...
- `16.4`
  - The previous Checkout API that allowed for redirect payments is replaced by the Payment Intents
    API that supports direct payments. harpiya/harpiya#123573
  - The support for eMandates for recurring payments is added. harpiya/harpiya#123573
  - The responses of webhook notifications are sent with the proper HTTP code. harpiya/harpiya#117940
- `16.0`
  - Stripe uses the payment methods set up on the account when none are assigned to the payment
    provider in Harpiya, instead of only offering the "Card" payment method. harpiya/harpiya#107647
  - The support for express checkout is added. harpiya/harpiya#88374
- `15.4`
  - The support for full and partial refunds is added. harpiya/harpiya#92235
- `15.3`
  - Webhook notifications accept three new events based on the PaymentIntent and SetupIntent objects
    ...
    harpiya/harpiya#84150
  - The support for manual capture is added. harpiya/harpiya#69598
- `15.2`
  - An HTTP 404 "Forbidden" error is raised instead of a Validation error when the authenticity of
    the webhook notification cannot be verified. harpiya/harpiya#81607
- `15.0`
  - A new button is added to create a webhook automatically. harpiya/harpiya#79621
  - The support for the Stripe Connect onboarding flow is added. harpiya/harpiya#79621
- `14.3`
  - The previous direct payment flow that was supported by the SetupIntent API is replaced by a
    payment with redirection flow using the Checkout API. harpiya/harpiya#141661
