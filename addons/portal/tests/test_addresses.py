...
from harpiya.tests.common import JsonRpcException
...
class TestPortalAddresses(BaseCommon, HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.company_partner = cls.env['res.partner'].create({
            'name': 'Test Harpiya Software Technologies, LLC',
            'is_company': True,
            'email': 'harpiya@harpiya.com',
    ...
    def test_address_archiving(self):
        ...
        with self.assertRaises(JsonRpcException), mute_logger('harpiya.http'):
            self.make_jsonrpc_request(
