from harpiya.addons.hr_expense.tests.common import TestExpenseCommon
...
class TestUi(TestExpenseCommon, HttpCase):
    ...
    def test_expense_manager_can_always_set_employee(self):
        ...
        self.start_tour('/harpiya', 'create_expense_no_employee_access_tour', login=self.expense_user_manager.login)
    ...
    def test_no_zero_amount_expense_in_expense(self):
        ...
        with mute_logger("harpiya.http"):
            self.start_tour('/harpiya', 'do_not_create_zero_amount_expense', login=self.expense_user_manager.login)
