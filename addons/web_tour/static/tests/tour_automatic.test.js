...
import { afterEach, beforeEach, describe, expect, test } from "@harpiya/hoot";
import { advanceTime, animationFrame, queryFirst } from "@harpiya/hoot-dom";
...
test("a tour with invalid step trigger", async () => {
    ...
    await harpiya.startTour("tour_invalid_trigger", { mode: "auto" }); // Use harpiya to run tour from registry because this is a test tour
    await waitForMacro();
    const expectedSteps = [
...
test("a failing tour logs the step that failed in run", async () => {
    ...
    await harpiya.startTour("tour2", { mode: "auto" }); // Use harpiya to run tour from registry because this is a test tour
    await waitForMacro();
    const expectedError = [
...
test("a failing tour with disabled element", async () => {
    ...
    await harpiya.startTour("tour3", { mode: "auto" });
    await waitForMacro();
    const expectedError = [
...
test("a failing tour logs the step that failed", async () => {
    ...
    await harpiya.startTour("tour1", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
...
test("check tour with inactive steps", async () => {
    ...
    await harpiya.startTour("pipu_tour", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
...
test("automatic tour with invisible element", async () => {
    ...
    await harpiya.startTour("tour_de_wallonie", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
...
test("automatic tour with alternative trigger", async () => {
    ...
    await mountWithCleanup(Root);
    await harpiya.startTour("tour_des_flandres", { mode: "auto" });
    await waitForMacro();
...
test("check not possible to click below modal", async () => {
    ...
    await harpiya.startTour("tour_check_modal", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
...
test("a tour where hoot trigger failed", async () => {
    ...
    await harpiya.startTour("tour_hoot_failed", { mode: "auto" });
    await waitForMacro();
    expect.verifySteps([
