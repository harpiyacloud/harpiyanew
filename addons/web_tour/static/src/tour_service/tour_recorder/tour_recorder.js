...
import { queryAll, queryFirst, queryOne } from "@harpiya/hoot-dom";
import { Component, useState, useExternalListener } from "@harpiya/owl";
...
const HARPIYA_CLASS_REGEX = /^oe?(-|_)[\w-]+$/;
...
const getShortestSelector = (paths) => {
    paths.reverse();
    let filteredPath = [];
    let hasHarpiyaClass = false;
    for (
        let currentElem = paths.pop();
        (currentElem && queryAll(filteredPath.join(" > ")).length !== 1) || !hasHarpiyaClass;
        ...
        const harpiyaClass = [...currentElem.classList].find((c) => c.match(HARPIYA_CLASS_REGEX));
        if (harpiyaClass) {
            currentPredicate = `.${harpiyaClass}`;
            hasHarpiyaClass = true;
        }
