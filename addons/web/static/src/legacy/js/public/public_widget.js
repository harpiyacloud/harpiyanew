...
function HarpiyaEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
HarpiyaEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};
HarpiyaEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
const EventDispatcherMixin = Object.assign({}, ParentedMixin, {
     ...
     * Harpiya where developers may want to replace existing callbacks with theirs.
     *
    ...
    "trigger_up": function (name, info) {
        var event = new HarpiyaEvent(this, name, info);
