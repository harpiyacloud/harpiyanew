...
(function (harpiya) {
    ...
    class ModuleLoader {
        /** @type {HarpiyaModuleLoader["bus"]} */
        bus = new EventTarget();
        /** @type {HarpiyaModuleLoader["checkErrorProm"]} */
        checkErrorProm = null;
        /** @type {HarpiyaModuleLoader["factories"]} */
        factories = new Map();
        /** @type {HarpiyaModuleLoader["failed"]} */
        failed = new Set();
        /** @type {HarpiyaModuleLoader["jobs"]} */
        jobs = new Set();
        /** @type {HarpiyaModuleLoader["modules"]} */
        modules = new Map();
        ...
        /** @type {HarpiyaModuleLoader["addJob"]} */
        addJob(name) {
            this.jobs.add(name);
            this.startModules();
        }

        /** @type {HarpiyaModuleLoader["define"]} */
        define(name, deps, factory, lazy = false) {
            ...
            this.factories.set(name, {
                deps,
                fn: factory,
                ignoreMissingDeps: globalThis.__harpiyaIgnoreMissingDependencies,
            });
            if (!lazy) {
                this.addJob(name);
        ...
        /** @type {HarpiyaModuleLoader["findErrors"]} */
        ...
        /** @type {HarpiyaModuleLoader["findJob"]} */
        findJob() {
        ...
        /** @type {HarpiyaModuleLoader["reportErrors"]} */
        ...
        /** @type {HarpiyaModuleLoader["startModules"]} */
        startModules() {
            let job;
        ...
        /** @type {HarpiyaModuleLoader["startModule"]} */
        startModule(name) {
            /** @type {(dependency: string) => HarpiyaModule} */
            const require = (dependency) => this.modules.get(dependency);
            this.jobs.delete(name);
            const factory = this.factories.get(name);
            /** @type {HarpiyaModule | null} */
            let module = null;
            try {
    ...
    if (harpiya.debug && !new URLSearchParams(location.search).has("debug")) {
        // remove debug mode if not explicitely set in url
        harpiya.debug = "";
    }

    const loader = new ModuleLoader();
    harpiya.define = loader.define.bind(loader);
    harpiya.loader = loader;
})((globalThis.harpiya ||= {}));
