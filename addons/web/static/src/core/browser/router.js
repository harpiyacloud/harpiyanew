...
export function startUrl() {
    return isScopedApp() ? "scoped_app" : "harpiya";
}
 ...
 * external website). Unfortunately, Harpiya wasn't designed to be compatible with this cache, which
...
browser.addEventListener("click", (ev) => {
    ...
    if (href && !href.startsWith("#")) {
        ...
        if (
            ...
            browser.location.pathname.startsWith("/harpiya") &&
            (["/web", "/harpiya"].includes(url.pathname) || url.pathname.startsWith("/harpiya/")) &&
            ev.target.target !== "_blank"
        ) {
            ev.preventDefault();
            state = router.urlToState(url);
            if (url.pathname.startsWith("/harpiya") && url.hash) {
