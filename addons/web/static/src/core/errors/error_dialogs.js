...
import { Component, useRef, useState, markup } from "@harpiya/owl";
...
export const harpiyaExceptionTitleMap = new Map(
    Object.entries({
        "harpiya.addons.base.models.ir_mail_server.MailDeliveryException": _t("MailDeliveryException"),
        "harpiya.exceptions.AccessDenied": _t("Access Denied"),
        "harpiya.exceptions.MissingError": _t("Missing Record"),
        "harpiya.addons.web.controllers.action.MissingActionError": _t("Missing Action"),
        "harpiya.exceptions.UserError": _t("Invalid Operation"),
        "harpiya.exceptions.ValidationError": _t("Validation Error"),
        "harpiya.exceptions.AccessError": _t("Access Error"),
        "harpiya.exceptions.Warning": _t("Warning"),
    })
);
...
ClientErrorDialog.title = _t("Harpiya Client Error");

// -----------------------------------------------------------------------------
// Network Error Dialog
// -----------------------------------------------------------------------------
export class NetworkErrorDialog extends ErrorDialog {}
NetworkErrorDialog.title = _t("Harpiya Network Error");
...
export class RPCErrorDialog extends ErrorDialog {
    ...
    inferTitle() {
        ...
        if (this.props.exceptionName && harpiyaExceptionTitleMap.has(this.props.exceptionName)) {
            this.title = harpiyaExceptionTitleMap.get(this.props.exceptionName).toString();
            return;
        }
        ...
        switch (this.props.type) {
            case "server":
                this.title = _t("Harpiya Server Error");
                break;
            case "script":
                this.title = _t("Harpiya Client Error");
                break;
            case "network":
                this.title = _t("Harpiya Network Error");
                break;
        }
    }
...
export class WarningDialog extends Component {
    ...
    inferTitle() {
        if (this.props.exceptionName && harpiyaExceptionTitleMap.has(this.props.exceptionName)) {
            return harpiyaExceptionTitleMap.get(this.props.exceptionName).toString();
        }
        return this.props.title || _t("Harpiya Warning");
    }
}
...
export class RedirectWarningDialog extends Component {
    ...
    setup() {
        ...
        this.title = capitalize(subType) || _t("Harpiya Warning");
        this.message = message;
...
registry
    ...
    .add("harpiya.exceptions.AccessDenied", WarningDialog)
    .add("harpiya.exceptions.AccessError", WarningDialog)
    .add("harpiya.exceptions.MissingError", WarningDialog)
    .add("harpiya.addons.web.controllers.action.MissingActionError", WarningDialog)
    .add("harpiya.exceptions.UserError", WarningDialog)
    .add("harpiya.exceptions.ValidationError", WarningDialog)
    .add("harpiya.exceptions.RedirectWarning", RedirectWarningDialog)
    .add("harpiya.http.SessionExpiredException", SessionExpiredDialog)
