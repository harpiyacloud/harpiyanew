...
/**
 * @typedef {import("../../env").HarpiyaEnv} HarpiyaEnv
...
export function swallowAllVisitorErrors(env, error, originalError) {
    if (!user.isInternalUser && !harpiya.debug && !session.test_mode) {
        return true;
    }
}
...
if (user.isInternalUser === undefined) {
    ...
    // apparently not be present in all Harpiya screens at the moment... TODO ?
