...
export const menuService = {
    ...
    async start(env) {
        ...
        const fetchMenus = async (reload) => {
            if (!reload && harpiya.loadMenusPromise) {
                return harpiya.loadMenusPromise;
            }
        ...
        if (storedMenus && storedMenusVersion === harpiya.info.server_version) {
            fetchMenus().then((res) => {
                if (res) {
            ...
            if (menusData) {
                browser.localStorage.setItem("webclient_menus_version", harpiya.info.server_version);
