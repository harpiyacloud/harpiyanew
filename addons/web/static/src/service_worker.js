...
const cacheName = "harpiya-sw-cache";
const cachedRequests = ["/harpiya/offline"];
...
const navigateOrDisplayOfflinePage = async (request) => {
    try {
        ...
        if (
            ...
            if (cachedRequests.includes("/harpiya/offline")) {
                const cache = await caches.open(cacheName);
                const cachedResponse = await cache.match("/harpiya/offline");
                if (cachedResponse) {
...
const serveShareTarget = (event) => {
    ...
    event.respondWith(Response.redirect("/harpiya?share_target=trigger"));
    event.waitUntil(
        (async () => {
            // The page sends this message to tell the service worker it's ready to receive the file.
            await waitingMessage("harpiya_share_target");
            const client = await self.clients.get(event.resultingClientId || event.clientId);
            const data = await event.request.formData();
            client.postMessage({
                shared_files: data.getAll("externalMedia") || [],
                action: "harpiya_share_target_ack",
            });
        })()
    );
};
