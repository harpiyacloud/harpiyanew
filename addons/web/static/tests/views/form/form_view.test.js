import { after, before, expect, test } from "@harpiya/hoot";
import {
    clear,
    click,
    hover,
...
test(`form view is not broken if save operation fails`, async () => {
    ...
    await animationFrame();
    expect(`.o_dialog`).toHaveCount(1);
    expect.verifyErrors(["RPC_ERROR: Harpiya Server Error"]);
            ...
            if (obj.name === "first line") {
                if (onChangeCount === 0) {
                    ...
                    throw makeErrorFromResponse({
                        ...
                        data: {
                            name: `harpiya.exceptions.${"UserError"}`,
                            debug: "traceback",
                            arguments: [],
                            context: {},
                            message: "error",
