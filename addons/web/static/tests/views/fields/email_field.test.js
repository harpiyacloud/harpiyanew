import { expect, getFixture, test } from "@harpiya/hoot";
import {
    contains,
    defineModels,
    fieldInput,
...
import { queryAllTexts, queryFirst } from "@harpiya/hoot-dom";
...
test("in form view", async () => {
    Contact._records = [{ id: 1, email: "john.doe@harpiya.com" }];
    await mountView({
        type: "form",
        resModel: "contact",
        resId: 1,
    ...
    expect(`.o_field_email input[type="email"]`).toHaveValue("john.doe@harpiya.com");
    expect(`.o_field_email a`).toHaveCount(1);
    expect(`.o_field_email a`).toHaveAttribute("href", "mailto:john.doe@harpiya.com");
    expect(`.o_field_email a`).toHaveAttribute("target", "_blank");
    await fieldInput("email").edit("new@harpiya.com");
    expect(`.o_field_email input[type="email"]`).toHaveValue("new@harpiya.com");
});

test("in editable list view", async () => {
    Contact._records = [
        { id: 1, email: "john.doe@harpiya.com" },
        { id: 2, email: "jane.doe@harpiya.com" },
    ];
    await mountView({
        type: "list",
        resModel: "contact",
    ...
    expect(queryAllTexts(`tbody td:not(.o_list_record_selector) a`)).toEqual([
        "john.doe@harpiya.com",
        "jane.doe@harpiya.com",
    ]);
    expect(".o_field_email a:first").toHaveAttribute("href", "mailto:john.doe@harpiya.com");
    ...
    expect(".o_field_email a:first").toHaveAttribute("href", "mailto:new@harpiya.com");
});
