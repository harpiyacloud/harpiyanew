import { beforeEach, destroy, expect, test } from "@harpiya/hoot";
import { queryAll, queryAllAttributes, queryAllTexts, resize } from "@harpiya/hoot-dom";
import { advanceTime, animationFrame, runAllTimers } from "@harpiya/hoot-mock";
import {
    clearRegistry,
    contains,
    defineMenus,
...
import { Component, onRendered, xml } from "@harpiya/owl";
...
test("href attribute on apps menu items", async () => {
    ...
    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveAttribute("href", "/harpiya/action-339");
});
...
test("href attribute with path on apps menu items", async () => {
    ...
    await mountWithCleanup(NavBar);
    await contains(".o_navbar_apps_menu button.dropdown-toggle").click();
    expect(".o-dropdown--menu .dropdown-item").toHaveAttribute("href", "/harpiya/my-path");
});
