...
import { describe, test, expect } from "@harpiya/hoot";
...
import { click, freezeTime, queryAllTexts } from "@harpiya/hoot-dom";
import {
    ClientErrorDialog,
    Error504Dialog,
    ErrorDialog,
...
test("WarningDialog", async () => {
    ...
    await mountWithCleanup(WarningDialog, {
        env,
        props: {
            exceptionName: "harpiya.exceptions.UserError",
            message: "...",
...
test("RedirectWarningDialog", async () => {
    ...
    expect(".o_dialog").toHaveCount(1);
    expect("header .modal-title").toHaveText("Harpiya Warning");
    expect("main").toHaveText("Some strange unreadable message");
...
test("SessionExpiredDialog", async () => {
    ...
    expect(".o_dialog").toHaveCount(1);
    expect(".o_dialog").toHaveCount(1);
    expect("header .modal-title").toHaveText("Harpiya Session Expired");
    expect("main p").toHaveText(
        "Your Harpiya session expired. The current page is about to be refreshed."
    );
    expect(".o_dialog footer button").toHaveText("Close");
