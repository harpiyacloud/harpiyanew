...
class WebManifestRoutesTest(HttpCaseWithUserDemo):
    ...
    def test_webmanifest(self):
        ...
        data = response.json()
        self.assertEqual(data["name"], "Harpiya")
        self.assertEqual(data["scope"], "/harpiya")
        self.assertEqual(data["start_url"], "/harpiya")
        self.assertEqual(data["display"], "standalone")
        self.assertEqual(data["background_color"], "#003563")
        self.assertEqual(data["theme_color"], "#003563")
        self.assertEqual(data["prefer_related_applications"], False)
        self.assertCountEqual(data["icons"], [
            {'src': '/web/static/img/harpiya-icon-192x192.png', 'sizes': '192x192', 'type': 'image/png'},
            {'src': '/web/static/img/harpiya-icon-512x512.png', 'sizes': '512x512', 'type': 'image/png'}
        ])
        ...
        for shortcut in data["shortcuts"]:
            ...
            self.assertTrue(shortcut["url"].startswith("/harpiya?menu_id="))
    ...
    def test_webmanifest_scoped(self):
        ...
        self.assertEqual(data["prefer_related_applications"], False)
        self.assertCountEqual(data["icons"], [
            {'src': "/web/static/img/harpiya-icon-192x192.png", 'sizes': 'any', 'type': 'image/png'}
        ])
        self.assertEqual(len(data["shortcuts"]), 0)
    ...
    def test_serviceworker(self):
        ...
        self.assertEqual(response.headers["Service-Worker-Allowed"], "/harpiya")

    def test_offline_url(self):
        """
        This route returns the offline page
        """
        response = self.url_open("/harpiya/offline")
        self.assertEqual(response.status_code, 200)
    ...
    def test_apple_touch_icon(self):
        ...
        response = self.url_open("/web/static/img/harpiya-icon-ios.png")
        self.assertEqual(response.status_code, 200)

        document = self.url_open("/harpiya")
        self.assertIn(
            '<link rel="apple-touch-icon" href="/web/static/img/harpiya-icon-ios.png"/>', document.text,
