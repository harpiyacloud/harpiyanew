    ...
    def test_01_click_everywhere_as_admin(self):
        ...
        for app_id in menus['root']['children']:
            with self.subTest(app=menus[app_id]['name']):
                ...
                self.browser_js("/harpiya", "harpiya.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere('%s');" % menus[app_id]['xmlid'], "harpiya.isReady === true", login="admin", timeout=1200, success_signal="clickbot test succeeded")
...
class TestMenusDemo(HttpCaseWithUserDemo):
    ...
    def test_01_click_everywhere_as_demo(self):
        ...
        for app_id in menus['root']['children']:
            with self.subTest(app=menus[app_id]['name']):
                _logger.runbot('Testing %s', menus[app_id]['name'])
                self.browser_js("/harpiya", "harpiya.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere('%s');" % menus[app_id]['xmlid'], "harpiya.isReady === true", login="demo", timeout=1200, success_signal="clickbot test succeeded")

@harpiya.tests.tagged('post_install', '-at_install')
class TestMenusAdminLight(harpiya.tests.HttpCase):
    allow_end_on_form = True

    @classmethod
    def _request_handler(cls, s: Session, r: PreparedRequest, /, **kw):
        # mock harpiyafin requests
    ...
    def test_01_click_apps_menus_as_admin(self):
        ...
        self.browser_js("/harpiya", "harpiya.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere(undefined, true);", "harpiya.isReady === true", login="admin", timeout=120, success_signal="clickbot test succeeded")
...
class TestMenusDemoLight(HttpCaseWithUserDemo):
    ...
    def test_01_click_apps_menus_as_demo(self):
        ...
        self.browser_js("/harpiya", "harpiya.loader.modules.get('@web/webclient/clickbot/clickbot_loader').startClickEverywhere(undefined, true);", "harpiya.isReady === true", login="demo", timeout=120, success_signal="clickbot test succeeded")
