...
class Home(http.Controller):
    ...
    def index(self, s_action=None, db=None, **kw):
        ...
        return request.redirect_query('/harpiya', query=request.params)

    def _web_client_readonly(self):
        return False

    # ideally, this route should be `auth="user"` but that don't work in non-monodb mode.
    @http.route(['/web', '/harpiya', '/harpiya/<path:subpath>', '/scoped_app/<path:subpath>'], type='http', auth="none", readonly=_web_client_readonly)
    ...
    def switch_to_admin(self):
        ...
        if request.env.user._is_system():
            uid = request.session.uid = harpiya.SUPERUSER_ID
    ...
    def health(self, db_server_status=False):
        ...
        if db_server_status:
            try:
                harpiya.sql_db.db_connect('postgres').cursor().close()
