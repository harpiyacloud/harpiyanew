...
try:
    ...
    from harpiya.tools._vendor.send_file import send_file

import harpiya
import harpiya.modules.registry
from harpiya import SUPERUSER_ID, _, http, api
from harpiya.addons.base.models.assetsbundle import ANY_UNIQUE
from harpiya.exceptions import AccessError, UserError
from harpiya.http import request, Response
from harpiya.tools import file_open, file_path, replace_exceptions, str2bool
from harpiya.tools.image import image_guess_size_from_field_name
from harpiya.tools.mimetypes import guess_mimetype

_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
Harpiya is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however Harpiya is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if harpiya.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=harpiya.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or harpiya.SUPERUSER_ID
        ...
        if not dbname:
            ...
            try:
                ...
                _logger.warning("While retrieving the company logo, using the Harpiya logo instead", exc_info=True)
