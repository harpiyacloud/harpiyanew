...
class GoogleEvent(abc.Set):
    ...
    Inspired by Harpiya recordset, one instance can be a single Google event or a
    ...
    def harpiya_id(self, env):
        self.harpiya_ids(env)  # load ids
        return self._harpiya_id

    def _meta_harpiya_id(self, dbname):
        """Returns the Harpiya id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_harpiya_id' % dbname)
        if o_id:
            return int(o_id)

    def harpiya_ids(self, env):
        ids = tuple(e._harpiya_id for e in self if e._harpiya_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_harpiya_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_harpiya_ids_from_metadata(env, model)

        return tuple(e._harpiya_id for e in self)

    def _load_harpiya_ids_from_metadata(self, env, model):
        unsure_harpiya_ids = tuple(e._meta_harpiya_id(env.cr.dbname) for e in self)
        harpiya_events = model.browse(_id for _id in unsure_harpiya_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same Harpiya id.
        # Therefore, we only consider Harpiya records without google id when trying
        # to match events.
        o_ids = harpiya_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            harpiya_id = e._meta_harpiya_id(env.cr.dbname)
            if harpiya_id in o_ids:
                e._events[e.id]['_harpiya_id'] = harpiya_id

    def _load_harpiya_ids_from_db(self, env, model):
        harpiya_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in harpiya_events}  # {google_id: harpiya_id}
        existing_google_ids = harpiya_events.mapped('google_id')
        for e in self:
            harpiya_id = mapping.get(e.id)
            if harpiya_id:
                e._events[e.id]['_harpiya_id'] = harpiya_id
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Harpiya.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in Harpiya (they are the owner) but userB syncs first.
        ...
        # (they are the organizer in Google). The "real" owner (in Harpiya) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Harpiya several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_harpiya_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.harpiya_ids(env)
        events.harpiya_ids(env)

        return self.filter(lambda e: e._harpiya_id)
    ...
    def get_harpiya_event(self, env):
        if self._get_model(env)._name == 'calendar.event':
            return env['calendar.event'].browse(self.harpiya_id(self.env))
        else:
            return env['calendar.recurrence'].browse(self.harpiya_id(self.env)).base_event_id
