...
class CalendarAttendee(models.Model):
    ...
    def _sync_event(self):
        ...
        for user in other_events.mapped('user_id'):
            ...
            other_events.filtered(lambda ev: ev.user_id.id == user.id).with_user(user)._sync_harpiya2google(service)
        google_service = GoogleCalendarService(self.env['google.service'])
        (all_events - other_events)._sync_harpiya2google(google_service)
