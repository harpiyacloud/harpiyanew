...
from harpiya.tools import ormcache_context, email_normalize
from harpiya.osv import expression
from harpiya.sql_db import BaseCursor

from harpiya.addons.google_calendar.utils.google_event import GoogleEvent
from harpiya.addons.google_calendar.utils.google_calendar import GoogleCalendarService
from harpiya.addons.google_account.models.google_service import TIMEOUT

_logger = logging.getLogger(__name__)


# API requests are sent to Google Calendar after the current transaction ends.
# This ensures changes are sent to Google only if they really happened in the Harpiya database.
# It is particularly important for event creation , otherwise the event might be created
# twice in Google if the first creation crashed in Harpiya.
...
class GoogleCalendarSync(models.AbstractModel):
    ...
    def unlink(self):
        ...
        if self.env.context.get('archive_on_error') and self._active_name:
            ...
            # Then, since it has been deleted on Google, the event is also deleted on Harpiya DB (_sync_google2harpiya).
            self.action_archive()
            return True
        return super().unlink()
    ...
    def _sync_harpiya2google(self, google_service: GoogleCalendarService):
        if not self:
            return
    ...
    @api.model
    def _sync_google2harpiya(self, google_events: GoogleEvent, write_dates=None, default_reminders=()):
        """Synchronize Google recurrences in Harpiya. Creates new recurrences, updates
        existing ones.

        :param google_recurrences: Google recurrences to synchronize in Harpiya
        :param write_dates: A dictionary mapping Harpiya record IDs to their write dates.
        :return: synchronized harpiya recurrences
        """
        ...
        harpiya_values = [
            dict(self._harpiya_values(e, default_reminders), need_sync=False)
            for e in new
        ]
        new_harpiya = self.with_context(dont_notify=True)._create_from_google(new, harpiya_values)
        cancelled = existing.cancelled()
        cancelled_harpiya = self.browse(cancelled.harpiya_ids(self.env))

        # Check if it is a recurring event that has been rescheduled.
        # We have to check if an event already exists in Harpiya.
        # Explanation:
        ...
        if rescheduled_events:
            ...
            cancelled_harpiya += self.env['calendar.event'].search([('google_id', 'in', google_ids_to_remove)])

        cancelled_harpiya.exists()._cancel()
        synced_records = new_harpiya + cancelled_harpiya
        pending = existing - cancelled
        pending_harpiya = self.browse(pending.harpiya_ids(self.env)).exists()
        for gevent in pending:
            harpiya_record = self.browse(gevent.harpiya_id(self.env))
            if harpiya_record not in pending_harpiya:
                # The record must have been deleted in the mean time; nothing left to sync
                continue
            # Last updated wins.
            # This could be dangerous if google server time and harpiya server time are different
            updated = parse(gevent.updated)
            # Use the record's write_date to apply Google updates only if they are newer than Harpiya's write_date.
            harpiya_record_write_date = write_dates.get(harpiya_record.id, harpiya_record.write_date)
            # Migration from 13.4 does not fill write_date. Therefore, we force the update from Google.
            if not harpiya_record_write_date or updated >= pytz.utc.localize(harpiya_record_write_date):
                vals = dict(self._harpiya_values(gevent, default_reminders), need_sync=False)
                harpiya_record.with_context(dont_notify=True)._write_from_google(gevent, vals)
                synced_records |= harpiya_record
    ...
    def _get_records_to_sync(self, full_sync=False):
        """Return records that should be synced from Harpiya to Google
    ...
    def _check_any_records_to_sync(self):
        """ Returns True if there are pending records to be synchronized from Harpiya to Google, False otherwise. """
        is_active_clause = (self._active_name, '=', True) if self._active_name else expression.TRUE_LEAF
    ...
    @api.model
    def _harpiya_values(self, google_event: GoogleEvent, default_reminders=()):
