...
import { getRangePosition } from '@web_editor/js/editor/harpiya-editor/src/utils/utils';
import { utils as uiUtils } from "@web/core/ui/ui_service";
import { closestScrollableY } from "@web/core/utils/scrolling";
import { useThrottleForAnimation } from "@web/core/utils/timing";
import { onWillUnmount, onWillStart, reactive, status, useSubEnv } from "@harpiya/owl";
...
export class MassMailingHtmlField extends HtmlField {
    ...
    async commitChanges(...args) {
        ...
        this._pendingCommitChanges = (async () => {
            ...
            this.wysiwyg.harpiyaEditor.historyPauseSteps();
            ...
            this.wysiwyg.harpiyaEditor.observerUnactive('toInline');
            ...
            iframe.remove();
            this.wysiwyg.harpiyaEditor.observerActive('toInline');
            const inlineHtml = editableClone.innerHTML;
            $editorEnable.addClass('editor_enable');
            this.wysiwyg.harpiyaEditor.historyUnpauseSteps();
            this.wysiwyg.harpiyaEditor.historyRevertCurrentStep();
    ...
    async _resetIframe() {
        ...
        this.wysiwyg.$iframeBody.find('.harpiya-editor-editable').removeClass('o_editable');

        initializeDesignTabCss(this.wysiwyg.getEditable());
        this.wysiwyg.getEditable().find('img').attr('loading', '');

        this.wysiwyg.harpiyaEditor.observerFlush();
        this.wysiwyg.harpiyaEditor.historyReset();
    ...
    async _onSnippetsLoaded() {
        ...
        $themeSelectorNew.on('click', '.dropdown-item', async (e) => {
            ...
            // the Harpiya editor before resetting the history.
            setTimeout(() => {
                this.wysiwyg.historyReset();
                // Update undo/redo buttons
                this.wysiwyg.harpiyaEditor.dispatchEvent(new Event('historyStep'));

                // The selection has been lost when switching theme.
                const document = this.wysiwyg.harpiyaEditor.document;
                const selection = document.getSelection();
                const p = this.wysiwyg.harpiyaEditor.editable.querySelector('p');
                if (p && selection) {
        ...
        if (editableAreaIsEmpty) {
            ...
            // We actually hide the HarpiyaEditor toolbar by calling
        ...
        this.wysiwyg.harpiyaEditor.activateContenteditable();
    }
    _getCodeViewEl() {
    ...
    async _switchThemes(themeParams) {
        ...
        this.wysiwyg.harpiyaEditor.resetContent($newLayout[0].outerHTML);
        ...
        this.onIframeUpdated();
        this.wysiwyg.harpiyaEditor.historyStep(true);
    ...
    async _lazyloadWysiwyg() {
        ...
        const wysiwygModule = await harpiya.loader.modules.get('@mass_mailing/js/mass_mailing_wysiwyg');
