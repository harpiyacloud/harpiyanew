...
# Harpiya
harpiya_url = 'http://localhost:8069'
harpiya_db = 'harpiya_db'
harpiya_username = 'admin'
harpiya_password = 'admin'
...
def get_blobs_to_be_deleted(blob_urls, batch_size=1000):
    common = xmlrpc.client.ServerProxy(f'{harpiya_url}/xmlrpc/2/common')
    uid = common.authenticate(harpiya_db, harpiya_username, harpiya_password, {})
    models = xmlrpc.client.ServerProxy(f'{harpiya_url}/xmlrpc/2/object')
    for blob_urls_ in split_every(batch_size, blob_urls):
        blob_urls_ = list(blob_urls_)
        attachments = models.execute_kw(harpiya_db, uid, harpiya_password, 'ir.attachment', 'search_read', [
