...
from harpiya.addons.mail.models.mail_mail import _UNFOLLOW_REGEX
...
class RecipientsNotificationTest(MailCommon):
    ...
    def test_subscribe_post_author(self):
        """ Test author is added in followers, unless it is archived / harpiyabot """
        # some automated action post on behalf of author
        test_record = self.env['mail.test.simple'].create({'name': 'Test'})
        self.partner_root.active = True  # edge case, people activating Harpiyabot partner (not user)
        ...
        for user, author, exp_followers in [
            ...
            (self.user_2, self.user_root.partner_id, self.env['res.partner']),  # no harpiyabot !
        ]:
...
class UnfollowLinkTest(MailCommon, HttpCase):
    ...
    @mute_logger('harpiya.addons.base.models', 'harpiya.addons.mail.controllers.mail', 'harpiya.http', 'harpiya.models')
