...
from harpiya.addons.l10n_in_ewaybill.tools.ewaybill_api import EWayBillApi, EWayBillError
...
class L10nInEwaybill(models.Model):
    ...
    def _handle_internal_warning_if_present(self, response):
        if warnings := response.get('harpiya_warning'):
    ...
    def _generate_ewaybill(self):
        ...
        self._write_successfully_response({
            'name': name,
            'state': 'generated',
            'ewaybill_date': self._indian_timezone_to_harpiya_utc(
                response_data['ewayBillDate']
            ),
            'ewaybill_expiry_date': self._indian_timezone_to_harpiya_utc(
    ...
    @api.model
    def _indian_timezone_to_harpiya_utc(self, str_date, time_format='%d/%m/%Y %I:%M:%S %p'):
        """
    ...
    def _set_data_from_attachment(self):
        ...
        see https://github.com/harpiya/upgrade/pull/6624 for futher information
        """
        self.ensure_one()
        if self.attachment_id:
            ...
            if not ewb_name:
                return False
            ewb_date = self._indian_timezone_to_harpiya_utc(res_json.get("ewayBillDate") or res_json.get("EwbDt"))
            ewb_validity = self._indian_timezone_to_harpiya_utc(res_json.get("validUpto") or res_json.get("EwbValidTill"))
            self.write({
                "name": ewb_name,
