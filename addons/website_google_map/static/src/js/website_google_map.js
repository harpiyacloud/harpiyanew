...
function initialize_map() {
    ...
    var map = new google.maps.Map(document.getElementById('harpiya-google-map'), {
        zoom: 1,
    ...
    async function initializeMarkers(harpiyaPartnerData) {
        // Create the markers and cluster them on the map
        if (harpiyaPartnerData) { /* harpiya_partner_data special variable should have been defined in google_map.xml */
            const markerPromises = [];
            for (var i = 0; i < harpiya_partner_data.counter; i++) {
                const prom = set_marker(harpiya_partner_data.partners[i])
    ...
    initializeMarkers(harpiya_partner_data);
