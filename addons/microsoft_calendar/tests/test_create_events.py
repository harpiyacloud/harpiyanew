...
from harpiya.addons.mail.tests.common import mail_new_test_user
...
from harpiya.addons.microsoft_calendar.models.res_users import ResUsers
from harpiya.addons.microsoft_calendar.tests.common import TestCommon, mock_get_token, _modified_date_in_the_future
...
class TestCreateEvents(TestCommon):
    ...
    def test_create_simple_event_without_sync(self, mock_insert):
        """
        A Harpiya event is created when Outlook sync is not enabled.
        """
    ...
    def test_create_simple_event_without_email(self):
        ...
        with self.assertRaises(ValidationError):
            record._sync_harpiya2microsoft()

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_organizer_calendar(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Harpiya organizer calendar.
        """
        ...
        self.assert_harpiya_event(new_records, self.expected_harpiya_event_from_outlook)
        self.assertEqual(new_records.user_id, self.organizer_user)
        self.assertEqual(new_records.need_sync_m, False)

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_exists_in_harpiya(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Harpiya attendee calendar.
        There is a Harpiya user that matches with the organizer email address.
        """
    ...
    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_and_organizer_does_not_exist_in_harpiya(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Harpiya attendee calendar.
        no Harpiya user that matches with the organizer email address.
        """
        ...
        outlook_event = dict(self.simple_event_from_outlook_attendee, organizer={
            'emailAddress': {'address': "john.doe@harpiya.com", 'name': "John Doe"},
        })
        expected_event = dict(self.expected_harpiya_event_from_outlook, user_id=False)
        ...
        new_records = (records - existing_records)
        self.assertEqual(len(new_records), 1)
        self.assert_harpiya_event(new_records, expected_event)

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_simple_event_from_outlook_attendee_calendar_where_email_addresses_are_capitalized(self, mock_get_events):
        """
        An event has been created in Outlook and synced in the Harpiya attendee calendar.
        The email addresses of the attendee and the organizer are in different case than in Harpiya.
        """
    ...
    def test_create_recurrent_event_without_sync(self, mock_insert):
        """
        A Harpiya recurrent event is created when Outlook sync is not enabled.
        """
        if not self.sync_harpiya_recurrences_with_outlook_feature():
            return
    ...
    def test_create_recurrent_event_with_sync(self, mock_insert, mock_get_events):
        """
        A Harpiya recurrent event is created when Outlook sync is enabled.
        """
        if not self.sync_harpiya_recurrences_with_outlook_feature():
            return
    ...
    def test_create_recurrent_event_with_sync_by_another_user(self, mock_insert, mock_get_events):
        """
        A Harpiya recurrent event has been created and synced with Outlook by another user, but nothing
    ...
    def test_create_recurrent_event_from_outlook_organizer_calendar(self, mock_get_events):
        """
        A recurrent event has been created in Outlook and synced in the Harpiya organizer calendar.
        """
        ...
        self.assert_harpiya_recurrence(new_recurrences, self.expected_harpiya_recurrency_from_outlook)
        for i, e in enumerate(sorted(new_events, key=lambda e: e.id)):
            self.assert_harpiya_event(e, self.expected_harpiya_recurrency_events_from_outlook[i])

    @patch.object(MicrosoftCalendarService, 'get_events')
    def test_create_recurrent_event_from_outlook_attendee_calendar(self, mock_get_events):
        """
        A recurrent event has been created in Outlook and synced in the Harpiya attendee calendar.
        """
    ...
    def test_forbid_recurrences_creation_synced_outlook_calendar(self, mock_insert):
        """
        Forbids new recurrences creation in Harpiya due to Outlook spam limitation of updating recurrent events.
        """
    ...
    def test_create_event_with_sync_config_paused(self, mock_insert):
        ...
        # Try to create a simple event in Harpiya Calendar.
        record = self.env["calendar.event"].with_user(self.organizer_user).create(self.simple_event_values)
        self.call_post_commit_hooks()
        record.invalidate_recordset()
    ...
    def test_create_event_for_another_user(self, mock_insert, mock_get_events):
        """
        Allow the creation of event for another user only if the proposed user have its Harpiya Calendar synced.
        ...
        # In Harpiya, this event will be created (behind the screens) by a synced Harpiya user as attendee (self.attendee_user).
        ...
        # Mock event from Microsoft and sync event with Harpiya through self.attendee_user (synced user).
        ...
        # Ensure that event was successfully created in Harpiya (no ACL error was triggered blocking creation).
    ...
    def test_create_simple_event_from_outlook_without_organizer(self, mock_get_events):
        """
        Allow creation of an event without organizer in Outlook and sync it in Harpiya.
        """
    ...
    def test_create_event_with_default_and_undefined_sensitivity(self):
        """ Check if microsoft events are created in Harpiya when 'None' sensitivity setting is defined and also when it is not. """
        # Sync events from Microsoft to Harpiya after adding the sensitivity (privacy) property.
        ...
        self.env['calendar.event']._sync_microsoft2harpiya(MicrosoftEvent([undefined_privacy_event, default_privacy_event]))

        # Ensure that synced events have the correct privacy field in Harpiya.
        undefined_privacy_harpiya_event = self.env['calendar.event'].search([('microsoft_id', '=', 100)])
        default_privacy_harpiya_event = self.env['calendar.event'].search([('microsoft_id', '=', 200)])
        self.assertFalse(undefined_privacy_harpiya_event.privacy, "Event with undefined privacy must have False value in privacy field.")
        self.assertFalse(default_privacy_harpiya_event.privacy, "Event with custom privacy must have False value in privacy field.")

    @patch.object(MicrosoftCalendarService, 'get_events')
    @patch.object(MicrosoftCalendarService, 'insert')
    def test_create_videocall_sync_microsoft_calendar(self, mock_insert, mock_get_events):
        """
        Test syncing an event from Harpiya to Microsoft Calendar.
        ...
        # Verify the event's videocall_location is updated in Harpiya
    ...
    def test_skip_sync_for_non_synchronized_users_new_events(self, mock_insert):
        ...
        Harpiya, as well cause problems in the future the synchronization of that event for the original owner.
        """
        ...
        self.assertTrue(event, "The event for the not synchronized owner must be created in Harpiya.")
        # Synchronize the calendar of user A, then make sure insert was not called.
        event.with_user(self.organizer_user).sudo()._sync_harpiya2microsoft()
        mock_insert.assert_not_called()
