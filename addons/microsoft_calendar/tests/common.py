...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        # simple event values to create a Harpiya event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Harpiya events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to Harpiya events
        # -----------------------------------------------------------------------------------------

        self.expected_harpiya_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_harpiya_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_harpiya_recurrency_events_from_outlook = [
            {
    ...
    def sync_harpiya_recurrences_with_outlook_feature(self):
        """
    ...
    def create_events_for_tests(self):
        ...
        # Currently, it is forbidden to create recurrences in Harpiya. A trick for deactivating the checking
    ...
    def assert_harpiya_event(self, harpiya_event, expected_values):
        """
        Assert that an Harpiya event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        harpiya_event_values = harpiya_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(harpiya_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, harpiya_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_harpiya_recurrence(self, harpiya_recurrence, expected_values):
        """
        Assert that an Harpiya recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        harpiya_recurrence_values = harpiya_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, harpiya_recurrence_values.get(k), msg=f"'{k}' mismatch")
