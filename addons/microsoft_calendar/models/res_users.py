...
import logging
import requests
from harpiya.addons.microsoft_calendar.models.microsoft_sync import microsoft_calendar_token
from datetime import datetime, timedelta

from harpiya import api, fields, models, _, Command
from harpiya.exceptions import UserError
from harpiya.loglevels import exception_to_unicode
from harpiya.addons.microsoft_account.models import microsoft_service
from harpiya.addons.microsoft_calendar.utils.microsoft_calendar import InvalidSyncToken
...
class ResUsers(models.Model):
    ...
    def _sync_microsoft_calendar(self):
        ...
        # Microsoft -> Harpiya
        synced_events, synced_recurrences = self.env['calendar.event']._sync_microsoft2harpiya(events) if events else (self.env['calendar.event'], self.env['calendar.recurrence'])

        # Harpiya -> Microsoft
        recurrences = self.env['calendar.recurrence']._get_microsoft_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_harpiya2microsoft()
        ...
        (events - synced_events)._sync_harpiya2microsoft()
        self.sudo().microsoft_last_sync_date = datetime.now()
