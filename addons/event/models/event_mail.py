...
from harpiya.tools import exception_to_unicode
...
class EventMail(models.Model):
    ...
    configuration allowing to send emails on events since Harpiya 9. A cron exists
    ...
    def _warn_error(self, exception):
        ...
        if not last_error_dt or last_error_dt < now - relativedelta(hours=1):
            ...
            template_link = Markup('<a href="%s">%s (%s)</a>') % (
                f"{self.get_base_url()}/harpiya/{template._name}/{template.id}",
