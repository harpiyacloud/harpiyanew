...
class InsufficientCreditDialog extends Component {
    ...
    async onWillStart() {
        ...
        const { isEnterprise } = harpiya.info;
        if (errorData.trial && isEnterprise) {
            this.buttonMessage = _t("Start a Trial at Harpiya");
        } else {
...
function insufficientCreditHandler(env, error, originalError) {
    ...
    if (data && data.name === "harpiya.addons.iap.tools.iap_tools.InsufficientCreditError") {
