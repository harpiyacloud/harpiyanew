...
from harpiya.exceptions import AccessError, LockError, UserError
from harpiya.tools import float_is_zero, float_compare

from harpiya.addons.l10n_in.models.account_invoice import EDI_CANCEL_REASON
...
class AccountMove(models.Model):
    ...
    def _l10n_in_edi_send_invoice(self):
        ...
        if error := response.get('error', {}):
            harpiyabot_id = self.env.ref('base.partner_root').id
    ...
    def _l10n_in_edi_cancel_invoice(self):
        ...
        # Creating a lambda function so it fetches the harpiyabot id only when needed
        _get_harpiyabot_id = (
        ...
        if error := response.get('error'):
            ...
            if '9999' in error_codes:
                ...
                self.message_post(
                    author_id=_get_harpiyabot_id(self),
                    body=_(
        ...
        if "error" not in response:
            ...
            self.message_post(author_id=_get_harpiyabot_id(self), body=_(
    ...
    def _l10n_in_edi_connect_to_server(self, url_end_point, json_payload=False, params=False):
        ...
        if (error := response.get('error')) and '1005' in [e.get("code") for e in error]:
            ...
            # This happen when authenticate called from another harpiya instance with same credentials (like. Demo/Test)
