...
import {
    ...
    getRangePosition
} from '@web_editor/js/editor/harpiya-editor/src/utils/utils';
...
export class HtmlField extends Component {
    ...
    _filterPowerBoxCommands(commands) {
        ...
        if (this.wysiwyg && this.wysiwyg.harpiyaEditor) {
            const selection = this.wysiwyg.harpiyaEditor.document.getSelection();
    ...
    async startWysiwyg(wysiwyg) {
        ...
        wysiwyg.$editable[0].classList.add("harpiya-editor-qweb");
        ...
        this.wysiwyg.harpiyaEditor.addEventListener("historyStep", () =>
            this.props.record.model.bus.trigger("FIELD_IS_DIRTY", this._isDirty())
        );

        if (this.props.isCollaborative) {
            this.wysiwyg.harpiyaEditor.addEventListener("onExternalHistorySteps", () =>
                this.props.record.model.bus.trigger("FIELD_IS_DIRTY", this._isDirty())
            );
        }
    ...
    toggleCodeView() {
        ...
        if (this.wysiwyg) {
            this.wysiwyg.harpiyaEditor.observerUnactive('toggleCodeView');
            if (this.state.showCodeView) {
                this.wysiwyg.$editable.remove();
                this.wysiwyg.harpiyaEditor.toolbarHide();
                const value = this.wysiwyg.getValue();
                this.props.record.update({ [this.props.name]: value });
            } else {
                this.wysiwyg.harpiyaEditor.observerActive('toggleCodeView');
            }
        }
        if (!this.state.showCodeView) {
    ...
    onDynamicPlaceholderValidate(chain, defaultValue) {
        if (chain) {
            ...
            this.wysiwyg.harpiyaEditor.execCommand('insert', t);
            // Ensure the dynamic placeholder <t> element is sanitized.
            this.wysiwyg.harpiyaEditor.sanitize(t);
        }
    }
    ...
    async commitChanges({ urgent, shouldInline } = {}) {
        if (this._isDirty() || urgent || (shouldInline && this.props.isInlineStyle)) {
            ...
            if (this.wysiwyg && this.wysiwyg.harpiyaEditor) {
                this.wysiwyg.harpiyaEditor.observerUnactive('commitChanges');
                ...
                this.wysiwyg.harpiyaEditor.observerActive('commitChanges');
            }
    ...
    async _lazyloadWysiwyg() {
        ...
        let wysiwygModule = await harpiya.loader.modules.get('@web_editor/js/wysiwyg/wysiwyg');
        this.MoveNodePlugin = (await harpiya.loader.modules.get('@web_editor/js/wysiwyg/MoveNodePlugin'))?.MoveNodePlugin;
        // Otherwise, load the module.
        if (!wysiwygModule) {
            await ensureJQuery();
            await loadBundle('web_editor.backend_assets_wysiwyg');
            wysiwygModule = await harpiya.loader.modules.get('@web_editor/js/wysiwyg/wysiwyg');
            this.MoveNodePlugin = (await harpiya.loader.modules.get('@web_editor/js/wysiwyg/MoveNodePlugin')).MoveNodePlugin;
        }
    ...
    async _toInline() {
        ...
        this.wysiwyg.harpiyaEditor.sanitize(this.wysiwyg.harpiyaEditor.editable);
        const html = this.wysiwyg.getValue();
        const $harpiyaEditor = $editable.closest('.harpiya-editor-editable');
        // Save correct nodes references.
        // Remove temporarily the class so that css editing will not be converted.
        $harpiyaEditor.removeClass('harpiya-editor-editable');
        $editable.html(html);
        await toInline($editable, { $iframe: this.wysiwyg.$iframe, wysiwyg:this.wysiwyg });
        $harpiyaEditor.addClass('harpiya-editor-editable');
...
export const htmlField = {
    ...
    extractProps({ attrs, options }, dynamicInfo) {
        ...
        return {
            codeview: Boolean(harpiya.debug && options.codeview),
            placeholder: attrs.placeholder,
