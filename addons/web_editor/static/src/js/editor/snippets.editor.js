...
import { closestElement, isUnremovable } from "@web_editor/js/editor/harpiya-editor/src/utils/utils";
...
import { Tooltip as HarpiyaTooltip } from "@web/core/tooltip/tooltip";
...
var SnippetEditor = publicWidget.Widget.extend({
    ...
    clone: async function (recordUndo) {
        ...
        if (recordUndo) {
            this.options.wysiwyg.harpiyaEditor.historyStep(true);
        }
        await new Promise(resolve => {
            this.trigger_up('call_for_each_child_snippet', {
                $snippet: $clone,
    ...
    _outPreviousDropzone(self, currentDropzoneEl) {
        ...
        if (rowEl.classList.contains('o_grid_mode')) {
            ...
            self.options.wysiwyg.harpiyaEditor.observerActive('dragAndDropMoveSnippet');
            gridUtils._resizeGrid(rowEl);
            self.options.wysiwyg.harpiyaEditor.observerUnactive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStart({ helper, addStyle }) {
        this.options.wysiwyg.harpiyaEditor.observerUnactive('dragAndDropMoveSnippet');
        this.trigger_up('drag_and_drop_start');
        this.options.wysiwyg.harpiyaEditor.automaticStepUnactive();
        var self = this;
        this.dragState = {};
        ...
        if (rowEl.classList.contains('row') && this.options.isWebsite) {
            if (allowGridMode) {
                ...
                if (!rowEl.classList.contains('o_grid_mode')) {
                    this.options.wysiwyg.harpiyaEditor.observerActive('dragAndDropMoveSnippet');
    ...
    _onDragAndDropStop({ x, y }) {
        this.options.wysiwyg.harpiyaEditor.automaticStepActive();
        this.options.wysiwyg.harpiyaEditor.automaticStepSkipStack();
        this.options.wysiwyg.harpiyaEditor.unbreakableStepUnactive();
...
class SnippetsMenu extends Component {
    ...
    setup() {
        ...
        // Harpiya Editor uses the HTML Element to bind commands.
        ...
        onMounted(async () => {
            ...
            this.options.wysiwyg.harpiyaEditor.bindExecCommand(titleButtons);

            // Get table container and bind commands to Harpiya Editor.
            const customizeTableBlock = this.customizePanel.querySelector('#o-we-editor-table-container');
            this.options.wysiwyg.harpiyaEditor.bindExecCommand(customizeTableBlock);
    ...
    async start() {
        ...
        $(this.options.wysiwyg.harpiyaEditor.document.body).on('click', this._checkEditorToolbarVisibilityCallback);
        ...
        this.options.wysiwyg.harpiyaEditor.addEventListener('historyUndo', refreshSnippetEditors);
        this.options.wysiwyg.harpiyaEditor.addEventListener('historyRedo', refreshSnippetEditors);
        ...
        return Promise.all(defs).then(() => {
            const updateHistoryButtons = () => {
                this.state.canRedo = this.options.wysiwyg.harpiyaEditor.historyCanRedo();
                this.state.canUndo = this.options.wysiwyg.harpiyaEditor.historyCanUndo();
            };
            this.options.wysiwyg.harpiyaEditor.addEventListener('historyStep', updateHistoryButtons);
            this.options.wysiwyg.harpiyaEditor.addEventListener('observerApply', () => {
                $(this.options.wysiwyg.harpiyaEditor.editable).trigger('content_changed');
            });
    ...
    _makeSnippetDraggable() {
        ...
        const dragAndDropOptions = this.options.getDragAndDropOptions({
            ...
            onDragStart: ({ element }) => {
                ...
                const doc = this.options.wysiwyg.harpiyaEditor.document;
                $(doc.body).addClass('oe_dropzone_active');

                this.options.wysiwyg.harpiyaEditor.automaticStepUnactive();

                this.$el.find('.oe_snippet_thumbnail').addClass('o_we_ongoing_insertion');
                this.options.wysiwyg.harpiyaEditor.observerUnactive('dragAndDropCreateSnippet');
            ...
            onDragEnd: async ({ x, y, helper }) => {
                ...
                this.options.wysiwyg.harpiyaEditor.observerActive('dragAndDropCreateSnippet');
    ...
    _onClick(ev) {
        ...
        if ($oeStructure.length && !$oeStructure.children().length && this.snippets.size > 0) {
            ...
            this._activateSnippet(false).then(() => {
                this.$el.find('.oe_snippet').harpiyaBounce();
            });
            return;
        }
        this._activateSnippet($target);
   ...
   _onSnippetDragAndDropStart() {
    ...
    _showSnippetTooltip(snippetEl, delay = 1500) {
        ...
        this.hideShownTooltip = this.popover.add(snippetEl, HarpiyaTooltip, {
    ...
    _onSaveRequest(ev) {
        ...
        // If it's an HarpiyaEvent sent by sub-widgets, we prevent the event
        ...
        if (!ev.currentTarget.matches(".o_disabled") && ev.currentTarget.dataset.snippetGroup) {
            ...
            for (const el of $els) {
                if (!el.children.length) {
                    $(el).harpiyaBounce('o_we_snippet_area_animation');
                }
            }
        }
    }
    ...
    _checkEditorToolbarVisibility(e) {
        ...
        const selection = this.options.wysiwyg.harpiyaEditor.document.getSelection();
    ...
    _onRequestEditable(ev) {
        ev.data.callback($(this.options.wysiwyg.harpiyaEditor.editable));
    }
    /**
    ...
    _installModule(moduleID, snippetName) {
        ...
        const linkUrl = '/harpiya/action-base.open_module_tree/' + encodeURIComponent(moduleID);
