...
import * as HarpiyaEditorLib from "@web_editor/js/editor/harpiya-editor/src/HarpiyaEditor";
...
import { isIconElement, isSelectionInSelectors, peek } from '@web_editor/js/editor/harpiya-editor/src/utils/utils';
...
const HarpiyaEditor = HarpiyaEditorLib.HarpiyaEditor;
const getDeepRange = HarpiyaEditorLib.getDeepRange;
const getInSelection = HarpiyaEditorLib.getInSelection;
const isProtected = HarpiyaEditorLib.isProtected;
const rgbToHex = HarpiyaEditorLib.rgbToHex;
const preserveCursor = HarpiyaEditorLib.preserveCursor;
const closestElement = HarpiyaEditorLib.closestElement;
const setSelection = HarpiyaEditorLib.setSelection;
const endPos = HarpiyaEditorLib.endPos;
const hasValidSelection = HarpiyaEditorLib.hasValidSelection;
const parseHTML = HarpiyaEditorLib.parseHTML;
const closestBlock = HarpiyaEditorLib.closestBlock;
const getRangePosition = HarpiyaEditorLib.getRangePosition;
const childNodeIndex = HarpiyaEditorLib.childNodeIndex;
const fillEmpty = HarpiyaEditorLib.fillEmpty;
const isVisible = HarpiyaEditorLib.isVisible;
const getSelectedNodes = HarpiyaEditorLib.getSelectedNodes;
const getDeepestPosition = HarpiyaEditorLib.getDeepestPosition;
const paragraphRelatedElements = HarpiyaEditorLib.paragraphRelatedElements;
...
export class Wysiwyg extends Component {
    ...
    setup() {
        ...
        const getColorPickedHandler = (colorType) => {
            return (params) => {
                if (this.hadNonCollapsedSelectionBeforeColorpicker) {
                    this.harpiyaEditor.historyResetLatestComputedSelection(true);
                }
                // Unstash the mutations now that the color is picked.
                this.harpiyaEditor.historyUnstash();
                ...
                // hides it).
                if (hasValidSelection(this.harpiyaEditor.editable)) {
                    this.harpiyaEditor.deselectTable();
                }
        ...
        const getColorHoverHandler = (colorType) => {
            return (props) => {
                ...
                this.harpiyaEditor.historyPauseSteps();
                try {
                    this._processAndApplyColor(colorType, props.color, true);
                    this.harpiyaEditor._computeHistorySelection();
                } finally {
                    this.harpiyaEditor.historyUnpauseSteps();
                }
            }
        };
        ...
        const colorPaletteCommonOptions = {
            ...
            onColorLeave: () => {
                ...
                this.harpiyaEditor.withoutRollback(() => this.harpiyaEditor.historyRevertCurrentStep());
    ...
    async startEdition() {
        ...
        this.harpiyaEditor = new HarpiyaEditor(this.$editable[0], Object.assign({
            _t: _t,
        ...
        this.harpiyaEditor.addEventListener('contentChanged', function () {
        ...
        if (this.harpiyaEditor.document !== document) {
            this.harpiyaEditor.document.addEventListener("mousemove", this._signalOnline, true);
            this.harpiyaEditor.document.addEventListener("keydown", this._signalOnline, true);
            this.harpiyaEditor.document.addEventListener("keyup", this._signalOnline, true);
        }
        ...
        if ($('wrapwrap').length) {
            this.multiSelectionTarget = $().getScrollingTarget(this.harpiyaEditor.document);
            this.multiSelectionTarget.addEventListener('scroll', this.harpiyaEditor.multiselectionRefresh, { passive: true });
        }
        ...
        this._observeHarpiyaFieldChanges();
        this.$editable.on(
            'mousedown touchstart',
            '[data-oe-field]',
            function () {
                self.harpiyaEditor.observerUnactive();
                const $field = $(this);
                ...
                self.harpiyaEditor.observerActive();
            }
        );
        ...
        this.$editable.on('dblclick', mediaSelector, ev => {
            ...
            if (isEditable) {
                ...
                if (!isProtected(this.harpiyaEditor.document.getSelection().anchorNode)) {
        ...
        if (options.snippets) {
            ...
            $(this.harpiyaEditor.document.body).addClass('editor_enable');
        ...
        if (this.options.getContentEditableAreas) {
            $(this.options.getContentEditableAreas(this.harpiyaEditor)).find('*').off('mousedown mouseup click');
        }
        ...
        $(this.harpiyaEditor.editable).on('mouseup', this._updateEditorUI.bind(this));
        $(this.harpiyaEditor.editable).on('keydown', this._updateEditorUI.bind(this));
        $(this.harpiyaEditor.editable).on('keydown', this._handleShortcuts.bind(this));
        ...
        this.harpiyaEditor.document.addEventListener('selectionchange', this._onSelectionChange);
        if (!this.state.showSnippetsMenu) {
            this.setCSSVariables(this.toolbarEl);
        }

        this.harpiyaEditor.addEventListener('preObserverActive', () => {
            ...
            this._onPostSanitize(this.harpiyaEditor.editable);
        });

        if (this.options.autohideToolbar) {
            if (this.harpiyaEditor.isMobile) {
                this.harpiyaEditor.editable.before(this.toolbarEl);
            } else {
    ...
    setupToolbar(toolbarEl) {
        ...
        this.harpiyaEditor.setupToolbar(toolbarEl);
    ...
    destroy() {
        ...
        if (this.harpiyaEditor) {
            this.harpiyaEditor.document.removeEventListener("mousemove", this._signalOnline, true);
            this.harpiyaEditor.document.removeEventListener("keydown", this._signalOnline, true);
            this.harpiyaEditor.document.removeEventListener("keyup", this._signalOnline, true);
            this.harpiyaEditor.document.removeEventListener('selectionchange', this._onSelectionChange);
            this.harpiyaEditor.destroy();
        }
        ...
        this.multiSelectionTarget?.removeEventListener('scroll', this.harpiyaEditor.multiselectionRefresh, { passive: true });
    ...
    _renderElement() {
        ...
        if (this.options.resizable && !isMobileOS()) {
            const $wrapper = $('<div class="o_wysiwyg_wrapper harpiya-editor">');
    ...
    getValue(options) {
        ...
        if (this.harpiyaEditor) {
            this.harpiyaEditor.cleanForSave($editable[0]);
     ...
     */
    historyReset() {
        this.harpiyaEditor.historyReset();
    }
    /**
    ...
    async saveContent(reload = true, editable = false) {
        ...
        if (!editable) {
            ...
            const editables = "getContentEditableAreas" in this.options ? this.options.getContentEditableAreas(this.harpiyaEditor) : [];
     ...
     */
    setValue(value) {
        this.harpiyaEditor.resetContent(value);
    }
    /**
     * Undo one step of change in the editor.
     */
    undo() {
        this.harpiyaEditor.historyUndo();
    }
    /**
     * Redo one step of change in the editor.
     */
    redo() {
        this.harpiyaEditor.historyRedo();
    }
    /**
     ...
     */
    focus() {
        if (this.harpiyaEditor && !this.harpiyaEditor.historyResetLatestComputedSelection(true)) {
            ...
            const selection = this.harpiyaEditor.document.getSelection();
            selection.removeAllRanges();
            selection.addRange(range);
        }
    }
    getDeepRange() {
        return getDeepRange(this.harpiyaEditor.editable);
    }
    closestElement(...args) {
        return closestElement(...args);
    }
    async cleanForSave(editable = this.harpiyaEditor.editable) {
        if (this.harpiyaEditor) {
            this.harpiyaEditor.cleanForSave(editable);
    ...
    isSelectionInEditable() {
        return this.harpiyaEditor.isSelectionInEditable();
    }
    /**
     * Start or resume the Harpiya field changes muation observers.
     *
     * Necessary to keep all copies of a given field at the same value throughout the page.
     */
    _observeHarpiyaFieldChanges() {
        ...
        if (this.harpiyaFieldObservers) {
            for (let observerData of this.harpiyaFieldObservers) {
                observerData.observer.observe(observerData.field, observerOptions);
            }
        } else {
            const harpiyaFieldSelector = '[data-oe-model], [data-oe-translation-source-sha]';
            const $harpiyaFields = this.$editable.find(harpiyaFieldSelector);
            const renderingClassesSelector = this.harpiyaEditor.options.renderingClasses
                .map(className => `.${className}`).join(", ");
            this.harpiyaFieldObservers = [];

            $harpiyaFields.each((i, field) => {
                const observer = new MutationObserver((mutations) => {
                    mutations = this.harpiyaEditor.filterMutationRecords(mutations);
                    ...
                    let $nodes = $harpiyaFields.filter(function () {
                        return this !== field;
                    });
                    ...
                    this._pauseHarpiyaFieldObservers(field);
                    ...
                    for (const node of fieldNodeClone.querySelectorAll(renderingClassesSelector)) {
                        node.classList.remove(...this.harpiyaEditor.options.renderingClasses);
                    }
                    const html = $(fieldNodeClone).html();
                    this.harpiyaEditor.withoutRollback(() => {
                ...
                this.harpiyaFieldObservers.push({field: field, observer: observer});
            });
        }
    }
    /**
     ...
     */
    _pauseHarpiyaFieldObservers(currentlyEditedField) {
        for (const fieldObserverData of this.harpiyaFieldObservers) {
    ...
    openLinkToolsFromSelection() {
        ...
        for (const el of getSelectedNodes(this.harpiyaEditor.editable)) {
            if (
        ...
        if (targetEl.nodeType === Node.ELEMENT_NODE
            ...
            this.harpiyaEditor.dispatchEvent(new Event('activate_image_link_tool'));
            return;
        }
        this.toggleLinkTools();
    }
    ...
    async toggleLinkTools(options = {}) {
        ...
        const linkEl = getInSelection(this.harpiyaEditor.document, 'a');
        ...
        if (this.state.showSnippetsMenu && !options.forceDialog) {
            ...
            if (options.forceOpen || !this.state.linkToolProps) {
                ...
                if (!this.state.linkToolProps || ![options.link, ...wysiwygUtils.ancestors(options.link)].includes(this.linkToolsInfos.link)) {
                    const { link } = getOrCreateLink({
                        containerNode: this.harpiyaEditor.editable,
                    ...
                    const addHintClasses = () => {
                        this.harpiyaEditor.observerUnactive("hint_classes");
                        link.classList.add('oe_edited_link');
                        $button.addClass('active');
                        this.harpiyaEditor.observerActive("hint_classes");
                    };
                    ...
                    this.state.linkToolProps = {
                        ...
                        wysiwyg: this,
                        editable: this.harpiyaEditor.editable,
                        link,
                ...
                this.harpiyaEditor.document.removeEventListener('click', this._onClick, true);
                ...
                this.harpiyaEditor.document.addEventListener('click', this._onClick, true);
                document.addEventListener('click', this._onClick, true);
            } else {
                this.destroyLinkTools();
            }
        } else {
            const historyStepIndex = this.harpiyaEditor.historySize() - 1;
            this.harpiyaEditor.historyPauseSteps();
            ...
            this.env.services.dialog.add(LinkDialog, {
                ...
                onSave: (data) => {
                    ...
                    this.harpiyaEditor.historyUnpauseSteps();
                    this.harpiyaEditor.historyStep();
                ...
                onClose: () => {
                    this.harpiyaEditor.historyUnpauseSteps();
                    this.harpiyaEditor.historyRevertUntil(historyStepIndex)
                }
            });
        }
    }
    ...
    openChatGPTDialog(mode = 'prompt', options={}) {
        const restore = preserveCursor(this.harpiyaEditor.document);
        const params = {
            insert: content => {
                this.harpiyaEditor.historyPauseSteps();
                const insertedNodes = this.harpiyaEditor.execCommand('insert', content);
                this.harpiyaEditor.historyUnpauseSteps();
                ...
                if (start && end) {
                    const divContainer = this.harpiyaEditor.editable.parentElement;
        ...
        if (mode === 'alternatives' || mode === 'translate') {
            params.originalText = this.harpiyaEditor.document.getSelection().toString() || '';
        }
        if (mode === 'translate') {
            params.language = options.language;
        }
        this.harpiyaEditor.document.getSelection().collapseToEnd();
    ...
    removeLink() {
        if (this.state.showSnippetsMenu && wysiwygUtils.isImg(this.lastElement)) {
            this.mutex.exec(() => {
                this.harpiyaEditor.dispatchEvent(new Event('deactivate_image_link_tool'));
            });
        } else {
            this.harpiyaEditor.execCommand('unlink');
        }
    }
    /**
    ...
    showImageFullscreen(url) {
        ...
        this.harpiyaEditor.document.getSelection()?.collapseToEnd();
        this.harpiyaEditor.editable.blur();
    }
    /**
    ...
    openMediaDialog(params = {}) {
        const sel = this.harpiyaEditor.document.getSelection();
        ...
        const restoreSelection = preserveCursor(this.harpiyaEditor.document);

        const editable = HarpiyaEditorLib.closestElement(params.node || range.startContainer, '.o_editable') || this.harpiyaEditor.editable;
    ...
    // todo: test me
    showEmojiPicker() {
        const targetEl = this.harpiyaEditor.document.getSelection();
        ...
        this.popover.add(closest, EmojiPicker, {
                onSelect: (str) => {
                    restoreSelection();
                    this.harpiyaEditor.execCommand('insert', str);
                },
            }, {
    ...
    _onMediaDialogSave(params, element) {
        ...
        if (params.node) {
            ...
            this.harpiyaEditor.unbreakableStepUnactive();
            ...
            if (params.node.matches(".oe_unremovable")) {
                ...
                this.harpiyaEditor.historyStep();
                this.harpiyaEditor.observerUnactive("unremovable");
                element.classList.add("oe_unremovable");
                this.harpiyaEditor.observerActive("unremovable");
            } else {
                this.harpiyaEditor.historyStep();
            }

            // Refocus again to save updates when calling `_onWysiwygBlur`
            this.harpiyaEditor.editable.focus();
        } else {
            const result = this.harpiyaEditor.execCommand('insert', element);
            // Refocus again to save updates when calling `_onWysiwygBlur`
            this.harpiyaEditor.editable.focus();
            return result;
        }
    ...
    getInSelection(selector) {
        return getInSelection(this.harpiyaEditor.document, selector);
    }
    /**
    ...
    async getSnippetsMenuClass() {
        const snippetsEditor = await harpiya.loader.modules.get('@web_editor/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    _configureToolbar(options) {
        ...
        const openTools = e => {
            ...
            switch (e.currentTarget.id) {
                ...
                case 'open-chatgpt': {
                    this.openChatGPTDialog(this.harpiyaEditor.document.getSelection()?.isCollapsed ? 'prompt' : 'alternatives');
                    break;
                }
            }
        };
        ...
        $toolbar.find('#image-transform').click(e => {
            ...
            $image.transfo({document: this.harpiyaEditor.document});
            const destroyTransfo = () => {
                $image.transfo('destroy');
                $(this.harpiyaEditor.document).off('mousedown', mousedown).off('mouseup', mouseup);
                this.harpiyaEditor.document.removeEventListener('keydown', keydown);
            }
            const mouseup = () => {
                imgTransformBtn.classList.toggle('active', $image[0].matches('[style*="transform"]'));
            };
            $(this.harpiyaEditor.document).on('mouseup', mouseup);
            ...
            $(this.harpiyaEditor.document).on('mousedown', mousedown);
            ...
            this.harpiyaEditor.document.addEventListener('keydown', keydown);
        });
        $toolbar.find('#image-delete').click(e => {
            ...
            this.harpiyaEditor.toolbarHide();
        });
    ...
    _showImageCrop() {
        ...
        this.harpiyaEditor.toolbarHide();
        $(this.lastMediaClicked).on('image_cropper_destroyed', () => this.harpiyaEditor.toolbarShow());
    }
    /**
     * @private
     * @param {jQuery} $
    ...
    onColorpaletteDropdownShow(colorType) {
        ...
        this.harpiyaEditor.historyStash();
    }
    ...
    _processAndApplyColor(colorType, color, previewMode) {
        ...
        const selectedTds = this.harpiyaEditor.document.querySelectorAll('td.o_selected_td');
        ...
        let coloredElements = this.harpiyaEditor.execCommand('applyColor', color, colorType === 'text' ? 'color' : 'backgroundColor', this.lastMediaClicked);
        // Some nodes returned by applyColor can be removed of the document by the sanitization in historyStep
        coloredElements = coloredElements.filter(element => this.harpiyaEditor.document.contains(element));
        ...
        if (selectedTds.length === 1 && !previewMode) {
            ...
            range.setEnd(...endPos(last));
                const { startContainer, startOffset, endContainer, endOffset } = getDeepRange(this.harpiyaEditor.editable, { range });
            if (isSelForward) {
        ...
        this.harpiyaEditor.updateColorpickerLabels({
     ...
     */
    _updateEditorUI(e) {
        let selection = this.harpiyaEditor.document.getSelection();
        if (!selection) return;
        const anchorNode = selection.anchorNode;
        if (isProtected(anchorNode)) {
            return;
        }
        if (this.harpiyaEditor.document.querySelector(".transfo-container")) {
            return;
        }

        this.harpiyaEditor.automaticStepSkipStack();
        // We need to use the editor's window so the tooltip displays in its
        // document even if it's in an iframe.
        const editorWindow = this.harpiyaEditor.document.defaultView;
        ...
        if (isInMedia) {
            ...
            const selection = this.harpiyaEditor.document.getSelection();
            const range = this.harpiyaEditor.document.createRange();
        ...
        this.harpiyaEditor.updateToolbarPosition();
    ...
    _updateMediaJustifyButton(commandState) {
        ...
        // Change the ID to prevent HarpiyaEditor from controlling it as this is
    ...
    _getBannerCommand(title, emoji, alertClass, iconClass, description, priority) {
        return {
            ...
            isDisabled: () => isSelectionInSelectors('.o_editor_banner') || !this.harpiyaEditor.isSelectionInBlockRoot(),
            callback: () => {
                const bannerElement = parseHTML(this.harpiyaEditor.document, `
                ...
                `).childNodes[0];
                this.harpiyaEditor.execCommand('insert', bannerElement);
                this.harpiyaEditor.activateContenteditable();
    ...
    _getPowerboxOptions() {
        ...
        const commands = [
            ...
            this._getBannerCommand(_t('Banner Danger'), '❌', 'danger', 'fa-exclamation-circle', _t('Insert a danger banner'), 21),
            {
                ...
                isDisabled: () => !this.harpiyaEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.harpiyaEditor.execCommand('setTag', 'blockquote');
                },
            },
            {
                category: _t('Structure'),
                ...
                isDisabled: () => !this.harpiyaEditor.isSelectionInBlockRoot(),
                callback: () => {
                    this.harpiyaEditor.execCommand('setTag', 'pre');
                },
            },
            {
                ...
                callback: async () => {
                    ...
                    if (currentUser && currentUser.signature) {
                        this.harpiyaEditor.execCommand('insert', parseHTML(this.harpiyaEditor.document, currentUser.signature));
                    }
                },
            },
            {
            ...
            commands.push(
                {
                    ...
                    callback: () => this.harpiyaEditor.execCommand('columnize', 2, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.harpiyaEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                        const anchor = this.harpiyaEditor.document.getSelection().anchorNode;
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.harpiyaEditor.execCommand('columnize', 3, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.harpiyaEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.harpiyaEditor.execCommand('columnize', 4, editorOptions.insertParagraphAfterColumns),
                    isDisabled: () => {
                        if (!this.harpiyaEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
                ...
                },
                {
                    ...
                    fontawesome: 'fa-columns',
                    callback: () => this.harpiyaEditor.execCommand('columnize', 0),
                    isDisabled: () => {
                        if (!this.harpiyaEditor.isSelectionInBlockRoot()) {
                            return true;
                        }
    ...
    async _saveElement($el, context) {
        ...
        const websiteCore = iframe.contentWindow.harpiya.__WOWL_DEBUG__.root.env.services["public.interactions"];
        websiteCore.stopInteractions();
        // remove ZeroWidthSpace from harpiya field value
        // ZeroWidthSpace may be present from HarpiyaEditor edition process
    ...
    _onSelectionChange() {
        if (this.linkPopover && this.isSelectionInEditable()) {
            const selectionInLink = getInSelection(this.harpiyaEditor.document, 'a') === this.linkPopover.target;
            const isVisible = this.linkPopover.el.offsetParent;
            if (
                isVisible &&
                (
                    (this.options.autohideToolbar && !this.harpiyaEditor.document.getSelection().isCollapsed) ||
                    !selectionInLink
                )
            ) {
                this.linkPopover.hide();
    ...
    async _signalOnline() {
        ...
        this.ptp.notifyAllClients('oe_history_step', peek(this.harpiyaEditor.historyGetSteps()), { transport: 'rtc' });
    }
    /**
    ...
    async _processMissingSteps(missingSteps) {
        ...
        this.ptp && this.harpiyaEditor.onExternalHistorySteps(missingSteps);
        return true;
    }
    _showConflictDialog() {
        if (this._conflictDialogOpened) return;
        const content = markup(this.harpiyaEditor.editable.cloneNode(true).outerHTML);
    ...
    _getNewPtp() {
        ...
        return new PeerToPeer({
            ...
            onRequest: {
                ...
                get_missing_steps: (params) => this.harpiyaEditor.historyGetMissingSteps(params.requestPayload),
                get_history_from_snapshot: () => this._getHistorySnapshot(),
                get_collaborative_selection: () => this.harpiyaEditor.getCurrentCollaborativeSelection(),
                recover_document: (params) => {
                    const { serverDocumentId, fromStepId } = params.requestPayload;
                    if (!this.harpiyaEditor.historyGetBranchIds().includes(serverDocumentId)) {
                        return;
                    }
                    return {
                        missingSteps: this.harpiyaEditor.historyGetMissingSteps({ fromStepId }),
            ...
            onNotification: async ({ fromClientId, notificationName, notificationPayload }) => {
                switch (notificationName) {
                    case 'ptp_remove':
                        this.harpiyaEditor.multiselectionRemove(notificationPayload);
                        break;
                    case 'ptp_disconnect':
                        this.ptp.removeClient(fromClientId);
                        this.harpiyaEditor.multiselectionRemove(fromClientId);
                        break;
                    case 'rtc_data_channel_open': {
                        ...
                        const getClientNamePromise = this.requestClient(
                            ...
                            this.harpiyaEditor.multiselectionRefresh();
                        });
                    ...
                    case 'oe_history_step':
                        if (this._historySyncFinished) {
                            this.harpiyaEditor.onExternalHistorySteps([notificationPayload]);
                        } else {
                    ...
                    case 'oe_history_set_selection': {
                        ...
                        this.harpiyaEditor.onExternalMultiselectionUpdate(selection);
                        break;
                    }
                }
            }
    ...
    async _setCollaborativeSelection(fromClientId) {
        ...
        if (remoteSelection) {
            this.harpiyaEditor.onExternalMultiselectionUpdate(remoteSelection);
        }
    }
    /**
    ...
    _isLastDocumentStale() {
        ...
        return !this.harpiyaEditor.historyGetBranchIds().includes(this._serverLastStepId);
    }
    /**
            ...
            for (const peerId of allPeers) {
                this.requestClient(
                    ...
                    'recover_document', {
                        ...
                        fromStepId: peek(this.harpiyaEditor.historyGetBranchIds()),
                    },
                    { transport: 'rtc' }
                ).then((response) => {
                    nbPendingResponses--;
    ...
    async _resetFromServerAndResyncWithClients() {
        ...
        // lastHistoryId will be different if the harpiya bus did not have time to
        // notify the user.
    ...
    async _resetFromClient(fromClientId, resetCollabCount) {
        ...
        if (this._historyStepsBuffer.length) {
            this.harpiyaEditor.onExternalHistorySteps(this._historyStepsBuffer);
            this._historyStepsBuffer = [];
        }
    ...
    async resetValue(value) {
        ...
        if (this._serverLastStepId) {
            this.harpiyaEditor.historySetInitialId(this._serverLastStepId);
        }
    }
    /**
    ...
    async resetEditor(value, options) {
        ...
        this._currentClientId = this._generateClientId();
        this.harpiyaEditor.collaborationSetClientId(this._currentClientId);
        this.resetValue(value);
    ...
    _getHistorySnapshot() {
        return Object.assign(
            {},
            this.harpiyaEditor.historyGetSnapshotSteps(),
    ...
    _applySnapshot(snapshot) {
        ...
        this._historySyncAtLeastOnce = true;
        this.harpiyaEditor.historyResetFromSteps(steps, historyIds);
        this.harpiyaEditor.historyResetLatestComputedSelection();
        return true;
    }
    /**
    ...
    _onPostSanitize(node) {
        ...
        // solution is merged.
        if (node?.querySelectorAll && this.harpiyaEditor && !this.harpiyaEditor._fixLinkMutatedElements) {
    ...
    _attachHistoryIds(editable = this.harpiyaEditor.editable) {
        if (this.options.collaborative) {
            ...
            const historyIds = this.harpiyaEditor.historyGetBranchIds().join(',');
