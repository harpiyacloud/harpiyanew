...
import { deduceURLfromText } from "@web_editor/js/editor/harpiya-editor/src/utils/sanitize";

const { getDeepRange, getInSelection, EMAIL_REGEX, PHONE_REGEX } = HarpiyaEditorLib;
...
export class Link extends Component {
    ...
    _isAbsoluteURLInCurrentDomain(url) {
        ...
        return urlObj.origin === window.location.origin
            ...
            // its Harpiya instance domain. We just suppose it should be a relative
            // URL (if unexpected behavior, the user can just not enter its Harpiya
            // instance domain but its real domain, or opt-out from the domain
            // stripping). Mentioning an .harpiya.com domain, especially its own
            // one, is always a bad practice anyway.
            || new RegExp(`^https?://${session.db}\\.harpiya\\.com(/.*)?$`).test(urlObj.origin);
    }
    /**
    ...
    async _updateState(props) {
        ...
        if (this.state.range) {
            this.$link = this.$link || $(HarpiyaEditorLib.getInSelection(this.editable.ownerDocument, 'a'));
