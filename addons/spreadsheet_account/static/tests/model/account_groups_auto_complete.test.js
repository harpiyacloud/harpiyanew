...
test("HARPIYA.ACCOUNT.GROUP type", async function () {
    const { store: composer } = await makeStore(CellComposerStore);

    composer.startEdition("=HARPIYA.ACCOUNT.GROUP(");
    ...
    expect(composer.currentContent).toBe('=HARPIYA.ACCOUNT.GROUP("asset_receivable"');
