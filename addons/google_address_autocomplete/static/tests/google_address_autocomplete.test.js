...
import { runAllTimers } from "@harpiya/hoot-mock";
...
test("correctly fill all standard fields", async () => {
    ...
    currentInput = "harpiya farm 2";
    await contains(".o_field_widget[name='street'] input").edit("harpiya farm 2", { confirm: false });
    await runAllTimers();
    ...
    currentInput = "harpiya farm 3";
    await contains(".o_field_widget[name='street'] input").edit("harpiya farm 3", { confirm: false });
    await runAllTimers();
    expect.verifySteps(["/autocomplete/address"]);
...
test("fills current field with values of unknown ones", async () => {
    ...
    await contains(".o_field_widget[name='some_char'] input").edit("harpiya farm 2", {
        confirm: false,
    });
    await runAllTimers();
    await contains(
...
test("typing in input should make form dirty", async () => {
    ...
    expect.verifySteps([{street: 'harpiya farm 3'}]);
});
