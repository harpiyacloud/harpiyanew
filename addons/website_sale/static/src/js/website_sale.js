...
import "@website/libs/zoomharpiya/zoomharpiya";
        ...
        if (salePage.dataset.ecomZoomAuto && !uiUtils.isSmall()) {
            ...
            for (const image of images) {
                ...
                this.zoomCleanup.push(() => {
                    ...
                    const zoomHarpiya = $image.data("zoomHarpiya");
                    if (zoomHarpiya) {
                        zoomHarpiya.hide();
                        $image.unbind();
                    }
            ...
            for (const image of images ) {
                const handler = () => {
                    if (salePage.dataset.ecomZoomAuto) {
                        // Remove any flyout
                        const flyouts = document.querySelectorAll(".zoomharpiya-flyout");
...
publicWidget.registry.WebsiteSaleLayout = publicWidget.Widget.extend({
    ...
    _onApplyShopLayoutChange: function (ev) {
        ...
        if (wysiwyg) {
            wysiwyg.harpiyaEditor.observerUnactive('_onApplyShopLayoutChange');
        }
        ...
        if (wysiwyg) {
            wysiwyg.harpiyaEditor.observerActive('_onApplyShopLayoutChange');
        }
    },
});
