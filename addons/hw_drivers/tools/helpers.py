...
from harpiya import http, release, service
from harpiya.tools.func import lazy_property
from harpiya.tools.misc import file_path
...
class IoTRestart(Thread):
    """
    Thread to restart harpiya server in IoT Box when we must return a answer before
    """
...
def require_db(function):
    ...
    def wrapper(*args, **kwargs):
        fname = f"<function {function.__module__}.{function.__qualname__}>"
        server_url = get_harpiya_server_url()
        iot_box_ip = get_ip()
...
def check_certificate():
    ...
    if cn == 'HarpiyaTempIoTBoxCertificate' or datetime.datetime.now() > cert_end_date:
...
def check_git_branch(server_url=None):
    """Check if the local branch is the same as the connected Harpiya DB and
    checkout to match it if needed.

    :param server_url: The URL of the connected Harpiya database (provided by decorator).
    """
    try:
        ...
        _logger.exception('Could not reach configured server to get the Harpiya version')
        return
    except ValueError:
    ...
    try:
        git = ['git', '--work-tree=/home/pi/harpiya/', '--git-dir=/home/pi/harpiya/.git']
        ...
        _logger.info(
            "Current IoT Box local git branch: %s / Associated Harpiya database's git branch: %s",
            local_branch,
            db_branch,
        )
        ...
        if db_branch != local_branch:
            try:
                with writable():
                    ...
                    _logger.info("Updating harpiya folder to the branch %s", db_branch)
                    subprocess.run(
                        ['/home/pi/harpiya/addons/iot_box_image/configuration/checkout.sh'], check=True
                    )
...
def check_image():
    ...
    """
    try:
        response = requests.get('https://nightly.harpiya.com/master/iotbox/SHA1SUMS.txt', timeout=5)
        response.raise_for_status()
...
def save_conf_server(url, token, db_uuid, enterprise_code):
    """
    Save server configurations in harpiya.conf
...
@cache
def get_harpiya_server_url():
    """Get the URL of the linked Harpiya database.

    :return: The URL of the linked Harpiya database.
    :rtype: str or None
    """
...
def get_commit_hash():
    return subprocess.run(
        ['git', '--work-tree=/home/pi/harpiya/', '--git-dir=/home/pi/harpiya/.git', 'rev-parse', '--short', 'HEAD'],
...
def get_version(detailed_version=False):
    if platform.system() == 'Linux':
        image_version = read_file_first_line('/var/harpiya/iotbox_version')
...
def load_certificate():
    """
    Send a request to Harpiya with customer db_uuid and enterprise_code to get a true certificate
    """
    ...
    try:
        response = requests.post(
            'https://www.harpiya.com/harpiya-enterprise/iot/x509',
        ...
        _logger.exception("An error occurred while trying to reach harpiya.com servers.")
...
def delete_iot_handlers():
    ...
    try:
        ...
        filenames = [
            f"harpiya/addons/hw_drivers/iot_handlers/{file.relative_to(iot_handlers)}"
...
def download_iot_handlers(auto=True, server_url=None):
    """Get the drivers from the configured Harpiya server.
    ...
    delete_iot_handlers()
    with writable():
        path = path_file('harpiya', 'addons', 'hw_drivers', 'iot_handlers')
        zip_file = zipfile.ZipFile(io.BytesIO(data))
        zip_file.extractall(path)


def compute_iot_handlers_addon_name(handler_kind, handler_file_name):
    return "harpiya.addons.hw_drivers.iot_handlers.{handler_kind}.{handler_name}".\
        format(handler_kind=handler_kind, handler_name=handler_file_name.removesuffix('.py'))

def load_iot_handlers():
    """
    This method loads local files: 'harpiya/addons/hw_drivers/iot_handlers/drivers' and
    'harpiya/addons/hw_drivers/iot_handlers/interfaces'
...
def harpiya_restart(delay=0):
    """
    Restart Harpiya service
...
def path_file(*args):
    """Return the path to the file from IoT Box root or Windows Harpiya
    server folder
    ...
    platform_os = platform.system()
    if platform_os == 'Linux':
        return Path("~pi", *args).expanduser()  # Path.home() returns harpiya user's home instead of pi's
    elif platform_os == 'Windows':
...
def download_from_url(download_url, path_to_filename):
    ...
    (Example: 'C:\\Program Files\\Harpiya\\downloaded_file.zip')
    """
    try:
...
def unzip_file(path_to_filename, path_to_extract):
    ...
    Example: unzip_file('C:\\Program Files\\Harpiya\\downloaded_file.zip', 'C:\\Program Files\\Harpiya\\new_folder'))
...
def update_conf(values, section='iot.box'):
    """Update harpiya.conf with the given key and value.
    ...
    """
    _logger.debug("Updating harpiya.conf with values: %s", values)
    conf = get_conf()

    if not conf.has_section(section):
        _logger.debug("Creating new section '%s' in harpiya.conf", section)
    ...
    with writable():
        with open(path_file("harpiya.conf"), "w", encoding='utf-8') as f:
            conf.write(f)


def get_conf(key=None, section='iot.box'):
    """Get the value of the given key from harpiya.conf, or the full config if no key is provided.
    ...
    conf.read(path_file("harpiya.conf"))
...
def disconnect_from_server():
    ...
    get_harpiya_server_url.cache_clear()
    update_conf({
        'remote_server': '',
        'token': '',
        'db_uuid': '',
