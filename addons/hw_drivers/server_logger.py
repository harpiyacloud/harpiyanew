...
from harpiya.addons.hw_drivers.tools import helpers
from harpiya.netsvc import DBFormatter

_logger = logging.getLogger(__name__)

IOT_LOG_TO_SERVER_CONFIG_NAME = 'iot_log_to_server'  # config name in harpiya.conf
...
class AsyncHTTPHandler(logging.Handler):
    ...
    def __init__(self, harpiya_server_url, active):
        """
        :param harpiya_server_url: Harpiya Server URL
        """
        super().__init__()
        self._harpiya_server_url = harpiya_server_url
    ...
    def toggle_active(self, is_active):
        ...
        if self._active and self._harpiya_server_url:
    ...
    def _periodic_flush(self):
        harpiya_session = requests.Session()
        while self._harpiya_server_url and self._active:  # allow to exit the loop on thread.join
            time.sleep(self._FLUSH_INTERVAL)
            self._flush_logs(harpiya_session)

    def _flush_logs(self, harpiya_session):
        ...
        if not self._harpiya_server_url or queue_size == 0:
            return
        try:
            harpiya_session.post(
                self._harpiya_server_url + '/iot/log',
                data=empty_queue(),
...
def get_harpiya_config_log_to_server_option():
    return helpers.get_conf(IOT_LOG_TO_SERVER_CONFIG_NAME, section='options') or True  # Enabled by default


def check_and_update_harpiya_config_log_to_server_option(new_state):
    """
    :return: wherever the config file need to be updated or not
    """
    if get_harpiya_config_log_to_server_option() != new_state:
...
_server_log_sender_handler = AsyncHTTPHandler(helpers.get_harpiya_server_url(), get_harpiya_config_log_to_server_option())
...
# Set it in the 'root' logger, on which every logger (including harpiya) is a child
