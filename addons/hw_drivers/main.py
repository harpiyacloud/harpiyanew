...
from harpiya.addons.hw_drivers.websocket_client import WebsocketClient
...
class Manager(Thread):
    ...
    def send_all_devices(self, server_url=None):
        """This method send IoT Box and devices information to Harpiya database

        :param server_url: URL of the Harpiya server (provided by decorator).
        """
    ...
    def run(self):
        """Thread that will load interfaces and drivers and contact the harpiya server
        ...
        if platform.system() == 'Linux' and helpers.get_harpiya_server_url():
