...
from harpiya import http, tools

from harpiya.addons.hw_drivers.event_manager import event_manager
from harpiya.addons.hw_drivers.main import iot_devices, manager
from harpiya.addons.hw_drivers.tools import helpers
from harpiya.addons.hw_drivers.tools import route
...
class DriverController(http.Controller):
    ...
    def download_logs(self):
        ...
        """
        log_path = tools.config['logfile'] or "/var/log/harpiya/harpiya-server.log"
        try:
        ...
        log_file_name = f"iot-harpiya-{gethostname()}-{datetime.now().strftime('%Y-%m-%d_%H-%M-%S')}.log"
        # intentionally don't use Stream.from_path as the path used is not in the addons path
        # for instance, for the iot-box it will be in /var/log/harpiya
        return http.Stream(
                type='path',
                path=log_path,
