...
from harpiya.addons.hw_drivers.main import manager, iot_devices
...
class ConnectionManager(Thread):
    ...
    def _should_fetch_pairing_code(self):
        return (
            not helpers.get_harpiya_server_url() and
            helpers.get_ip() and
    ...
    def _call_iot_proxy(self, pairing_code, pairing_uuid):
        ...
        try:
            req = requests.post(
                'https://iot-proxy.harpiya.com/harpiya-enterprise/iot/connect-box', json=data, verify=False, timeout=5
            )
            return req.json().get('result', {})
        except Exception:
            _logger.exception('Could not reach iot-proxy.harpiya.com')
            return {}
