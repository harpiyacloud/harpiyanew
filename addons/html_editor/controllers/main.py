...
from harpiya import _, http, tools, SUPERUSER_ID
from harpiya.addons.html_editor.tools import get_video_url_data
from harpiya.exceptions import UserError, MissingError, AccessError
...
DEFAULT_LIBRARY_ENDPOINT = 'https://media-api.harpiya.com'
    ...
    def link_preview_metadata_internal(self, preview_url):
        try:
            ...
            if (action_name.startswith('m-') or '.' in action_name) and action_name in request.env and not request.env[action_name]._abstract:
                # if path format is `harpiya/<model>/<record_id>` so we use `action_name` as model name
