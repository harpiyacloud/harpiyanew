...
export function prepareUpdate(...args) {
    const closestRoot =
        args.length &&
        ancestors(args[0]).find((ancestor) => ancestor.classList.contains("harpiya-editor-editable"));
    ...
    while (positions.length) {
        ...
        if (options.debug) {
            const editable = el && closestElement(el, ".harpiya-editor-editable");
...
export function restoreState(prevStateData, debug = false) {
    ...
    if (debug) {
        const editable = closestElement(node, ".harpiya-editor-editable");
        console.log(
            "%c" +
