...
export class ClipboardPlugin extends Plugin {
    ...
    onCopy(ev) {
        ...
        ev.clipboardData.setData("application/vnd.harpiya.harpiya-editor", htmlContent);
    }
    ...
    onPaste(ev) {
        ...
        this.handlePasteUnsupportedHtml(selection, ev.clipboardData) ||
            this.handlePasteHarpiyaEditorHtml(ev.clipboardData) ||
     ...
     * @param {DataTransfer} clipboardData
     */
    handlePasteHarpiyaEditorHtml(clipboardData) {
        const harpiyaEditorHtml = clipboardData.getData("application/vnd.harpiya.harpiya-editor");
        const textContent = clipboardData.getData("text/plain");
        if (ONLY_LINK_REGEX.test(textContent)) {
            return false;
        }
        if (harpiyaEditorHtml) {
    ...
    onDragStart(ev) {
        if (ev.target.nodeName === "IMG") {
            ...
            ev.dataTransfer.setData(
                "application/vnd.harpiya.harpiya-editor-node",
    ...
    async onDrop(ev) {
        ...
        const imageNodeHTML = ev.dataTransfer.getData("application/vnd.harpiya.harpiya-editor-node");
        const image =
            imageNodeHTML &&
            this.dragImage &&
...
// @phoenix @todo: move to Harpiya plugin?
/**
