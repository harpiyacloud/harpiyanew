...
describe("Link creation", () => {
    describe("Creation by space", () => {
        ...
        test("typing invalid URL + space should not convert to link", async () => {
            const { editor, el } = await setupEditor("<p>[]</p>");
            await insertText(editor, "www.harpiya");
            await insertText(editor, " ");
            expect(cleanLinkArtifacts(getContent(el))).toBe("<p>www.harpiya []</p>");
        });
    });
...
describe("shortcut", () => {
    ...
    test("should be able to create link with ctrl+k and ctrl+k", async () => {
        const { el } = await setupEditor(`<p>[]</p>`);
        // Open harpiya global command bar
        await press(["ctrl", "k"]);
        await waitFor('.o_command span[title="Create link"]');
...
describe("link preview", () => {
    test("test internal link preview", async () => {
        ...
        onRpc("/harpiya/project/1/tasks/8", () => new Response("", { status: 200 }));
        const { editor, el } = await setupEditor(`<p>[]</p>`);
        await insertText(editor, "/link");
        await animationFrame();
        await click(".o-we-command-name:first");
        await contains(".o-we-linkpopover input.o_we_href_input_link").fill(
            window.location.origin + "/harpiya/project/1/tasks/8"
        );
        await animationFrame();
    ...
    test("test external link preview", async () => {
        onRpc("/html_editor/link_preview_external", () => ({
            og_description:
                "From ERP to CRM, eCommerce and CMS. Download Harpiya or use it in the cloud. Grow Your Business.",
            og_image: "https://www.harpiya.com/web/image/41207129-1abe7a15/homepage-seo.png",
            og_title: "Open Source ERP and CRM | Harpiya",
            og_type: "website",
            og_site_name: "Harpiya",
            source_url: "http://harpiya.com/",
        }));
        const { editor } = await setupEditor(`<p>[]</p>`);
        await insertText(editor, "/link");
        await animationFrame();
        await click(".o-we-command-name:first");
        await contains(".o-we-linkpopover input.o_we_href_input_link").fill("http://harpiya.com/");
        await animationFrame();
        expect(".o_we_replace_title_btn").toHaveCount(1);
        expect(".o_extra_info_card").toHaveCount(1);
        expect(".o_we_url_link").toHaveText("Open Source ERP and CRM | Harpiya");
    ...
    test("test internal metadata cached correctly", async () => {
        ...
        onRpc("/harpiya/cachetest/8", () => new Response("", { status: 200 }));
        const { editor } = await setupEditor(`<p>abc[]</p>`);
        await insertText(editor, "/link");
        await animationFrame();
        await click(".o-we-command-name:first");
        await contains(".o-we-linkpopover input.o_we_href_input_link").fill(
            window.location.origin + "/harpiya/cachetest/8"
        );
        await animationFrame();
