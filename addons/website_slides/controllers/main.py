...
from harpiya import fields, http, tools, _
...
from harpiya.addons.website_profile.controllers.main import WebsiteProfile
from harpiya.exceptions import AccessError, ValidationError, UserError, MissingError
from harpiya.http import request, Response
from harpiya.osv import expression
from harpiya.tools import consteq, email_normalize_all
    ...
    def _set_viewed_slide(self, slide, quiz_attempts_inc=False):
        if not slide.channel_id.is_member:
            if not isinstance(request.session.get('viewed_slides'), dict):
                # Compatibility layer with Harpiya 15.0,
                # where `viewed_slides` are stored as `list` in sessions.
                # For performance concerns, `viewed_slides` is changed to a dict,
                # but sessions coming from Harpiya 15.0 after an upgrade should still be compatible.
                # This compatibility layer regarding `viewed_slides` must remain from Harpiya 16.0 and above,
                # as this is possible to do a jump of multiple versions in one go,
                # and carry the sessions with the upgrade.
                # e.g. upgrade from Harpiya 15.0 to 18.0.
