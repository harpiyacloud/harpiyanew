...
from harpiya import api, Command, fields, models, tools, SUPERUSER_ID, _
from harpiya.addons.rating.models import rating_data
from harpiya.addons.web_editor.tools import handle_history_divergence
from harpiya.exceptions import UserError, ValidationError, AccessError
from harpiya.osv import expression
from harpiya.tools import format_list, SQL, LazyTranslate
from harpiya.addons.resource.models.utils import filter_domain_leaf
from harpiya.addons.project.controllers.project_sharing_chatter import ProjectSharingChatter
...
class ProjectTask(models.Model):
    ...
    # Need this field to check there is no email loops when Harpiya reply automatically
    ...
    def _notify_by_email_get_headers(self, headers=None):
        ...
        if self.project_id:
            current_objects = [h for h in headers.get('X-Harpiya-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-Harpiya-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-Harpiya-Tags'] = ','.join(self.tag_ids.mapped('name'))
        return headers
    ...
    def action_redirect_to_project_task_form(self):
        ...
        return {
            'type': 'ir.actions.act_url',
            'url': f"/harpiya/1/action-project.act_project_project_2_project_task_all/{self.id}?menu_id={menu_id}",
            'target': 'new',
        }
