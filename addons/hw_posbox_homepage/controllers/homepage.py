...
from harpiya.addons.hw_drivers.tools import helpers, route, wifi
from harpiya.addons.hw_drivers.main import iot_devices
from harpiya.addons.hw_drivers.connection_manager import connection_manager
from harpiya.tools.misc import file_path
from harpiya.addons.hw_drivers.server_logger import (
    check_and_update_harpiya_config_log_to_server_option,
    get_harpiya_config_log_to_server_option,
...
class IotBoxOwlHomePage(http.Controller):
    ...
    # ---------------------------------------------------------- #
    @http.route('/hw_posbox_homepage/restart_harpiya_service', auth='none', type='http', cors='*')
    def harpiya_service_restart(self):
        helpers.harpiya_restart(0)
        return json.dumps({
            'status': 'success',
            'message': 'Harpiya service restarted',
        })

    @http.route('/hw_posbox_homepage/iot_logs', auth='none', type='http', cors='*')
    def get_iot_logs(self):
        logs_path = "/var/log/harpiya/harpiya-server.log" if platform.system() == 'Linux' else Path().absolute().parent.joinpath('harpiya.log')
    ...
    def get_homepage_data(self):
        ...
        harpiya_server_url = helpers.get_harpiya_server_url() or ''
        ...
        return json.dumps({
            ...
            'server_status': harpiya_server_url,
            'pairing_code': connection_manager.pairing_code,
            'pairing_code_expired': connection_manager.pairing_code_expired and not harpiya_server_url,
    ...
    def get_version_info(self):
        ...
        return json.dumps({
            ...
            'harpiyaIsUpToDate': current_commit == last_available_commit or not bool(helpers.get_harpiya_server_url()),
    ...
    def log_levels(self):
        ...
        return json.dumps({
            'title': "Harpiya's IoT Box - Handlers list",
            'breadcrumb': 'Handlers list',
            'drivers_list': drivers_list,
            'interfaces_list': interfaces_list,
            'server': helpers.get_harpiya_server_url(),
            'is_log_to_server_activated': get_harpiya_config_log_to_server_option(),
            'root_logger_log_level': self._get_logger_effective_level_str(logging.getLogger()),
            'harpiya_current_log_level': self._get_logger_effective_level_str(logging.getLogger('harpiya')),
        ...
        if wifi.reconnect(essid, password, force_update=True):
            ...
            res_payload = {
                ...
                'server': {
                    ...
                    'message': 'Redirect to Harpiya Server' if server else 'Redirect to IoT Box'
                }
            }
        else:
            res_payload = {
    ...
    def connect_to_harpiya_server(self, token):
        if token:
            try:
    ...
    def update_log_level(self, name, value):
        ...
        if name == 'log-to-server':
            check_and_update_harpiya_config_log_to_server_option(value)

        name = name[len(IOT_LOGGING_PREFIX):]
        if name == 'root':
            self._update_logger_level('', value, AVAILABLE_LOG_LEVELS)
        elif name == 'harpiya':
            self._update_logger_level('harpiya', value, AVAILABLE_LOG_LEVELS)
    ...
    def _update_logger_level(self, logger_name, new_level, available_log_levels, handler_folder=False):
        """Update (if necessary) Harpiya's configuration and logger to the given logger_name to the given level.
        ...
        HARPIYA_TOOL_CONFIG_HANDLER_NAME = 'log_handler'
        LOG_HANDLERS = (helpers.get_conf(HARPIYA_TOOL_CONFIG_HANDLER_NAME, section='options') or []).split(',')
        ...
        if not IS_NEW_LEVEL_PARENT:
            ...
            _logger.debug('Adding to harpiya config log_handler: %s', new_entry)
        conf[HARPIYA_TOOL_CONFIG_HANDLER_NAME] = ','.join(log_handlers_without_logger)
    ...
    def _get_iot_handler_logger(self, handler_name, handler_folder_name):
        """
        Get Harpiya Iot logger given an IoT handler name
        ...
        """
        harpiya_addon_handler_path = helpers.compute_iot_handlers_addon_name(handler_folder_name, handler_name)
        return harpiya_addon_handler_path in logging.Logger.manager.loggerDict and \
               logging.getLogger(harpiya_addon_handler_path)
