...
export class Homepage extends Component {
    ...
    async restartHarpiyaService() {
        try {
            await this.store.rpc({
                url: "/hw_posbox_homepage/restart_harpiya_service",
            });

            this.state.waitRestart = true;
        } catch {
            console.warn("Error while restarting Harpiya Service");
        }
    }
    ...
    <div t-if="!this.state.loading" class="w-100 d-flex flex-column align-items-center justify-content-center background">
        <div class="bg-white p-4 rounded overflow-auto position-relative w-100 main-container">
            <div class="position-absolute end-0 top-0 mt-3 me-4 d-flex gap-1">
                ...
                <IconButton onClick.bind="restartHarpiyaService" icon="'fa-power-off'" />
            </div>
            ...
            <SingleData t-if="!store.base.is_access_point_up" name="'Harpiya database connected'" value="state.data.server_status" icon="'fa-link'">
				<t t-set-slot="button">
					<ServerDialog />
				</t>
			</SingleData>
            <SingleData t-if="state.data.pairing_code and !this.store.base.is_access_point_up" name="'Pairing Code'" value="state.data.pairing_code + ' - Enter this code in the IoT app in your Harpiya database'" icon="'fa-code'"/>
            ...
            <div class="d-flex justify-content-center gap-2 mt-2" t-if="!store.base.is_access_point_up">
                <a href="https://www.harpiya.com/fr_FR/help" target="_blank" class="link-primary">Help</a>
                <a href="https://www.harpiya.com/documentation/master/applications/general/iot.html" target="_blank" class="link-primary">Documentation</a>
            </div>
        </div>
    </div>
