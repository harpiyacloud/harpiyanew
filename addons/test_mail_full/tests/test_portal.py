...
from harpiya.addons.auth_signup.models.res_partner import ResPartner
from harpiya.addons.mail.tests.common import MailCommon
from harpiya.addons.test_mail_full.tests.common import TestMailFullCommon
from harpiya.addons.test_mail_sms.tests.common import TestSMSRecipients
from harpiya.tests import tagged, users
from harpiya.tests.common import HttpCase
from harpiya.tools import html_escape
...
class TestPortalControllers(TestPortal):
    ...
    def test_redirect_to_records(self):
        ...
        path = url_parse(response.url).path
        self.assertEqual(path, f'/harpiya/mail.test.portal/{self.record_portal.id}')
    ...
    def test_portal_share_comment(self):
        ...
        # Only messages from the current user not HarpiyaBot
...
class TestPortalFlow(MailCommon, HttpCase):
    ...
    def test_employee_access(self):
        ...
        res = self.url_open(self.record_access_url)
        self.assertEqual(res.status_code, 200)
        self.assert_URL(res.url, f'/harpiya/mail.test.portal/{self.record_portal.id}')
