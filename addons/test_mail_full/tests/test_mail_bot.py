...
@tagged("harpiyabot")
class TestHarpiyabot(MailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.harpiyabot_ping_body = f'<a href="http://harpiya.com/harpiya/res.partner/{cls.harpiyabot.id}" class="o_mail_redirect" data-oe-id="{cls.harpiyabot.id}" data-oe-model="res.partner" target="_blank">@HarpiyaBot</a>'
    ...
    def test_fetch_listener(self):
        channel = self.user_employee.with_user(self.user_employee)._init_harpiyabot()
        harpiyabot = self.env.ref("base.partner_root")
        harpiyabot_in_fetch_listeners = self.env['discuss.channel.member'].search([('channel_id', '=', channel.id), ('partner_id', '=', harpiyabot.id)])
    ...
    def test_onboarding_flow(self):
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.harpiyabot_state, 'onboarding_attachement')
