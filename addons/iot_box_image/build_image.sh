...
if [[ "${1:-}" == "-c" || "${1:-}" == "--cleanup" ]]; then
    ...
    rm -rf overwrite_before_init/home/pi/harpiya
    rm -rfv iotbox.img
...
current_repo="$(git remote get-url $current_remote | sed 's/.*github.com[\/:]//' | sed 's/\/harpiya.git//')"
read -p "Enter repo [${current_repo}]: " REPO
REPO="https://github.com/${REPO:-$current_repo}/harpiya.git"
...
CLONE_DIR="${OVERWRITE_FILES_BEFORE_INIT_DIR}/home/pi/harpiya"
...
harpiya/
harpiya-bin" | tee --append .git/info/sparse-checkout > /dev/null
    git read-tree -mu HEAD
    git remote set-url origin "https://github.com/harpiya/harpiya.git" # ensure remote is the original repo
fi
...
# copy iotbox version
mkdir -pv "${MOUNT_POINT}"/var/harpiya/
echo "${VERSION_IOTBOX}" > "${MOUNT_POINT}"/var/harpiya/iotbox_version
