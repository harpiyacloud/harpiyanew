...
from harpiya import fields, http
...
from harpiya.addons.website_event.tests.common import TestEventOnlineCommon, OnlineEventCase
...
class TestEventRegisterUTM(HttpCase, TestEventOnlineCommon):
    def test_event_registration_utm_values(self):
        ...
        self.opener.cookies.update({
            'harpiya_utm_campaign': event_campaign.name,
            'harpiya_utm_source': self.env.ref('utm.utm_source_newsletter').name,
            'harpiya_utm_medium': self.env.ref('utm.utm_medium_email').name
        })
