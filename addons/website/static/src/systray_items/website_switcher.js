...
export class WebsiteSwitcherSystray extends Component {
    ...
    getElements() {
        return this.websiteService.websites.map((website) => ({
            ...
            callback: () => {
                if (!session.website_bypass_domain_redirect // Used by the Harpiya support (bugs to be expected)
                        && website.domain
                    ...
                    window.location.href = `${encodeURI(website.domain)}/harpiya/action-website.website_preview?path=${encodeURIComponent(path)}&website_id=${encodeURIComponent(website.id)}`;
                } else {
