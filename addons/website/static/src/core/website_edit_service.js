...
patch(Colibri.prototype, {
    protectSyncAfterAsync(interaction, name, fn) {
        ...
        return (...args) => {
            ...
            wysiwyg?.harpiyaEditor.observerUnactive(fullName);
            const result = fn(...args);
            wysiwyg?.harpiyaEditor.observerActive(fullName);
            return result;
        };
    },
    addListener(target, event, fn, options) {
        ...
        const wysiwyg = window.$?.("#wrapwrap").data("wysiwyg");
        let stealthFn = fn;
        if (wysiwyg?.harpiyaEditor && !fn.isHandler && stealth) {
            const name = `${this.interaction.constructor.name}/${event}`;
            stealthFn = async (...args) => {
                wysiwyg.harpiyaEditor.observerUnactive(name);
                const result = await fn(...args);
                wysiwyg.harpiyaEditor.observerActive(name);
                return result;
            };
        }
