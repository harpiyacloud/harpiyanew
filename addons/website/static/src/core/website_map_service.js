            ...
            async loadGMapAPI(editableMode, refetch) {
                ...
                if (refetch || !gmapAPILoading) {
                    gmapAPILoading = new Promise(async resolve => {
                        ...
                        window.harpiya_gmap_api_post_load = (async function harpiya_gmap_api_post_load() {
                            ...
                            if (!editableMode && user.isAdmin) {
                                ...
                                notification.add(
                                    markup(`<div>
                                        ...
                                        <a href="/harpiya/action-website.action_website_configuration">${escape(urlTitle)}</a>
                                    </div>`),
                        ...
                        await loadJS(`https://maps.googleapis.com/maps/api/js?v=3.exp&libraries=places&callback=harpiya_gmap_api_post_load&key=${encodeURIComponent(key)}`);
                    });
                }
