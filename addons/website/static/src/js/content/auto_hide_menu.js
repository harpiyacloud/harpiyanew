...
async function autoHideMenu(el, options) {
    ...
    function _adapt() {
        ...
        const harpiyaEditor = wysiwyg && wysiwyg.harpiyaEditor;
        if (harpiyaEditor) {
            harpiyaEditor.observerUnactive("adapt");
            harpiyaEditor.withoutRollback(__adapt);
            harpiyaEditor.observerActive("adapt");
            return;
        }
        __adapt();
    }
