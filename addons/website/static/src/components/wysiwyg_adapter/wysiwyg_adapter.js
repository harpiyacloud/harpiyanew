...
import { isMediaElement } from '@web_editor/js/editor/harpiya-editor/src/utils/utils';
...
import { Component, onWillStart, useEffect, onWillUnmount } from "@harpiya/owl";
...
export class WysiwygAdapterComponent extends Wysiwyg {
    ...
    async startEdition() {
        ...
        const webEditorFilterMutationRecords = this.harpiyaEditor.options.filterMutationRecords;
        Object.assign(this.harpiyaEditor.options, {
            /**
             * @override
             */
        ...
        // Disable HarpiyaEditor observer's while setting up classes
        this.harpiyaEditor.observerUnactive();
        ...
        // the dirty observer (not to be confused with harpiyaEditor
        // observer) as the widgets might trigger DOM mutations.
        this._setObserver();
        this.harpiyaEditor.observerActive();
    }
    /**
    ...
    async destroy() {
        ...
        const mod = await harpiya.loader.modules.get('@website/snippets/s_social_media/options')[Symbol.for('default')];
        mod.clearDbSocialValuesCache();

        const formOptionsMod = await harpiya.loader.modules.get('@website/snippets/s_website_form/options')[Symbol.for('default')];
    ...
    _setObserver() {
        const processRecords = (records) => {
            records = this.harpiyaEditor.filterMutationRecords(records);
        ...
        this.harpiyaEditor.addEventListener('observerUnactive', () => {
            if (this.observer) {
                processRecords(this.observer.takeRecords());
                this.observer.disconnect();
            }
        });
        this.harpiyaEditor.addEventListener('observerActive', this._observe.bind(this));
    }
    /**
     * @private
     */
    ...
    _addEditorMessages() {
        ...
        for (let htmlEl of $wrap.not("[placeholder]").filter('[data-oe-sanitize="no_block"]')) {
            ...
            if (htmlEl.innerText.trim().length === 0) {
                ...
                // It is removed by HarpiyaEditor when text is entered.
    ...
    async getSnippetsMenuClass() {
        const snippetsEditor = await harpiya.loader.modules.get('@website/js/editor/snippets.editor')[Symbol.for('default')];
    ...
    async _onSaveRequest(event) {
        ...
        if (event.data.reload || event.data.reloadEditor) {
            ...
            callback = () => redirect(`/harpiya/action-website.website_preview?website_id=${encodeURIComponent(websiteId)}&path=${currentPath}&enable_editor=1`);
