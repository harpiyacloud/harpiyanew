...
export class WebsitePreview extends Component {
    ...
    setup() {
        ...
        onMounted(() => {
            ...
            // HarpiyaFrameContentLoaded event to unblock the iframe, as it is
            // triggered faster than the load event.
            this.iframe.el.addEventListener('HarpiyaFrameContentLoaded', () => this.websiteService.unblockPreview('load-iframe'), { once: true });
        });
        ...
        /**
         * This removes the 'Harpiya' prefix of the title service to display
        ...
        useEffect(() => {
            this.iframe.el.addEventListener('HarpiyaFrameContentLoaded', toggleIsMobile);
            return () => this.iframe.el.removeEventListener('HarpiyaFrameContentLoaded', toggleIsMobile);
        }, () => []);
    }
    ...
    reloadIframe(url) {
        return new Promise((resolve, reject) => {
            this.websiteService.websiteRootInstance = undefined;
            this.iframe.el.addEventListener('HarpiyaFrameContentLoaded', resolve, { once: true });
            if (url) {
    ...
    _isTopWindowURL({ host, pathname }) {
        const backendRoutes = ['/web', '/web/session/logout', '/harpiya'];
        return host !== window.location.host
            || (pathname
                && (backendRoutes.includes(pathname)
                    || pathname.startsWith('/@/')
                    || pathname.startsWith('/harpiya/')
    ...
    /**
     * This replaces the browser url (/harpiya/action-website...) with
    ...
    _replaceBrowserUrl() {
        if (!wUtils.isHTTPSorNakedDomainRedirection(this.iframe.el.contentWindow.location.origin, window.location.origin)) {
            ...
            // trigger a traceback.
            history.replaceState(history.state, document.title, '/harpiya');
            return;
        }
    ...
    _onKeydownRefresh(ev) {
        ...
        redirect(`/harpiya/action-website.website_preview?path=${encodeURIComponent(path)}${debugMode}`);
    }
}
