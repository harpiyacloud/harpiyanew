...
from harpiya.addons.base.tests.common import HttpCaseWithUserPortal
from harpiya.addons.website.controllers.form import WebsiteForm
from harpiya.addons.website.tools import MockRequest
from harpiya.tests.common import tagged, TransactionCase
...
class TestWebsiteFormEditor(HttpCaseWithUserPortal):
    ...
    def test_website_form_contact_us_edition_with_email(self):
        self.start_tour('/harpiya', 'website_form_contactus_edition_with_email', login="admin")
    ...
    def test_website_form_contact_us_edition_no_email(self):
        ...
        self.start_tour('/harpiya', 'website_form_contactus_edition_no_email', login="admin")
        self.start_tour('/contactus', 'website_form_contactus_submit', login="portal")
        mail = self.env['mail.mail'].search([], order='id desc', limit=1)
        self.assertEqual(
            mail.email_to,
    ...
    def test_website_form_html_escaping(self):
        ...
        with MockRequest(self.env, website=website):
            WebsiteFormController.insert_record(
                ...
                {'email_from': 'harpiyabot@example.com', 'subject': 'John <b>Smith</b>', 'email_to': 'company@company.company'},
                "John <b>Smith</b>",
            )
            mail = self.env['mail.mail'].search([], order='id desc', limit=1)
