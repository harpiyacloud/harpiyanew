...
class TestWebsiteAssets(harpiya.tests.HttpCase):
    ...
    def test_01_multi_domain_assets_generation(self):
        ...
        self.url_open(domain_1 + '/harpiya')
        check_asset()
        self.url_open(domain_2 + '/harpiya')
        check_asset()
        self.url_open(domain_1 + '/harpiya')
        check_asset()
        self.url_open(domain_2 + '/harpiya')
...
class TestWebAssets(harpiya.tests.HttpCase):
    def test_assets_url_validation(self):
        website_id = self.env['website'].search([], limit=1, order='id desc').id

        with harpiya.tools.mute_logger('harpiya.addons.web.controllers.binary'):
            self.assertEqual(
