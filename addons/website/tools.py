...
import harpiya.http
from harpiya.tools.misc import hmac, DotDict, frozendict
...
def MockRequest(
    ...
    from harpiya.tests.common import HttpCase  # noqa: PLC0415
    ...
    if HttpCase.http_port():
        ...
        base_url = f"http://{HOST}:{harpiya.tools.config['http_port']}"
    request = Mock(
        # request
        httprequest=Mock(
            host='localhost',
            path=path,
            app=harpiya.http.root,
            environ=dict(
                EnvironBuilder(
                    path=path,
                    base_url=base_url,
        ...
        type='http',
        future_response=harpiya.http.FutureResponse(),
        params={},
        redirect=env['ir.http']._redirect,
        session=DotDict(
            harpiya.http.get_default_session(),
            context={'lang': ''},
        ),
        geoip=harpiya.http.GeoIP('127.0.0.1'),
    ...
    if country_code:
        try:
            request.geoip._city_record = harpiya.http.geoip2.models.City(['en'], country={'iso_code': country_code})
        except TypeError:
            request.geoip._city_record = harpiya.http.geoip2.models.City({'country': {'iso_code': country_code}})
    ...
    #
    #   router = harpiya.http.root.get_db_router()
    ...
    with contextlib.ExitStack() as s:
        ...
        s.callback(harpiya.http._request_stack.pop)
        s.enter_context(patch('harpiya.http.root.get_db_router', router))
