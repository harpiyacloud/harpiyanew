...
xml_id_url_prefix = "harpiya://ir_menu_xml_id/"

harpiya_view_link_prefix = "harpiya://view/"


def harpiya_charts(data):
    """return all harpiya chart definition in the spreadsheet"""
    figures = []
    for sheet in data.get("sheets", []):
        figures += [
            dict(figure["data"], id=figure["id"])
            for figure in sheet.get("figures", [])
            if figure["tag"] == "chart" and figure["data"]["type"].startswith("harpiya_")
        ]
    return figures
...
def harpiya_view_links(data):
    """return all view definitions embedded in link cells.
    urls looks like harpiya://view/{... view data...}
    """
    return [
        json.loads(url[len(harpiya_view_link_prefix):])
        for url in links_urls(data)
        if url.startswith(harpiya_view_link_prefix)
    ]
...
def filter_fields(data):
    ...
    charts = harpiya_charts(data)
    harpiya_version = data.get("harpiyaVersion", 1)
    if harpiya_version < 5:
        ...
        for pivot in data["pivots"].values():
            if pivot.get("type", "HARPIYA") == "HARPIYA":
                model = pivot["model"]
...
def fields_in_spreadsheet(data):
    ...
    harpiya_pivots = (pivot for pivot in data.get("pivots", dict()).values() if pivot.get("type", "HARPIYA") == "HARPIYA")
    all_fields = chain(
        extract_fields(list_fields, data.get("lists", dict()).values()).items(),
        extract_fields(pivot_fields, harpiya_pivots).items(),
        extract_fields(chart_fields, harpiya_charts(data)).items(),
        extract_fields(harpiya_view_fields, harpiya_view_links(data)).items(),
...
def menus_xml_ids_in_spreadsheet(data):
    ...
    return set(data.get("chartHarpiyaMenusReferences", {}).values()) | {
