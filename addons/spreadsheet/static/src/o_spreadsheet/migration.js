...
export const HARPIYA_VERSION = 13;

const MAP_V1 = {
    PIVOT: "HARPIYA.PIVOT",
    "PIVOT.HEADER": "HARPIYA.PIVOT.HEADER",
    "PIVOT.POSITION": "HARPIYA.PIVOT.POSITION",
    "FILTER.VALUE": "HARPIYA.FILTER.VALUE",
    LIST: "HARPIYA.LIST",
    "LIST.HEADER": "HARPIYA.LIST.HEADER",
};

const MAP_FN_NAMES_V10 = {
    "HARPIYA.PIVOT": "PIVOT.VALUE",
    "HARPIYA.PIVOT.HEADER": "PIVOT.HEADER",
    "HARPIYA.PIVOT.TABLE": "PIVOT",
};

const dmyRegex = /^([0|1|2|3][1-9])\/(0[1-9]|1[0-2])\/(\d{4})$/i;

migrationStepRegistry.add("harpiya_migration", {
    versionFrom: "16.1",
    migrate(data) {
        return migrateHarpiyaData(data);
    },
});
migrationStepRegistry.add("harpiya_migration_sorted_column", {
    versionFrom: "24.1",
    migrate(data) {
        const version = data.harpiyaVersion || 0;
        if (version < 13) {
...
function migrateHarpiyaData(data) {
    const version = data.harpiyaVersion || 0;
    if (version < 1) {
...
function migratePivotDaysParameters(formulaString) {
    ...
    const convertedAst = convertAstNodes(ast, "FUNCALL", (ast) => {
        if (["HARPIYA.PIVOT", "HARPIYA.PIVOT.HEADER"].includes(ast.value.toUpperCase())) {
...
/**
 * Migrate the pivot data to add the type, by default "HARPIYA". And replace the
...
function migrate11to12(data) {
    // remove the calls to HARPIYA.PIVOT.POSITION and replace
    ...
    for (const sheet of data.sheets || []) {
        for (const xc in sheet.cells || []) {
            ...
            if (
                ...
                cell.content.includes("HARPIYA.PIVOT.POSITION")
            ) {
                const tokens = tokenize(cell.content);
                /* given that harpiya.pivot.position is automatically set, we know that:
                1) it is always on the form of HARPIYA.PIVOT.POSITION(1, ...)
                2) it is always preceded by a dimension of a pivot or header, inside another pivot formula
                3) there is only one harpiya.pivot.position per cell
                4) harpiya.pivot.position can only exist after the 3rd token and needs at least 7 tokens to be valid*/
                for (let i = 2; i < tokens.length - 7; i++) {
                    const token = tokens[i];
                    if (
                        token.type === "SYMBOL" &&
                        token.value.toUpperCase() === "HARPIYA.PIVOT.POSITION"
                    ) {
                        const order = tokens[i + 6];
                        tokens[i - 2].value = '"#' + tokens[i - 2].value.slice(1); // "dimension" becomes "#dimension"
                        tokens.splice(i, 7); // remove "HARPIYA.PIVOT.POSITION", "(", "1", ",", "dimension", ", ", order
                        // tokens[i-1] is the comma before harpiya.pivot.position
                        tokens[i] = order;
                        cell.content = tokensToString(tokens);
                    }
                }
...
/**
 * Change pivot sortedColumn of the harpiya's pivot model to the sortedColumn of o-spreadsheet
 */
...
export class HarpiyaVersion extends HarpiyaCorePlugin {
    static getters = /** @type {const} */ ([]);

    export(data) {
        data.harpiyaVersion = HARPIYA_VERSION;
    }
}

corePluginRegistry.add("harpiyaMigration", HarpiyaVersion);
