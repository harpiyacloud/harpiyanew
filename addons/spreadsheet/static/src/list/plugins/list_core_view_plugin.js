...
import { HarpiyaCoreViewPlugin } from "@spreadsheet/plugins";
...
export class ListCoreViewPlugin extends HarpiyaCoreViewPlugin {
    ...
    handle(cmd) {
        switch (cmd.type) {
            ...
            case "SET_GLOBAL_FILTER_VALUE":
                this._addDomains();
                break;
            case "UPDATE_HARPIYA_LIST":
            case "UPDATE_HARPIYA_LIST_DOMAIN": {
            ...
            case "REDO": {
                ...
                const updateCommands = cmd.commands.filter(
                    (cmd) =>
                        cmd.type === "UPDATE_HARPIYA_LIST_DOMAIN" ||
                        cmd.type === "UPDATE_HARPIYA_LIST" ||
                        cmd.type === "INSERT_HARPIYA_LIST"
                );
     ...
     */
    _refreshHarpiyaLists() {
        for (const listId of this.getters.getListIds()) {
            this._refreshHarpiyaList(listId);
        }
    }
