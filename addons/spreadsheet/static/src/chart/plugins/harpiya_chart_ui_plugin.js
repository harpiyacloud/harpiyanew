...
export class HarpiyaChartUIPlugin extends HarpiyaUIPlugin {
    static getters = /** @type {const} */ (["getChartDataSource", "getHarpiyaEnv"]);
    ...
    constructor(config) {
        ...
        globalFiltersFieldMatchers["chart"] = {
            ...
            waitForReady: () => this._getHarpiyaChartsWaitForReady(),
    ...
    beforeHandle(cmd) {
        switch (cmd.type) {
            case "START":
                for (const chartId of this.getters.getHarpiyaChartIds()) {
        ...
        switch (cmd.type) {
            ...
            case "REDO": {
                ...
                for (const cmd of domainEditionCommands) {
                    if (!this.getters.getHarpiyaChartIds().includes(cmd.id)) {
                        continue;
                    }
                    const dataSource = this.getChartDataSource(cmd.id);
                    if (
    ...
    getChartDataSource(chartId) {
        const dataSourceId = this._getHarpiyaChartDataSourceId(chartId);
     ...
     * @return {Promise[]}
     */
    _getHarpiyaChartsWaitForReady() {
        return this.getters
            .getHarpiyaChartIds()
            .map((chartId) => this.getChartDataSource(chartId).loadMetadata());
    }

    _getHarpiyaChartDataSourceId(chartId) {
     ...
     */
    _refreshHarpiyaCharts() {
        for (const chartId of this.getters.getHarpiyaChartIds()) {
            this._refreshHarpiyaChart(chartId);
        }
    }
}
