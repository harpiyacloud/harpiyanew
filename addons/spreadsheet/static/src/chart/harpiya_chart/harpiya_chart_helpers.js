...
export function onHarpiyaChartItemClick(getters, chart) {
    return navigateInHarpiyaMenuOnClick(getters, chart, (chartJsItem) => {
...
export function onWaterfallHarpiyaChartItemClick(getters, chart) {
    return navigateInHarpiyaMenuOnClick(getters, chart, (chartJsItem) => {
...
function navigateInHarpiyaMenuOnClick(getters, chart, getDomainFromChartItem) {
    return async (event, items) => {
        const env = getters.getHarpiyaEnv();
        if (!items.length) {
            return;
        }
        if (!env) {
...
export function onHarpiyaChartItemHover() {
    return (event, items) => {
        if (items.length > 0) {
