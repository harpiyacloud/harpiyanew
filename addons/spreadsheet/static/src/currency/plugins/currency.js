import { EvaluationError, helpers, registries } from "@harpiya/o-spreadsheet";
...
export class CurrencyPlugin extends HarpiyaUIPlugin {
    ...
    constructor(config) {
        ...
        this._serverData = config.custom.harpiyaDataProvider?.serverData;
    }

    get serverData() {
        if (!this._serverData) {
            throw new Error(
                "'serverData' is not defined, please make sure a 'HarpiyaDataProvider' instance is provided to the model."
            );
        }
...
featurePluginRegistry.add("harpiyaCurrency", CurrencyPlugin);
