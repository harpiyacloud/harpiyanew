...
import { parse, helpers, iterateAstNodes } from "@harpiya/o-spreadsheet";
import { isLoadingError } from "@spreadsheet/o_spreadsheet/errors";
import { HarpiyaSpreadsheetModel } from "@spreadsheet/model";
...
import {
    ...
    isIrMenuXmlUrl,
} from "@spreadsheet/ir_ui_menu/harpiya_menu_link_cell";
...
export function createSpreadsheetModel({ env, data, revisions }) {
    const harpiyaDataProvider = new HarpiyaDataProvider(env);
    const model = new HarpiyaSpreadsheetModel(data, { custom: { harpiyaDataProvider } }, revisions);
    return model;
}

/**
 * @param {HarpiyaSpreadsheetModel} model
 */
export async function waitForHarpiyaSources(model) {
    ...
    promises.push(
        ...model.getters
            .getPivotIds()
            .filter((pivotId) => model.getters.getPivotCoreDefinition(pivotId).type === "HARPIYA")
...
export async function waitForDataLoaded(model) {
    await waitForHarpiyaSources(model);
    const harpiyaDataProvider = model.config.custom.harpiyaDataProvider;
    if (!harpiyaDataProvider) {
        return;
    }
    await new Promise((resolve, reject) => {
        function check() {
            ...
            if (isLoaded(model)) {
                harpiyaDataProvider.removeEventListener("data-source-updated", check);
                resolve();
            }
        }
        harpiyaDataProvider.addEventListener("data-source-updated", check);
        check();
    });
}

function containsLinkToHarpiya(link) {
    if (link && link.url) {
        return (
 ...
 */
export async function freezeHarpiyaData(model) {
    ...
    for (const sheet of Object.values(data.sheets)) {
        ...
        for (const [xc, content] of Object.entries(sheet.cells)) {
            ...
            if (containsHarpiyaFunction(content)) {
                const pivotId = model.getters.getPivotIdFromPosition(position);
                if (pivotId && model.getters.getPivotCoreDefinition(pivotId).type !== "HARPIYA") {
                    continue;
                }
            ...
            if (containsLinkToHarpiya(evaluatedCell.link)) {
        ...
        for (const figure of sheet.figures) {
            if (
                figure.tag === "chart" &&
                (figure.data.type.startsWith("harpiya_") || figure.data.type === "geo")
            ) {
                const img = harpiyaChartToImage(model, figure);
                figure.tag = "image";
                figure.data = {
                    path: img,
    ...
    if (data.pivots) {
        data.pivots = Object.fromEntries(
            Object.entries(data.pivots).filter(([id, def]) => def.type !== "HARPIYA")
        );
    }
    data.lists = {};
 ...
 * @returns {boolean}
 */
function containsHarpiyaFunction(content) {
    if (
        !content ||
        !content.startsWith("=") ||
        (!content.toUpperCase().includes("HARPIYA.") &&
    ...
    try {
        ...
        return iterateAstNodes(ast).some(
            (ast) =>
                ast.type === "FUNCALL" &&
                (ast.value.toUpperCase().startsWith("HARPIYA.") ||
 ...
 * @returns {string}
 */
function harpiyaChartToImage(model, figure) {
