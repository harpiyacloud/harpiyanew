...
export class PivotCoreViewGlobalFilterPlugin extends HarpiyaCoreViewPlugin {
     ...
     */
    _addDomain(pivotId) {
        if (this.getters.getPivotCoreDefinition(pivotId).type !== "HARPIYA") {
            return;
        }
        const domainList = [];
    ...
    _addDomains() {
        for (const pivotId of this.getters
            .getPivotIds()
            .filter((pivotId) => this.getters.getPivot(pivotId).type === "HARPIYA")) {
    ...
    _getPivotsWaitForReady() {
        return this.getters
            ...
            .filter((pivot) => pivot.type === "HARPIYA")
