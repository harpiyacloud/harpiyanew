...
import { freezeHarpiyaData } from "@spreadsheet/helpers/model";
import { HarpiyaPivot, HarpiyaPivotRuntimeDefinition } from "@spreadsheet/pivot/harpiya_pivot";
...
test("harpiya pivot functions are replaced with their value", async function () {
    ...
    const data = await freezeHarpiyaData(model);
...
test("Pivot with a type different of HARPIYA is not converted", async function () {
    // Add a pivot with a type different of HARPIYA
...
test("harpiya pivot functions detection is not case sensitive", async function () {
    const { model } = await createSpreadsheetWithPivot();
    setCellContent(model, "A1", '=pivot.value(1,"probability:avg")');
    const data = await freezeHarpiyaData(model);
...
test("harpiya charts are replaced with an image", async function () {
...
test("harpiya links are replaced with their label", async function () {
    const view = {
        name: "an harpiya view",
        viewType: "list",
        action: {
            modelName: "partner",
            views: [[false, "list"]],
    ...
    const data = {
        sheets: [
            {
                cells: {
                    A1: "[menu_xml](harpiya://ir_menu_xml_id/test_menu)",
                    A2: "[menu_id](harpiya://ir_menu_id/12)",
                    A3: `[harpiya_view](harpiya://view/${JSON.stringify(view)})`,
                    A4: "[external_link](https://harpiya.com)",
    ...
    const frozenData = await freezeHarpiyaData(model);
    expect(frozenData.sheets[0].cells.A1).toBe("menu_xml");
    expect(frozenData.sheets[0].cells.A2).toBe("menu_id");
    expect(frozenData.sheets[0].cells.A3).toBe("harpiya_view");
    expect(frozenData.sheets[0].cells.A4).toBe("[external_link](https://harpiya.com)");
