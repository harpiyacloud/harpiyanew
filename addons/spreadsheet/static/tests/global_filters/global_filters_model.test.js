...
import { animationFrame, mockDate, mockTimeZone } from "@harpiya/hoot-mock";

import { DispatchResult, Model, helpers, tokenize } from "@harpiya/o-spreadsheet";
...
test("Adding new DataSource will set its fieldMatching according to other ones with the same model", async function () {
    ...
    fieldMatching = model.getters.getHarpiyaChartFieldMatching(chartId, filterId);
    expect(fieldMatching.chain).toBe("created_on");
    expect(fieldMatching.type).toBe("date");
    expect(fieldMatching.offset).toBe(undefined);
});
...
test("HARPIYA.FILTER.VALUE text filter", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=HARPIYA.FILTER.VALUE("Text Filter")`);
...
test("HARPIYA.FILTER.VALUE date filter", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A10", `=HARPIYA.FILTER.VALUE("Date Filter")`);
    await animationFrame();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
...
test("HARPIYA.FILTER.VALUE date from/to without values", async function () {
    const { model } = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
    ...
    setCellContent(model, "A1", `=HARPIYA.FILTER.VALUE("Date Filter")`);
    expect(getEvaluatedCell(model, "A1").value).toBe("");
    expect(getEvaluatedCell(model, "B1").value).toBe("");
});

test("HARPIYA.FILTER.VALUE date from/to with only from defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=HARPIYA.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("HARPIYA.FILTER.VALUE date from/to with only to defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=HARPIYA.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("HARPIYA.FILTER.VALUE date from/to with from and to defined", async function () {
    const { model } = await createModelWithDataSource();
    setCellContent(model, "A1", `=HARPIYA.FILTER.VALUE("Date Filter")`);
    await addGlobalFilter(model, {
        id: "42",
...
test("HARPIYA.FILTER.VALUE relation filter", async function () {
    const { model } = await createModelWithDataSource({
    ...
    setCellContent(model, "A10", `=HARPIYA.FILTER.VALUE("Relation Filter")`);
    await animationFrame();
    await addGlobalFilter(model, {
        id: "42",
        type: "relation",
...
test("HARPIYA.FILTER.VALUE with escaped quotes in the filter label", async function () {
    const { model } = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "text",
    ...
    setCellContent(model, "A1", '=HARPIYA.FILTER.VALUE("my \\"special\\" filter")');
    expect(getCellValue(model, "A1")).toBe("Jean-Jacques");
});

test("HARPIYA.FILTER.VALUE formulas are updated when filter label is changed", async function () {
    const { model } = await createModelWithDataSource();
    await addGlobalFilter(model, {
        id: "42",
        type: "date",
    ...
    setCellContent(
        model,
        "A10",
        `=HARPIYA.FILTER.VALUE("Cuillère") & HARPIYA.FILTER.VALUE( "Cuillère" )`
    );
    const [filter] = model.getters.getGlobalFilters();
    const newFilter = {
        ...filter,
    ...
    expect(getCellFormula(model, "A10")).toBe(
        `=HARPIYA.FILTER.VALUE("Interprete") & HARPIYA.FILTER.VALUE("Interprete")`
    );
});
...
test("field matching is removed when list is deleted", async function () {
    ...
    model.dispatch("REMOVE_HARPIYA_LIST", { listId });
...
test("field matching is removed when an Harpiya chart is deleted", async function () {
    ...
    expect(model.getters.getHarpiyaChartFieldMatching(chartId, filter.id)).toEqual(matching);
    model.dispatch("DELETE_FIGURE", { id: chartId, sheetId });
    expect(() => model.getters.getHarpiyaChartFieldMatching(chartId, filter.id)).toThrow(undefined, {
...
test("Updating the pivot domain should keep the global filter domain", async () => {
    ...
    model.dispatch("UPDATE_HARPIYA_PIVOT_DOMAIN", {
        pivotId,
        domain: [["foo", "in", [55]]],
    });
...
test("Updating a non-harpiya pivot should not crash on global filter", async () => {
    const grid = {
        A1: "Customer",
        B1: "Price",
        C1: `=PIVOT(1)`,
