...
test("click a web link", async () => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: "[Harpiya](https://harpiya.com)" },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
    const cell = getEvaluatedCell(model, "A1");
    expect(urlRepresentation(cell.link, model.getters)).toBe("https://harpiya.com");
    openLink(cell.link, env);
    expect.verifySteps(["https://harpiya.com"]);
});
...
test("click a menu link", async () => {
    ...
    const data = {
        sheets: [
            {
                cells: { A1: "[label](harpiya://ir_menu_xml_id/test_menu)" },
            },
        ],
    };
    const model = new Model(data, { custom: { env } });
...
test("click a menu link [2]", async () => {
    ...
    setCellContent(model, "A1", `[a view](harpiya://view/${JSON.stringify(view)})`);
    const cell = getEvaluatedCell(model, "A1");
    expect(urlRepresentation(cell.link, model.getters)).toBe("an harpiya view");
    openLink(cell.link, env);
...
test("Click a link containing an action xml id", async () => {
    ...
    const model = new Model({}, { custom: { env } });
    setCellContent(model, "A1", `[an action link](harpiya://view/${JSON.stringify(view)})`);
    const cell = getEvaluatedCell(model, "A1");
