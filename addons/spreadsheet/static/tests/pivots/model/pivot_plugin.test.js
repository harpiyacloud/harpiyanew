        ...
        sheets: [
            {
                id: "sheet1",
                cells: {
                    A1: '=HARPIYA.PIVOT(1, "probability")',
                },
            },
        ],
        pivots: {
...
test("evaluates only once when two pivots are loading", async function () {
    ...
    model.config.custom.harpiyaDataProvider.addEventListener("data-source-updated", () =>
...
test("can edit pivot domain with UPDATE_HARPIYA_PIVOT_DOMAIN", async () => {
    const { model } = await createSpreadsheetWithPivot();
    const [pivotId] = model.getters.getPivotIds();
