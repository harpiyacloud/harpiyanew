...
import { HarpiyaBarChart } from "@spreadsheet/chart/harpiya_chart/harpiya_bar_chart";
import { HarpiyaChart } from "@spreadsheet/chart/harpiya_chart/harpiya_chart";
import { HarpiyaLineChart } from "@spreadsheet/chart/harpiya_chart/harpiya_line_chart";
...
test("Can add an Harpiya Bar chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "harpiya_bar" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof HarpiyaBarChart).toBe(true);
...
test("Can add an Harpiya Line chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "harpiya_line" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof HarpiyaLineChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
    expect(model.getters.getChartRuntime(chartId).chartJsConfig.type).toBe("line");
});

test("Can add an Harpiya Pie chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "harpiya_pie" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof HarpiyaChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
...
test("Harpiya bar chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "harpiya_bar",
...
test("Harpiya pie chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "harpiya_pie",
        mockRPC: async function (route, args) {
            if (args.method === "web_read_group") {
...
test("Harpiya line chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "harpiya_line",
        mockRPC: async function (route, args) {
            if (args.method === "web_read_group") {
...
test("Can import/export an Harpiya chart", async () => {
    const { model } = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "harpiya_line");
    const data = model.exportData();
    const figures = data.sheets[0].figures;
    expect(figures.length).toBe(1);
    const figure = figures[0];
    expect(figure.tag).toBe("chart");
    expect(figure.data.type).toBe("harpiya_line");
...
test("Can undo/redo an Harpiya chart creation", async () => {
    const { model } = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "harpiya_line");
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("charts with no legend", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "harpiya_pie" });
    insertChartInSpreadsheet(model, "harpiya_bar");
    insertChartInSpreadsheet(model, "harpiya_line");
    const sheetId = model.getters.getActiveSheetId();
...
test("Can copy/paste Harpiya chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "harpiya_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[1]) instanceof HarpiyaChart).toBe(true);
...
test("Can cut/paste Harpiya chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "harpiya_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[0]) instanceof HarpiyaChart).toBe(true);
    expect(JSON.stringify(model.getters.getChartRuntime(chartIds[0]))).toBe(
        JSON.stringify(chartRuntime)
    );
});

test("Duplicating a sheet correctly duplicates Harpiya chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "harpiya_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Load harpiya chart spreadsheet with models that cannot be accessed", async function () {
...
test("Can insert harpiya chart from a different model", async () => {
    const { model } = await createModelWithDataSource();
...
test("Harpiya chart legend color changes with background color update", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "harpiya_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
...
test("Remove harpiya chart when sheet is deleted", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "harpiya_line" });
    const sheetId = model.getters.getActiveSheetId();
    ...
    expect(model.getters.getHarpiyaChartIds().length).toBe(1);
    model.dispatch("DELETE_SHEET", { sheetId });
    expect(model.getters.getHarpiyaChartIds().length).toBe(0);
});

test("Harpiya chart datasource display name has a default when the chart title is empty", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "harpiya_line" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
    expect(model.getters.getHarpiyaChartDisplayName(chartId)).toBe("(#1) Partners");
    model.dispatch("UPDATE_CHART", {
        definition: {
            ...definition,
            title: { text: "" },
    ...
    expect(model.getters.getHarpiyaChartDisplayName(chartId)).toBe("(#1) Harpiya Line Chart");
});
...
test("Harpiya line and bar charts display only horizontal grid lines", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "harpiya_line",
    });
    const sheetId = model.getters.getActiveSheetId();
...
test("Can configure the chart datasets", async () => {
    ...
    const { model } = await createSpreadsheetWithChart({
        type: "harpiya_bar",
        serverData,
        definition: { type: "harpiya_bar", metaData, searchParams, id: "42" },
    });
    await waitForDataLoaded(model);
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
