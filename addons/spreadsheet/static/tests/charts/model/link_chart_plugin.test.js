...
test("Links between charts and ir.menus are correctly imported/exported", async function () {
    ...
    expect(exportedData.chartHarpiyaMenusReferences[chartId]).toBe(1, {
        message: "Link to harpiya menu is exported",
    });
    const importedModel = new Model(exportedData, { custom: { env } });
    const chartMenu = importedModel.getters.getChartHarpiyaMenu(chartId);
    expect(chartMenu.id).toBe(1, { message: "Link to harpiya menu is imported" });
});

test("Can undo-redo a LINK_HARPIYA_MENU_TO_CHART", async function () {
    const env = await makeMockEnv();
    const model = new Model({}, { custom: { env } });
    createBasicChart(model, chartId);
    model.dispatch("LINK_HARPIYA_MENU_TO_CHART", {
    ...
    expect(model.getters.getChartHarpiyaMenu(chartId)).toBe(undefined);
...
test("Links of Harpiya charts are duplicated when duplicating a sheet", async function () {
    ...
    expect(model.getters.getChartHarpiyaMenu(newChartId)).toEqual(
        model.getters.getChartHarpiyaMenu(chartId)
    );
});
