...
from harpiya import api, fields, models, _, Command, SUPERUSER_ID, modules, tools
from harpiya.tools.sql import column_exists, create_column
from harpiya.addons.account.tools import format_structured_reference_iso
from harpiya.exceptions import UserError, ValidationError, AccessError, RedirectWarning
from harpiya.osv import expression
from harpiya.tools.misc import clean_context
...
from harpiya.tools.mail import email_re, email_split, is_html_empty, generate_tracking_message_id
from harpiya.tools.misc import StackMap
...
class AccountMove(models.Model):
    ...
    quick_edit_total_amount = fields.Monetary(
        ...
        help='Use this field to encode the total amount of the invoice.\n'
             'Harpiya will automatically create one invoice line with default values to match it.',
    )
    ...
    def _get_invoice_reference_number_invoice(self):
        ...
        """
        ref = self._get_invoice_reference_harpiya_invoice() or ''
    ...
    def _get_invoice_reference_number_partner(self):
        ...
        """
        ref = self._get_invoice_reference_harpiya_partner()
        return ''.join(char for char in ref if char.isdigit())
    ...
    def _get_fields_to_copy_recurring_entries(self, values):
        ...
        values.update({
            ...
            'invoice_user_id': self.invoice_user_id.id,  # otherwise user would be HarpiyaBot
        })
