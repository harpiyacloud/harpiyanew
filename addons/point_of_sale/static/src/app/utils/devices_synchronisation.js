...
export default class DevicesSynchronisation {
    ...
    async dispatch(data) {
        ...
        await this.pos.data.call("pos.config", "notify_synchronisation", [
            harpiya.pos_config_id,
            harpiya.pos_session_id,
            harpiya.login_number,
            recordIds,
        ]);
    }
    ...
    async collect(data) {
        ...
        if (harpiya.debug === "assets") {
            ...
            console.info("Login number", harpiya.login_number, login_number);
            console.info("Session Ids", harpiya.pos_session_id, session_id);
        }

        if (login_number == harpiya.login_number || session_id != harpiya.pos_session_id) {
            return;
        }
    ...
    constructOrdersDomain(serverOpenOrders) {
        ...
        domain = Domain.and([domain, new Domain([["config_id", "=", harpiya.pos_config_id]])]);
