...
import { markRaw } from "@harpiya/owl";
...
export class PosData extends Reactive {
    ...
    initializeWebsocket() {
        this.onNotified = getOnNotified(this.bus, harpiya.access_token);
    }
    ...
    get databaseName() {
        return `config-id_${harpiya.pos_config_id}_${harpiya.access_token}`;
    }

    get serverDateKey() {
        return `data_server_date_${harpiya.pos_config_id}`;
    }
    ...
    async loadInitialData() {
        ...
        if (
            ...
            session?.id !== harpiya.pos_session_id ||
            harpiya.from_backend
        ) {
            try {
                const limitedLoading = this.isLimitedLoading();
                const serverDate = localData["pos.session"]?.[0]?._data_server_date;
                const lastConfigChange = DateTime.fromSQL(harpiya.last_data_change);
                ...
                const data = await this.orm.call(
                    "pos.session",
                    "load_data",
                    [harpiya.pos_session_id, PosData.modelToLoad],
                    {
                        context: {
    ...
    async loadFieldsAndRelations() {
        const key = `pos_data_params_${harpiya.pos_config_id}`;
        if (!navigator.onLine) {
