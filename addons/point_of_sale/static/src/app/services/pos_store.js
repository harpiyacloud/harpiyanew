...
import { markRaw, reactive } from "@harpiya/owl";
...
export class PosStore extends WithLazyGetterTrap {
    ...
    get session() {
        return this.data.models["pos.session"].get(harpiya.pos_session_id);
    }
    ...
    async processServerData() {
        ...
        this.models["pos.session"].getFirst().login_number = parseInt(harpiya.login_number);
    ...
    async getNextOrderRefs(order) {
        try {
            const [pos_reference, sequence_number, tracking_number] = await this.data.call(
                "pos.session",
                "get_next_order_refs",
                [[this.session.id], parseInt(harpiya.login_number, 10), null, ""]
            );
    ...
    getNextOrderRefsLocal(refPrefix, order) {
        ...
        const LL = (harpiya.login_number % 100).toString().padStart(2, "0");
    ...
    getSyncAllOrdersContext(orders, options = {}) {
        return {
            ...
            login_number: harpiya.login_number,
            ...(options.context || {}),
        };
    }
    ...
    redirectToBackend() {
        window.location = "/harpiya/action-point_of_sale.action_client_pos_menu";
    }
