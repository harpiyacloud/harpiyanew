...
class MailBot(models.AbstractModel):
    ...
    def _apply_logic(self, channel, values, command=None):
        ...
        The logic will only be applied if harpiyabot is in a chat with a user or
        if someone pinged harpiyabot.

         :param channel: the discuss channel where the user message was posted/harpiyabot will answer.
        ...
        if values.get("author_id") == harpiyabot_id or values.get("message_type") != "comment" and not command:
            return
    ...
    def _get_style_dict():
        return {
            ...
            "bold_start": Markup("<b>"),
            "bold_end": Markup("</b>"),
            "command_start": Markup("<span class='o_harpiyabot_command'>"),
            "command_end": Markup("</span>"),
            "document_link_start": Markup("<a href='https://www.harpiya.com/documentation' target='_blank'>"),
            "document_link_end": Markup("</a>"),
            "slides_link_start": Markup("<a href='https://www.harpiya.com/slides' target='_blank'>"),
    ...
    def _get_answer(self, channel, body, values, command=False):
        ...
        if channel.channel_type == "chat" and harpiyabot in channel.channel_member_ids.partner_id:
            # main flow
            source = _("Thanks")
            description = _("This is a temporary canned response to see how canned responses work.")
            if harpiyabot_state == 'onboarding_emoji' and self._body_contains_emoji(body):
                self.env.user.harpiyabot_state = "onboarding_command"
                self.env.user.harpiyabot_failed = False
                return self.env._(
            ...
            elif harpiyabot_state == 'onboarding_command' and command == 'help':
                self.env.user.harpiyabot_state = "onboarding_ping"
                self.env.user.harpiyabot_failed = False
                return self.env._(
                    ...
                    "%(command_start)s@HarpiyaBot%(command_end)s in a sentence.",
                    **self._get_style_dict()
                )
            elif harpiyabot_state == "onboarding_ping" and harpiyabot.id in values.get("partner_ids", []):
                self.env.user.harpiyabot_state = "onboarding_attachement"
                self.env.user.harpiyabot_failed = False
                return self.env._(
            ...
            elif harpiyabot_state == "onboarding_attachement" and values.get("attachment_ids"):
            ...
            elif harpiyabot_state == "onboarding_canned" and self.env.context.get("canned_response_ids"):
                ...
                return self.env._(
                    ...
                    "%(command_start)sstart the tour%(command_end)s. Enjoy discovering Harpiya!",
                    **self._get_style_dict()
                )
            # repeat question if needed
            elif harpiyabot_state == 'onboarding_canned' and not self._is_help_requested(body):
                self.env.user.harpiyabot_failed = True
                return self.env._(
            ...
            elif harpiyabot_state in (False, "idle", "not_initialized") and (_('start the tour') in body.lower()):
                self.env.user.harpiyabot_state = "onboarding_emoji"
                return _("To start, try to send me an emoji :)")
            # easter eggs
            elif harpiyabot_state == "idle" and body in ['❤️', _('i love you'), _('love')]:
            ...
            # help message
            elif self._is_help_requested(body) or harpiyabot_state == 'idle':
                return self.env._(
                ...
                # repeat question
                if harpiyabot_state == 'onboarding_emoji':
                    self.env.user.harpiyabot_failed = True
                    return self.env._(
                ...
                elif harpiyabot_state == 'onboarding_attachement':
                    self.env.user.harpiyabot_failed = True
                    return self.env._(
                ...
                elif harpiyabot_state == 'onboarding_command':
                    self.env.user.harpiyabot_failed = True
                    return self.env._(
                ...
                elif harpiyabot_state == 'onboarding_ping':
                    self.env.user.harpiyabot_failed = True
                    return self.env._(
                        "Sorry, I am not listening. To get someone's attention, %(bold_start)sping "
                        "him%(bold_end)s. Write %(command_start)s@HarpiyaBot%(command_end)s and select"
                        " me.",
                        **self._get_style_dict()
                    )
    ...
    def _is_help_requested(self, body):
        ...
        """
        return any(token in body for token in ['help', _('help'), '?']) or self.env.user.harpiyabot_failed
