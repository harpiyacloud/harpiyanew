...
from harpiya.service.server import CommonServer
from harpiya.tools import json_default, SQL
...
def get_notify_payload_max_length(default=8000):
    try:
        length = int(os.environ.get('HARPIYA_NOTIFY_PAYLOAD_MAX_LENGTH', default))
    except ValueError:
        _logger.warning("HARPIYA_NOTIFY_PAYLOAD_MAX_LENGTH has to be an integer, "
                ...
                with harpiya.sql_db.db_connect("postgres").cursor() as cr:
                    for payload in payloads:
                        cr.execute(
                            SQL(
                                "SELECT %s('imbus', %s)",
                                SQL.identifier(HARPIYA_NOTIFY_FUNCTION),
                                payload,
                            )
                        )
