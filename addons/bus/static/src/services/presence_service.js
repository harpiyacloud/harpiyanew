...
export const presenceService = {
    start(env) {
        ...
        function onFocusChange(isFocused) {
            ...
            isHarpiyaFocused = isFocused;
            browser.localStorage.setItem(`${LOCAL_STORAGE_PREFIX}.focus`, isHarpiyaFocused);
            if (isHarpiyaFocused) {
                lastPresenceTime = luxon.DateTime.now().ts;
                env.bus.trigger("window_focus", isHarpiyaFocused);
            }
        }

        function onStorage({ key, newValue }) {
            if (key === `${LOCAL_STORAGE_PREFIX}.focus`) {
                isHarpiyaFocused = JSON.parse(newValue);
