...
from harpiya.addons.survey.tests import common
...
class TestSurveyInvite(common.TestSurveyCommon, MailCase):
    ...
    def test_survey_invite_with_template_attachment(self):
        ...
        invite_form.emails = 'test_survey_invite_with_template_attachment@harpiya.gov'
        invite = invite_form.save()
        with self.mock_mail_gateway():
            invite.action_invite()

        self.assertEqual(self.env['mail.mail'].sudo().search([
            ('email_to', '=', 'test_survey_invite_with_template_attachment@harpiya.gov')
